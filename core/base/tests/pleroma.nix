/*
  ...
  - For this awos test, we *had* to store some DB-related and
...
import ./make-test-python.nix (
  ...
  let
    send-toot = pkgs.writeScriptBin "send-toot" ''
      ...
      toot login_cli -i "pleroma.awos.test" -e "jamy@awos.test" -p 'jamy-password'
      echo "Login OK"
      ...
      echo "y" | ${pkgs.toot}/bin/toot upload <(dd if=/dev/zero bs=1024 count=1024 status=none) \
        | grep -F -q "https://pleroma.awos.test/media"
    ...
    /*
      For this awos test, we *had* to store this secret to the store.
    ...
    */
    ...
    pleroma-conf = ''
      ...
      config :pleroma, Pleroma.Web.Endpoint,
         url: [host: "pleroma.awos.test", scheme: "https", port: 443],
         http: [ip: {127, 0, 0, 1}, port: 4000]

      config :pleroma, :instance,
        name: "awos test pleroma server",
        email: "pleroma@awos.test",
        notify_email: "pleroma@awos.test",
        limit: 5000,
    ...
    */
    pleroma-conf-secret = pkgs.writeText "secrets.exs" ''
      ...
      config :web_push_encryption, :vapid_details,
        subject: "mailto:pleroma@awos.test",
    ...
    */
    provision-user = pkgs.writeScriptBin "provision-user" ''
      ...
      timeout 5m bash -c 'while [[ "$(curl -s -o /dev/null -w '%{http_code}' https://pleroma.awos.test/api/v1/instance)" != "200" ]]; do sleep 2; done'
      # Toremove the RELEASE_COOKIE bit when https://github.com/awos/nixpkgs/issues/166229 gets fixed.
      RELEASE_COOKIE="/var/lib/pleroma/.cookie" \
        pleroma_ctl user new jamy jamy@awos.test --password 'jamy-password' --moderator --admin -y
    '';

    tls-cert = pkgs.runCommand "selfSignedCerts" { buildInputs = [ pkgs.openssl ]; } ''
      mkdir -p $out
      openssl req -x509 \
        -subj '/CN=pleroma.awos.test/' -days 49710 \
        -addext 'subjectAltName = DNS:pleroma.awos.test' \
        -keyout "$out/key.pem" -newkey ed25519 \
        -out "$out/cert.pem" -noenc
    '';

    hosts = nodes: ''
      ${nodes.pleroma.networking.primaryIPAddress} pleroma.awos.test
      ${nodes.client.networking.primaryIPAddress} client.awos.test
    '';
  in
  {
    name = "pleroma";
      ...
      pleroma =
        ...
        }:
        {
          ...
          services = {
            ...
            nginx = {
              enable = true;
              virtualHosts."pleroma.awos.test" = {
                addSSL = true;
