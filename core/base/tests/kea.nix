...
# towards a nameserver to allocate a name in the lan.awos.test zone.
...
}:
{
  ...
  nodes = {
      ...
      { config, pkgs, ... }:
      {
        ...
        services.kea.dhcp4 = {
          ...
          settings = {
            ...
            ddns-qualifying-suffix = "lan.awos.test.";
          };
        };
        ...
        services.kea.dhcp-ddns = {
          ...
          settings = {
            forward-ddns = {
              # Configure updates of a forward zone named `lan.awos.test`
    ...
    nameserver =
      { config, pkgs, ... }:
      {
        ...
        # Set up an authoritative nameserver, serving the `lan.awos.test`
        ...
        services.knot =
          let
            zone = pkgs.writeTextDir "lan.awos.test.zone" ''
              @ SOA ns.awos.test nox.awos.test 0 86400 7200 3600000 172800
              @ NS nameserver
  ...
  testScript =
    ...
    ''
      ...
      nameserver.wait_until_succeeds("kdig +short client.lan.awos.test @10.0.0.2 | grep -q 10.0.0.3")
