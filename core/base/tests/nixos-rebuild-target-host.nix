...
}:
{
  ...
  testScript =
            ...
            { lib, modulesPath, ... }: {
              ...
              ${lib.optionalString withNg # nix
                ''
                  nixpkgs.overlays = [
                    (final: prev: {
                      # Set tmpdir inside awos-rebuild-ng to test
                      # "Deploy works with very long TMPDIR"
                      awos-rebuild-ng = prev.awos-rebuild-ng.override { withTmpdir = "/tmp"; };
                    })
                  ];
                ''
              }
    ...
    ''
      ...
      target.succeed("awos-generate-config")
      deployer.succeed("scp alice@target:/etc/awos/hardware-configuration.nix /root/hardware-configuration.nix")
      ...
      with subtest("Deploy to root@target"):
        deployer.succeed("awos-rebuild switch -I awos-config=/root/configuration-1.nix --target-host root@target &>/dev/console")
      ...
      with subtest("Deploy to alice@target with passwordless sudo"):
        deployer.succeed("awos-rebuild switch -I awos-config=/root/configuration-2.nix --target-host alice@target --sudo &>/dev/console")
        target_hostname = deployer.succeed("ssh alice@target cat /etc/hostname").rstrip()
        assert target_hostname == "config-2-deployed", f"{target_hostname=}"

      with subtest("Deploy to bob@target with password based sudo"):
        # TODO: investigate why --ask-sudo-password from awos-rebuild-ng is not working here
        deployer.succeed(r'${lib.optionalString withNg "NIX_SSHOPTS=-t "}passh -c 3 -C -p ${nodes.target.users.users.bob.password} -P "\[sudo\] password" awos-rebuild switch -I awos-config=/root/configuration-3.nix --target-host bob@target --sudo &>/dev/console')
        target_hostname = deployer.succeed("ssh alice@target cat /etc/hostname").rstrip()
      ...
      with subtest("Deploy works with very long TMPDIR"):
        ...
        deployer.succeed(f"TMPDIR={tmp_dir} awos-rebuild switch -I awos-config=/root/configuration-1.nix --target-host root@target &>/dev/console")
    '';
}
