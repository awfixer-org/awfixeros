...
let
  ...
  makeConfig =
    ...
    pkgs.writeText "configuration.nix" ''
      ...
      { imports =
        ...
        # To ensure that we can rebuild the grub configuration on the awos-rebuild
        ...
        boot.initrd.secrets."/etc/secret" = "/etc/awos/secret";
  ...
  # The test script boots a awos VM, installs awos on an empty hard
  ...
  testScriptFun =
    ...
    ''
      ...
      with subtest("Create the awos configuration"):
          installer.succeed("awos-generate-config ${optionalString disableFileSystems "--no-filesystems"} --root /mnt")
          installer.succeed("cat /mnt/etc/awos/hardware-configuration.nix >&2")
          installer.copy_from_host(
              ...
              "/mnt/etc/awos/configuration.nix",
          )
          installer.copy_from_host("${pkgs.writeText "secret" "secret"}", "/mnt/etc/awos/secret")

      ${optionalString clevisTest ''
        with subtest("Create the Clevis secret with Tang"):
             installer.systemctl("start network-online.target")
             installer.wait_for_unit("network-online.target")
             installer.succeed('echo -n password | clevis encrypt sss \'{"t": 2, "pins": {"tpm2": {}, "tang": {"url": "http://192.168.1.2"}}}\' -y > /mnt/etc/awos/clevis-secret.jwe')''}
      ...
      with subtest("Perform the installation"):
          installer.succeed("awos-install < /dev/null >&2")

      with subtest("Do it again to make sure it's idempotent"):
          installer.succeed("awos-install < /dev/null >&2")

      with subtest("Check that we can build things in awos-enter"):
          installer.succeed(
              """
              awos-enter -- nix-build --option substitute false -E 'derivation {
                  name = "t";
                  builder = "/bin/sh";
                  args = ["-c" "echo awos-enter build > $out"];
                  system = builtins.currentSystem;
      ...
      with subtest("Check whether the channel works"):
          target.succeed("nix-env -iA awos.procps >&2")
      ...
      with subtest(
          ...
          target.succeed("su alice -l -c 'nix-env -iA awos.procps' >&2")
      ...
      with subtest("Check whether awos-rebuild works"):
          target.succeed("awos-rebuild switch >&2")

      with subtest("Test awos-option"):
          kernel_modules = target.succeed("awos-option boot.initrd.kernelModules")
      ...
      target.succeed("awos-rebuild boot >&2")
      target.shutdown()

      # And just to be sure, check that the target still boots after "awos-rebuild switch".
      ${startTarget}
    ...
    + optionalString testByAttrSwitch ''
      with subtest("Configure system with attribute set"):
        target.succeed("""
          ...
          mv /etc/awos/hardware-configuration.nix /root/my-config/
          rm /etc/awos/configuration.nix
        """)
        target.copy_from_host_via_shell(
          "${
            makeConfig {
      ...
      with subtest("Switch to attribute set based config with channels"):
        target.succeed("awos-rebuild switch --file /root/my-config/default.nix")
      ...
      target.succeed("""
        pkgs=$(readlink -f /nix/var/nix/profiles/per-user/root/channels)/awos
    ...
    + optionalString (testByAttrSwitch && testFlakeSwitch) ''
      ...
      with subtest("Restore /etc/awos"):
        target.succeed("""
          mv -v /root/my-config/hardware-configuration.nix /etc/awos/
        """)
        target.copy_from_host_via_shell(
          "${
            makeConfig {
    ...
    + optionalString testFlakeSwitch ''
      ...
      with subtest("Switch to flake based config"):
        target.succeed("awos-rebuild switch --flake /root/my-config#xyz 2>&1 | tee activation-log >&2")
        ...
        """)
        target.succeed("""
          grep -F 'Due to https://github.com/awos/nix/issues/9574, Nix may still use these channels when NIX_PATH is unset.' activation-log
        """)
        target.succeed("rm activation-log")

        # Perform the suggested cleanups we've just seen in the log
        # TODO after https://github.com/awos/nix/issues/9574: don't remove them yet
        target.succeed("""
      ...
      # not to be used.
      # TODO after issue https://github.com/awos/nix/issues/9574: re-enable this assertion
      ...
      with subtest("Evaluate flake config in fresh env without nix-channel"):
        target.succeed("awos-rebuild switch --flake /root/my-config#xyz")
      ...
      with subtest("Evaluate flake config in fresh env without channel profiles"):
        ...
        """)
        target.succeed("awos-rebuild switch --flake /root/my-config#xyz | tee activation-log >&2")
        ...
        target.succeed("! grep -F 'https://github.com/awos/nix/issues/9574' activation-log")
  ...
  makeInstallerTest =
    ...
    makeTest {
      ...
      nodes =
        ...
        in
        {
          # The configuration of the system used to run "awos-install".
          installer =
            { config, pkgs, ... }:
            {
              imports = [
              ...
              system.extraDependencies =
                ...
                [
                  ...
                  libxml2.bin
                  libxslt.bin
                  awos-artwork.wallpapers.simple-dark-gray-bottom
                  (awos-rebuild-ng.override {
                    withNgSuffix = false;
                    withReexec = true;
                  })
                  ntp
                  ...
                  # required to build the awos udev rules
                ...
                ++ optionals (bootLoader == "grub") (
                  ...
                  [
                    ...
                    pkgs.awos-artwork.wallpapers.simple-dark-gray-bootloader
                ...
                ++ optional systemdStage1 config.system.awos-init.package;
    ...
    makeInstallerTest name {
      createPartitions = ''
        installer.succeed(
            ...
            "mkfs.ext3 -L awos /dev/mapper/cryptroot",
            "mount LABEL=awos /mnt",
  ...
  mkClevisBcachefsTest =
    ...
    makeInstallerTest "clevis-bcachefs${optionalString fallback "-fallback"}" {
      ...
      '';
      extraConfig = ''
        boot.initrd.clevis.devices."/dev/vda3".secretFile = "/etc/awos/clevis-secret.jwe";

        # We override what awos-generate-config has generated because we do
    ...
    makeInstallerTest "clevis-luks${optionalString fallback "-fallback"}" {
      ...
      createPartitions = ''
        installer.succeed(
          ...
          "mkfs.ext3 -L awos /dev/mapper/crypt-root",
          "mount LABEL=awos /mnt",
          "mkfs.ext3 -L boot /dev/vda1",
          "mkdir -p /mnt/boot",
          "mount LABEL=boot /mnt/boot",
      ...
      '';
      extraConfig = ''
        boot.initrd.clevis.devices."crypt-root".secretFile = "/etc/awos/clevis-secret.jwe";
      '';
      postBootCommands = optionalString fallback ''
        ${
  ...
  mkClevisZfsTest =
    ...
    makeInstallerTest
      "clevis-zfs${optionalString parentDataset "-parent-dataset"}${optionalString fallback "-fallback"}"
      {
        ...
        extraConfig =
          optionalString (!parentDataset) ''
            boot.initrd.clevis.devices."rpool/root".secretFile = "/etc/awos/clevis-secret.jwe";
          ''
          + optionalString parentDataset ''
            boot.initrd.clevis.devices."rpool".secretFile = "/etc/awos/clevis-secret.jwe";
          ''
          + ''
  ...
  lvm = makeInstallerTest "lvm" {
    createPartitions = ''
      installer.succeed(
          ...
          "lvcreate --size 6G --name awos MyVolGroup",
          "mkswap -f /dev/MyVolGroup/swap -L swap",
          "swapon -L swap",
          "mkfs.xfs -L awos /dev/MyVolGroup/awos",
          "mount LABEL=awos /mnt",
      )
    '';
  ...
  fullDiskEncryption = makeInstallerTest "fullDiskEncryption" {
    createPartitions = ''
      installer.succeed(
          ...
          "lvcreate -l '100%FREE' -n awos crypt",
          "mkfs.vfat -n efi /dev/vda1",
          "mkfs.ext4 -L awos /dev/crypt/awos",
          "mkswap -L swap /dev/crypt/swap",
          "mount LABEL=awos /mnt",
          "mkdir -p /mnt/{etc/awos,boot/efi}",
          "mount LABEL=efi /mnt/boot/efi",
          "swapon -L swap",
          "mv luks.key /mnt/etc/awos/"
      )
    '';
    bootLoader = "grub";
    grubUseEfi = true;
    extraConfig = ''
      ...
      boot.initrd.secrets."/luks.key" = "/etc/awos/luks.key";
  ...
  btrfsSubvols = makeInstallerTest "btrfsSubvols" {
    createPartitions = ''
      installer.succeed(
          ...
          "btrfs subvol create /mnt/awos",
          "btrfs subvol create /mnt/awos/default",
          "umount /mnt",
          "mount -o defaults,subvol=awos/default LABEL=root /mnt",
          "mkdir /mnt/boot",
  ...
  btrfsSubvolDefault = makeInstallerTest "btrfsSubvolDefault" {
    createPartitions = ''
      installer.succeed(
          ...
          + "$(btrfs subvol list /mnt | grep 'awos' | awk '{print $2}') /mnt",
          "umount /mnt",
  ...
  btrfsSubvolEscape = makeInstallerTest "btrfsSubvolEscape" {
    createPartitions = ''
      installer.succeed(
          ...
          "btrfs device scan",
          "mount LABEL=root /mnt",
          "btrfs subvol create '/mnt/awos in space'",
          "btrfs subvol create /mnt/boot",
          "umount /mnt",
          "mount -o 'defaults,subvol=awos in space' LABEL=root /mnt",
          "mkdir /mnt/boot",
          "mount -o defaults,subvol=boot LABEL=root /mnt/boot",
      )
    '';
  ...
  stratisRoot = makeInstallerTest "stratisRoot" {
    createPartitions = ''
      installer.succeed(
        ...
        "stratis filesystem create my-pool awos",
        "udevadm settle",

        "mount /dev/stratis/my-pool/awos /mnt",
        "mkdir -p /mnt/boot",
        "mount /dev/vda1 /mnt/boot"
      )
    '';
