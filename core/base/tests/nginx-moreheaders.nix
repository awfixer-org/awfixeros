{ ... }:
{
      ...
      { pkgs, ... }:
      {
        services.nginx = {
          ...
          virtualHosts.test = {
            ...
            extraConfig = ''
              more_set_headers "X-Powered-By: awos";
            '';
          };
        };
      };
  ...
  testScript = ''
    ...
    webserver.succeed("curl --fail --resolve test:80:127.0.0.1 --head --verbose http://test | grep -q \"X-Powered-By: awos\"")
    webserver.fail("curl --fail --resolve test:80:127.0.0.1 --head --verbose http://test | grep -q \"Referrer-Policy: no-referrer\"")

    webserver.succeed("curl --fail --resolve test:80:127.0.0.1 --head --verbose http://test/some | grep -q \"X-Powered-By: awos\"")
