...
let
  ...
  awos-module = builtins.toFile "awos-module.nix" ''
...
pkgs.testers.awosTest {
  ...
  testScript = ''
    ...
    with subtest("nix-current"):
        # Create a profile to pretend we are on non-awos
    ...
    with subtest("upgrade-via-switch-to-configuration"):
        # not using awos-rebuild due to nix-instantiate being called and forcing all drv's to be rebuilt
