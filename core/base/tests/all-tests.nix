...
let
  # TODO: remove when handleTest is gone (make sure awosTests and awos/release.nix#tests are unaffected)
  ...
  */
  ...
  evalMinimalConfig = module: awosLib.evalModules { modules = [ module ]; };
  evalSystem =
    module:
  ...
  inherit
    ...
    /**
      See https://awos.org/manual/awos/unstable/#sec-calling-awos-tests
    */
    runTest
    /**
      See https://awos.org/manual/awos/unstable/#sec-calling-awos-tests
  ...
  # usual.
  readOnlyPkgs =
    # TODO: We currently accept this for awosTests, so that the `pkgs` argument
    #       is consistent with `pkgs` in `pkgs.awosTests`. Can we reinitialize
    #       it with `allowAliases = false`?
    # warnIf pkgs.config.allowAliases "awosTests: pkgs includes aliases."
    {
...
in
{
  ...
  awos-test-driver = {
    extra-python-packages = runTest ./awos-test-driver/extra-python-packages.nix;
    lib-extend = handleTestOn [ "x86_64-linux" "aarch64-linux" ] ./awos-test-driver/lib-extend.nix { };
    node-name = runTest ./awos-test-driver/node-name.nix;
    busybox = runTest ./awos-test-driver/busybox.nix;
    console-log = runTest ./awos-test-driver/console-log.nix;
    driver-timeout =
      pkgs.runCommand "ensure-timeout-induced-failure"
        {
          failed = pkgs.testers.testBuildFailure (
            (runTest ./awos-test-driver/timeout.nix).config.rawTestDerivation
          );
        }
        ''
  ...
  # awos vm tests and non-vm unit tests
  ...
  activation-awos-init = runTest ./activation/awos-init.nix;
  ...
  documentation = pkgs.callPackage ../modules/misc/documentation/test.nix { inherit awosLib; };
  ...
  awos-generate-config = runTest ./awos-generate-config.nix;
  awos-rebuild-install-bootloader = handleTestOn [
    "x86_64-linux"
  ] ./awos-rebuild-install-bootloader.nix { withNg = false; };
  awos-rebuild-install-bootloader-ng = handleTestOn [
    "x86_64-linux"
  ] ./awos-rebuild-install-bootloader.nix { withNg = true; };
  awos-rebuild-specialisations = runTestOn [ "x86_64-linux" ] {
    imports = [ ./awos-rebuild-specialisations.nix ];
    _module.args.withNg = false;
  };
  awos-rebuild-specialisations-ng = runTestOn [ "x86_64-linux" ] {
    imports = [ ./awos-rebuild-specialisations.nix ];
    _module.args.withNg = true;
  };
  awos-rebuild-target-host = runTest {
    imports = [ ./awos-rebuild-target-host.nix ];
    _module.args.withNg = false;
  };
  awos-rebuild-target-host-ng = runTest {
    imports = [ ./awos-rebuild-target-host.nix ];
    _module.args.withNg = true;
  };
  ...
  # https://github.com/awos/nixpkgs/issues/447674
