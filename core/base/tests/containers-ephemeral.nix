{ pkgs, lib, ... }:
{
  ...
  testScript = ''
    ...
    machine.succeed("awos-container start webserver")

    with subtest("Container got its own root folder"):
        machine.succeed("ls /run/awos-containers/webserver")

    with subtest("Container persistent directory is not created"):
        machine.fail("ls /var/lib/awos-containers/webserver")

    # Since "start" returns after the container has reached
    # multi-user.target, we should now be able to access it.
    ip = machine.succeed("awos-container show-ip webserver").rstrip()
    ...
    with subtest("Container's root folder was removed"):
        machine.fail("ls /run/awos-containers/webserver")
  '';
}
