...
}:
{
  ...
  nodes.machine1 =
    { config, ... }:
    {
      ...
      services.davis = {
        ...
        adminPasswordFile = "${pkgs.writeText "davisAdminPass" "awos"}";
      };
    };
  nodes.machine2 =
    { nodes, config, ... }:
  ...
  testScript = ''
    ...
    with subtest("login works"):
        ...
        r = machine1.succeed(
            f"curl -b /tmp/cookies --resolve davis.example.com:80:127.0.0.1 http://davis.example.com/login -X POST -F _username=admin -F _password=awos -F _csrf_token={csrf_token.strip()} -D headers"
        )
        print(r)
        machine1.succeed(
