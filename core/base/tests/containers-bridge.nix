...
{ pkgs, lib, ... }:
{
  ...
  testScript = ''
    ...
    assert "webserver" in machine.succeed("awos-container list")

    with subtest("Start the webserver container"):
        assert "up" in machine.succeed("awos-container status webserver")

    with subtest("Bridges exist inside containers"):
        machine.succeed(
            "awos-container run webserver -- ip link show eth0",
            "awos-container run web-noip -- ip link show eth0",
        )
    ...
    with subtest(
        "awos-container show-ip works in case of an ipv4 address "
        + "with subnetmask in CIDR notation."
    ):
        result = machine.succeed("awos-container show-ip webserver").rstrip()
        assert result == ip

    with subtest("Stop the container"):
        machine.succeed("awos-container stop webserver")
        machine.fail(
    ...
    machine.fail("awos-container destroy webserver")
  '';
}
