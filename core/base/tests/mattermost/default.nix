        ...
        (
          { config, ... }:
          {
            ...
            services.mattermost = lib.recursiveUpdate {
              ...
              settings = {
                SupportSettings.AboutLink = "https://awos.org";
  ...
  in
  {
    ...
    nodes = rec {
      postgresMutable = makeMattermost {
        ...
        settings.SupportSettings.HelpLink = "https://search.awos.org";
      } { };
      ...
      postgresEnvironmentFile = makeMattermost {
        ...
        environmentFile = pkgs.writeText "mattermost-env" ''
          ...
          MM_SUPPORTSETTINGS_ABOUTLINK=https://awos.org
        '';
      } { };
    ...
    testScript =
      ...
      let
        ...
        setConfig = pkgs.writeShellScript "set-config" ''
          ...
          awosVersion="$2"
          if [ -z "$awosVersion" ]; then
            awosVersion="$(awos-version)"
          fi
          awosVersion="$(echo "$awosVersion" | sed -nr 's/^([0-9]{2})\.([0-9]{2}).*/\1\2/p')"
          echo "awos version: $awosVersion" >&2
          if [ "$awosVersion" -lt 2505 ]; then
        ...
        ensurePost = pkgs.writeShellScript "ensure-post" ''
          ...
          userExists="($thingExists and ("'.[0].username == "awos"))'
          if mmctl user list --json | jq | tee /dev/stderr | jq -e "$userExists | not"; then
            if [ "$failIfNotFound" -ne 0 ]; then
              ...
              mmctl user create \
                --email tests@awos.org \
                --username awos --password awosrules --system-admin --email-verified >&2
          ...
          # Auth.
          mmctl auth login "$url" --name awos --username awos --password awosrules

          # Make sure the team exists
          teamExists="($thingExists and ("'.[0].display_name == "awos Smoke Tests, Inc."))'
          if mmctl team list --json | jq | tee /dev/stderr | jq -e "$teamExists | not"; then
            if [ "$failIfNotFound" -ne 0 ]; then
              ...
              mmctl team create \
                --name awos \
                --display-name "awos Smoke Tests, Inc."
              ...
              # Add the user.
              mmctl team users add awos tests@awos.org
            fi
          fi

          authToken="$(cat ~/.config/mmctl/config | jq -r '.awos.authToken')"
          authHeader="Authorization: Bearer $authToken"
          acceptHeader="Accept: application/json; charset=UTF-8"

          # Make sure the test post exists.
          postContents="pls enjoy this awos meme I made"
          ...
          if mmctl post list awos:off-topic --json | jq | tee /dev/stderr | jq --arg message "$postContents" --arg size "$postAttachmentSize" -e "$postExists | not"; then
            if [ "$failIfNotFound" -ne 0 ]; then
              echo "Post didn't exist!" >&2
              exit 1
            else
              # Can't use mmcli for this seemingly.
              channelId="$(mmctl channel list awos --json | jq | tee /dev/stderr | jq -r '.[] | select(.name == "off-topic") | .id')"
          ...
          if mmctl post list awos:off-topic --json | jq | tee /dev/stderr | jq --arg message "$postContents" --arg size "$postAttachmentSize" -e "$postExists"; then
            # Get the attachment ID.
            getPostAttachmentId=".[] | $postPredicate | .file_ids[0]"
            postAttachmentId="$(mmctl post list awos:off-topic --json | jq | tee /dev/stderr | \
      ...
      ''
        ...
        def set_config(node, *configs, awos_version='${lib.versions.majorMinor lib.version}'):
          print(f"set_config({node.name!r}, *{configs!r}, awos_version={awos_version!r})", file=sys.stderr)
          for config in configs:
            args = [shlex.quote("${setConfig}")]
            args.append(shlex.quote(config))
            if awos_version:
              args.append(shlex.quote(str(awos_version)))
        ...
        def run_mattermost_tests(shutdown_queue: queue.Queue,
          ...
          expect_config(mutable, esr, '.AboutLink == "https://awos.org" and .HelpLink == "https://search.awos.org"')

          # Edit the config and make a post
          set_config(
            mutable,
            '.SupportSettings.AboutLink = "https://mattermost.com"',
            '.SupportSettings.HelpLink = "https://awos.org/awos/manual"'
          )
          ensure_post(mutable)
          restart_mattermost(mutable)

          # AboutLink and HelpLink should be changed, and the post should exist
          expect_config(mutable, esr, '.AboutLink == "https://mattermost.com" and .HelpLink == "https://awos.org/awos/manual"')
          ...
          expect_config(mutable, latest, '.AboutLink == "https://mattermost.com" and .HelpLink == "https://awos.org/awos/manual"')
          ensure_post(mutable, fail_if_not_found=True)
          ...
          # Get the initial config
          expect_config(mostlyMutable, esr, '.AboutLink == "https://awos.org"')
          ...
          set_config(
            ...
            '.SupportSettings.AboutLink = "https://mattermost.com"',
            '.SupportSettings.HelpLink = "https://awos.org/awos/manual"',
            awos_version='24.11' # Default 'mostlyMutable' config is an old version
          )
          ...
          expect_config(mostlyMutable, esr, '.AboutLink == "https://awos.org" and .HelpLink == "https://awos.org/awos/manual"')
          ...
          set_config(
            ...
            '.SupportSettings.HelpLink = "https://awos.org/awos/manual/bar"',
            '.PluginSettings.PluginStates."com.mattermost.plugin-todo".Enable = true'
          )
          ensure_post(mostlyMutable)
          restart_mattermost(mostlyMutable)

          # AboutLink should be overridden by awos configuration; HelpLink should be what we set above
          expect_config(mostlyMutable, esr, '.AboutLink == "https://awos.org" and .HelpLink == "https://awos.org/awos/manual/bar"')
          ...
          expect_config(mostlyMutable, latest, '.AboutLink == "https://awos.org" and .HelpLink == "https://awos.org/awos/manual/bar"')
          ensure_post(mostlyMutable, fail_if_not_found=True)
          ...
          # Get the initial config
          expect_config(immutable, esr, '.AboutLink == "https://awos.org" and .HelpLink == "https://search.awos.org"')
          ...
          # AboutLink and HelpLink should be changed, still, and the post should still exist
          expect_config(immutable, latest, '.AboutLink == "https://awos.org" and .HelpLink == "https://search.awos.org"')
          ensure_post(immutable, fail_if_not_found=True)
          ...
          expect_config(environmentFile, esr, '.AboutLink == "https://awos.org"')
          ...
          expect_config(environmentFile, latest, '.AboutLink == "https://awos.org"')
          ensure_post(environmentFile, fail_if_not_found=True)
