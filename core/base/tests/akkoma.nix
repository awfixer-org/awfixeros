...
let
  ...
  tlsCertA = tlsCert [
    "akkoma-a.awos.test"
    "media.akkoma-a.awos.test"
  ];

  tlsCertB = tlsCert [
    "akkoma-b.awos.test"
    "media.akkoma-b.awos.test"
  ];
  ...
  commonConfig =
    { nodes, ... }:
    {
      ...
      networking.extraHosts = ''
        ${nodes.akkoma-a.networking.primaryIPAddress} akkoma-a.awos.test media.akkoma-a.awos.test
        ${nodes.akkoma-b.networking.primaryIPAddress} akkoma-b.awos.test media.akkoma-b.awos.test
        ${nodes.client-a.networking.primaryIPAddress} client-a.awos.test
        ${nodes.client-b.networking.primaryIPAddress} client-b.awos.test
      '';
    };
    ...
    { config, pkgs, ... }:
    {
      ...
      services.akkoma = {
        ...
        config = {
          ":pleroma" = {
            ":instance" = {
              name = "awos test Akkoma server";
              description = "awos test Akkoma server";
              email = "akkoma@awos.test";
              notify_email = "akkoma@awos.test";
...
in
{
  ...
  testScript = ''
    ...
    def registerUser(user, password):
      return 'pleroma_ctl user new {0} {0}@awos.test --password {1} -y'.format(
        quote(user), quote(password))

    def loginUser(instance, user, password):
      return 'toot login_cli -i {}.awos.test -e {}@awos.test -p {}'.format(
    ...
    client_b.succeed('toot follow {}@akkoma-a.awos.test'.format(userAName))
    ...
    client_a.succeed('${lib.getExe checkFe} akkoma-a.awos.test')
    client_b.succeed('${lib.getExe checkFe} akkoma-b.awos.test')
  '';
}
