    ...
    testScript = # python
      ''
        ...
        with subtest("should create a sample secret object"):
            k3s.succeed("k3s kubectl create secret generic awossecret --from-literal thesecret=abacadabra")

        with subtest("should check if secret is correct"):
            k3s.wait_until_succeeds("[[ $(kubectl get secrets awossecret -o json | jq -r .data.thesecret | base64 -d) == abacadabra ]]")

        with subtest("should have a secret in database"):
            etcd.wait_until_succeeds("[[ $(etcdctl get /registry/secrets/default/awossecret | head -c1 | wc -c) -ne 0 ]]")

        with subtest("should delete the secret"):
            k3s.succeed("k3s kubectl delete secret awossecret")

        with subtest("should not have a secret in database"):
            etcd.wait_until_fails("[[ $(etcdctl get /registry/secrets/default/awossecret | head -c1 | wc -c) -ne 0 ]]")
      '';
