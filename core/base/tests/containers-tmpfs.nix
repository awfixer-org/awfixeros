{ pkgs, lib, ... }:
{
  ...
  testScript = ''
    ...
    assert "tmpfs" in machine.succeed("awos-container list")

    with subtest("tmpfs container is up"):
        assert "up" in machine.succeed("awos-container status tmpfs")


    def tmpfs_cmd(command):
        return f"awos-container run tmpfs -- {command} 2>/dev/null"
    ...
    with subtest(
        ...
        machine.succeed(
            ...
            "test -e /var/lib/awos-containers/tmpfs/root/test.file",
        )
    ...
    with subtest(
        ...
        machine.fail("test -e /var/lib/awos-containers/tmpfs/some/random/path/test.file")
    ...
    with subtest(
        ...
        machine.succeed(
            "touch /var/lib/awos-containers/tmpfs/var/test.file",
            "test -e /var/lib/awos-containers/tmpfs/var/test.file",
            "ls -l /var/lib/awos-containers/tmpfs/var/ | grep -q test.file 2>/dev/null",
        )
