    ...
    ''
      ...
      def setup_clients_for(org, user, extra_init=no_extra_init):
          for client in [client1, client2]:
              with client.nested(f"initialize client for user {user}"):
                  ...
                  exportinfo = server.succeed(f"awos-taskserver user export {org} {user}")
      ...
      def re_add_imperative_user():
          with server.nested("(re-)add imperative user bar"):
              server.execute("awos-taskserver org remove imperativeOrg")
              server.succeed(
                  "awos-taskserver org add imperativeOrg",
                  "awos-taskserver user add imperativeOrg bar",
              )
      ...
      server.succeed(
          "awos-taskserver user list testOrganisation | grep -qxF alice",
          "awos-taskserver user list testOrganisation | grep -qxF foo",
          "awos-taskserver user list anotherOrganisation | grep -qxF bob",
      )
      ...
      server.fail("awos-taskserver user add imperativeOrg bar")
      ...
      with subtest("checking certificate revocation of user bar"):
          ...
          server.succeed("awos-taskserver user remove imperativeOrg bar")
          restart_server()
      ...
      with subtest("checking certificate revocation of org imperativeOrg"):
          ...
          server.succeed("awos-taskserver org remove imperativeOrg")
          restart_server()
      ...
      with subtest("check manual configuration"):
          ...
          server.succeed(
              "awos-taskserver org add manualOrg",
              "awos-taskserver user add manualOrg alice",
          )
