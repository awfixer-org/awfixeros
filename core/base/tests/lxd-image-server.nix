...
in
{
  ...
  testScript = ''
    ...
    machine.succeed(
        "incus image import ${incus-image-metadata} ${incus-image-rootfs} --alias awos"
    )

    loc = "/var/www/simplestreams/images/iats/awos/amd64/default/v1"

    with subtest("push image to server"):
        machine.succeed("incus launch awos test")
        machine.sleep(5)
