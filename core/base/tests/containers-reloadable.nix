{ pkgs, lib, ... }:
{
      ...
      { lib, ... }:
      {
        ...
        specialisation.c1.configuration = {
          containers.test1.config = {
            ...
            services.httpd.adminAddr = "awos@example.com";
          };
        };
  ...
  testScript = ''
    ...
    assert "client_base" in machine.succeed("awos-container run test1 cat /etc/check")
    ...
    with subtest("httpd is available after activating config1"):
        machine.succeed(
            ...
            "[[ $(awos-container run test1 cat /etc/check) == client_c1 ]] >&2",
    ...
    with subtest("httpd is not available any longer after switching to config2"):
        machine.succeed(
            ...
            "[[ $(awos-container run test1 cat /etc/check) == client_c2 ]] >&2",
