{ pkgs, lib, ... }:
{
  ...
  nodes.machine =
    { pkgs, ... }:
    {
      services.static-web-server = {
        ...
        listen = "[::]:8080";
        root = toString (
          pkgs.writeTextDir "awos-test.html" ''
            <h1>Hello awos!</h1>
          ''
        );
        configuration = {
  ...
  testScript = ''
    ...
    assert "awos-test.html" in response, "The directory listing page did not include a link to our awos-test.html file"
    response = machine.succeed("curl -fsS localhost:8080/awos-test.html")
    assert "Hello awos!" in response
