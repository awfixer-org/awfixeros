    ...
    images = init: {
      container = {
        metadata =
          ...
          + "/tarball/awos-image-lxc-*-${pkgs.stdenv.hostPlatform.system}.tar.xz";

        rootfs =
          (releases init).incusContainerImage.${pkgs.stdenv.hostPlatform.system}
          + "/awos-lxc-image-${pkgs.stdenv.hostPlatform.system}.squashfs";
      };

      virtual-machine = {
        metadata =
          (releases init).incusVirtualMachineImageMeta.${pkgs.stdenv.hostPlatform.system} + "/*/*.tar.xz";
        disk = (releases init).incusVirtualMachineImage.${pkgs.stdenv.hostPlatform.system} + "/awos.qcow2";
      };
    };
    ...
    + lib.optionalString canTestVm (
      (lib.foldl (
        ...
        + ''
          ...
          alias = "awos/virtual-machine/${variant}"
          variant = "${variant}"
      ...
      lib.optionalString featureUser # python
        ''
          ...
          with subtest("incus-user allows users to launch instances"):
              machine.succeed("su - testuser bash -c 'incus image import ${(images "systemd").container.metadata} ${(images "systemd").container.rootfs} --alias awos'")
              machine.succeed("su - testuser bash -c 'incus launch awos instance2'")
