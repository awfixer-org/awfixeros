{ pkgs, lib, ... }:
{
  ...
  testScript = ''
    ...
    with subtest("Can set up a wireless access point"):
        ...
        client.wait_until_succeeds("connmanctl tether wifi on awos-test reproducibility | grep -q 'Enabled'")
        client.wait_until_succeeds("iw wlan0 info | grep -q awos-test")
  '';
}
