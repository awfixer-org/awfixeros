...
let
  ...
  container =
    { config, ... }:
    {
      # We re-use the awos container option ...
...
in
{
  ...
  nodes.machine =
    { lib, ... }:
    {
      ...
      # do not try to access cache.awos.org
      ...
      networking.firewall.extraCommands = ''
        ${pkgs.iptables}/bin/iptables -A awos-fw -i ve-+ -p udp -m udp --dport 67 -j awos-fw-accept
      '';
    };
  ...
  testScript = ''
    ...
    # Install container
    # Workaround for awos-install
    machine.succeed("chmod o+rx /var/lib/machines");
    machine.succeed("awos-install --root ${containerRoot} --system ${containerSystem} --no-channel-copy --no-root-passwd");
    ...
    # https://awos.org/guides/nix-pills/07-working-derivation
