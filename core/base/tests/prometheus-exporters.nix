  ...
  exporterTests = {
    ...
    domain = {
      ...
      exporterTest = ''
        ...
        succeed("curl -sSf 'http://localhost:9222/probe?target=awos.org'")
      '';
    };
    ...
    influxdb = {
      ...
      exporterTest = ''
        ...
        succeed(
          "curl -XPOST http://localhost:9122/write --data-binary 'influxdb_exporter,distro=awos,added_in=21.09 value=1'"
        )
        succeed(
          "curl -sSf http://localhost:9122/metrics | grep 'awos'"
        )
        execute("sleep 5")
        fail(
          "curl -sSf http://localhost:9122/metrics | grep 'awos'"
