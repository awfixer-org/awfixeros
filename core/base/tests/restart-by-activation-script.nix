      ...
      system.activationScripts.test = {
        ...
        text = ''
          if [ -e /test-the-activation-script ]; then
            if [ "$awos_ACTION" != dry-activate ]; then
              touch /activation-was-run
              echo restart-me.service > /run/awos/activation-restart-list
              echo reload-me.service > /run/awos/activation-reload-list
            else
              echo restart-me.service > /run/awos/dry-activation-restart-list
              echo reload-me.service > /run/awos/dry-activation-reload-list
            fi
          fi
        '';
      };
  ...
  testScript = # python
    ''
      ...
      with subtest("dry activation"):
          ...
          machine.fail("test -f /run/awos/dry-activation-restart-list")
          machine.fail("test -f /run/awos/dry-activation-reload-list")
      ...
      with subtest("real activation"):
          ...
          machine.fail("test -f /run/awos/activation-restart-list")
          machine.fail("test -f /run/awos/activation-reload-list")
    '';
}
