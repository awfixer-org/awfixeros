...
in
{
  ...
  testScript = ''
    ...
    # Create a test user
    main.succeed("nipap-passwd add -u awostest -p nIx0st3st -n 'awos Test User'")

    # Try to log in with it on the web frontend
    main.succeed("curl -fvvv -Ls -b \"\" -d username=awostest -d password=nIx0st3st http://localhost:21337/auth/login | grep 'PrefixListController'")
