...
in
{
  ...
  testScript = ''
    ...
    machine.succeed("bootctl set-default awos-generation-1-specialisation-boot-luks-wrong-keyfile.conf")
    ...
    machine.succeed("bootctl set-default awos-generation-1-specialisation-boot-luks-missing-keyfile.conf")
    machine.succeed("sync")
    machine.crash()
