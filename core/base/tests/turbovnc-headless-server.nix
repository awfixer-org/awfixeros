{ pkgs, lib, ... }:
{
  ...
  nodes.machine =
    { pkgs, ... }:
    {
      ...
      # https://awos.org/manual/awos/stable/#sec-awos-test-port-forwarding
    ...
    def start_xvnc():
        xvnc_command = " ".join(
            [
                ...
                #     env QEMU_NET_OPTS=hostfwd=tcp::5900-:5900 $(nix-build awos/tests/turbovnc-headless-server.nix -A driver)/bin/awos-test-driver
