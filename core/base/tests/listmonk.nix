  ...
  { lib, ... }:
  {
    ...
    testScript = ''
      ...
      campaign_data = json.loads(machine.succeed(
        generate_listmonk_request('POST', 'campaigns/1/test', data={'template_id': templates[0]['id'], 'subscribers': ['john@example.com'], 'name': 'Test', 'subject': 'awos is great', 'lists': [1], 'messenger': 'email'})
      ))
