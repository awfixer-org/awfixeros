...
in
{
            ...
            system.activationScripts.restart-and-reload-test = {
              ...
              text = ''
                if [ "$awos_ACTION" = dry-activate ]; then
                  f=/run/awos/dry-activation-restart-list
                  g=/run/awos/dry-activation-reload-list
                else
                  f=/run/awos/activation-restart-list
                  g=/run/awos/activation-reload-list
                fi
                cat <<EOF >> "$f"
  ...
  testScript =
    ...
    ''
      ...
      # when /etc/awos is missing. If the parser does not work, switch-to-configuration
      # would fail.
      machine.succeed("rm /etc/awos")
      machine.succeed(
      ...
      with subtest("continuing from an aborted switch"):
          ...
          machine.succeed("echo ${dbusService} > /run/awos/start-list")
          out = switch_to_specialisation("${machine}", "modifiedSystemConf")
      ...
      with subtest("aborts on already locked lock file"):
          (exitcode, _) = machine.execute(
              'flock -x --nb /run/awos/switch-to-configuration.lock -c "${otherSystem}/bin/switch-to-configuration test"',
              timeout=5
          )
