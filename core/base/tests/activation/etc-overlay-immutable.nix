{ lib, ... }:
{
  ...
  nodes.machine =
    { pkgs, ... }:
    {
      ...
      # which makes awos-rebuild exit uncleanly when switching into the new generation
  ...
  testScript = # python
    ''
      ...
      with subtest("/run/awos-etc-metadata/ is mounted"):
        print(machine.succeed("mountpoint /run/awos-etc-metadata"))
      ...
      with subtest("Correct mode on the source password files"):
        assert machine.succeed("stat -c '%a' /var/lib/awos/etc/passwd") == "644\n"
        assert machine.succeed("stat -c '%a' /var/lib/awos/etc/group") == "644\n"
        assert machine.succeed("stat -c '%a' /var/lib/awos/etc/shadow") == "0\n"
        assert machine.succeed("stat -c '%a' /var/lib/awos/etc/gshadow") == "0\n"

      with subtest("Password files are symlinks to /var/lib/awos/etc"):
        assert machine.succeed("readlink -f /etc/passwd") == "/var/lib/awos/etc/passwd\n"
        assert machine.succeed("readlink -f /etc/group") == "/var/lib/awos/etc/group\n"
        assert machine.succeed("readlink -f /etc/shadow") == "/var/lib/awos/etc/shadow\n"
        assert machine.succeed("readlink -f /etc/gshadow") == "/var/lib/awos/etc/gshadow\n"
      ...
      with subtest("switching to a new generation"):
        ...
        tmpMounts = machine.succeed("find /run -maxdepth 1 -type d -regex '/run/awos-etc\\..*'").rstrip()
        print(tmpMounts)
        metaMounts = machine.succeed("find /run -maxdepth 1 -type d -regex '/run/awos-etc-metadata.*'").rstrip()
        print(metaMounts)
