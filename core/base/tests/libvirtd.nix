{ pkgs, ... }:
{
  ...
  testScript =
    let
      awosInstallISO = (import ../release.nix { }).iso_minimal.${pkgs.stdenv.hostPlatform.system};
    ...
    ''
      ...
      with subtest("check if awos install iso boots, network and autostart works"):
        virthost.succeed(
          "virt-install -n awos --osinfo awos-unstable --memory 1024 --graphics none --disk `find ${awosInstallISO}/iso -type f | head -n1`,readonly=on --import --noautoconsole --autostart"
        )
        virthost.succeed("virsh domstate awos | grep running")
        virthost.wait_until_succeeds("ping -c 1 awos")
        virthost.succeed("virsh ${virshShutdownCmd} awos")
        virthost.wait_until_succeeds("virsh domstate awos | grep 'shut off'")
        virthost.shutdown()
        virthost.wait_for_unit("multi-user.target")
        virthost.wait_until_succeeds("ping -c 1 awos")
