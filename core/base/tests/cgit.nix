  ...
  testScript =
    ...
    ''
      ...
      server.succeed("sudo -u cgit ${pkgs.writeShellScript "setup-cgit-test-repo" ''
        ...
        { echo -n "cgit awos Test at "; date; } > date.txt
        git add date.txt
      ...
      server.succeed(
          "curl -fsS 'http://localhost/%28c%29git/some-repo/about/' | grep -F 'cgit awos Test at'"
      )

      # add real readme
      server.succeed("sudo -u cgit ${pkgs.writeShellScript "cgit-commit-readme" ''
        set -e
        echo '# cgit awos test README' > reference/README.md
      ...
      server.succeed(
          "curl -fsS 'http://localhost/%28c%29git/some-repo/about/' | grep -F '# cgit awos test README'"
      )
      server.fail(
          "curl -fsS 'http://localhost/%28c%29git/some-repo/about/' | grep -F 'cgit awos Test at'"
      )
