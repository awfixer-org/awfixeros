{ lib, pkgs, ... }:
{
  ...
  testScript = ''
    ...
    machine.succeed("bootctl set-default awos-generation-1-specialisation-boot-luks.conf")
    ...
    machine.succeed("bootctl set-default awos-generation-1-specialisation-boot-luks-custom-keymap.conf")
    machine.succeed("sync")
    machine.crash()
