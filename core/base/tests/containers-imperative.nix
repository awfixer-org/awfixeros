{ pkgs, lib, ... }:
{
  ...
  nodes.machine =
    ...
    }:
    {
      ...
      nix.settings.substituters = [ ]; # don't try to access cache.awos.org
  ...
  testScript =
    ...
    in
    ''
      with subtest("Make sure we have a awos tree (required by ‘awos-container create’)"):
          machine.succeed("PAGER=cat nix-env -qa -A awos.hello >&2")

      id1, id2 = None, None

      with subtest("Create some containers imperatively"):
          id1 = machine.succeed("awos-container create foo --ensure-unique-name").rstrip()
          machine.log(f"created container {id1}")

          id2 = machine.succeed("awos-container create foo --ensure-unique-name").rstrip()
      ...
      with subtest(f"Put the root of {id2} into a bind mount"):
          machine.succeed(
              f"mv /var/lib/awos-containers/{id2} /id2-bindmount",
              f"mount --bind /id2-bindmount /var/lib/awos-containers/{id1}",
          )

          ip1 = machine.succeed(f"awos-container show-ip {id1}").rstrip()
          ip2 = machine.succeed(f"awos-container show-ip {id2}").rstrip()
          assert ip1 != ip2
      ...
      with subtest(
          ...
          for id in id1, id2:
              important_path = f"/var/lib/awos-containers/{id}/very/important/data"
              machine.succeed(
      ...
      with subtest("Stop a container early"):
          machine.succeed(f"awos-container stop {id1}")
          machine.succeed(f"awos-container start {id1} >&2 &")
      ...
      with subtest("tmpfiles are present"):
          ...
          machine.succeed(
              "awos-container create tmpfiles --config-file ${tmpfilesContainerConfig}"
          )
          machine.log("created, starting…")
          machine.succeed("awos-container start tmpfiles")
          machine.log("done starting, investigating…")
          machine.succeed(
              "echo $(awos-container run tmpfiles -- systemctl is-active foo.service) | grep -q active;"
          )
          machine.succeed("awos-container destroy tmpfiles")
      ...
      with subtest("Destroy the containers"):
          for id in id1, id2:
              machine.succeed(f"awos-container destroy {id}")
      ...
      with subtest("Ensure that the container path is gone"):
          print(machine.succeed("ls -lsa /var/lib/awos-containers"))
          machine.succeed(f"test ! -e /var/lib/awos-containers/{id1}")

      with subtest("Ensure that a failed container creation doesn'leave any state"):
          machine.fail(
              "awos-container create b0rk --config-file ${brokenCfg}"
          )
          machine.succeed("test ! -e /var/lib/awos-containers/b0rk")
    '';
}
