...
in
{
    ...
    server = {
      services.armagetronad.servers = {
        ...
        sty = {
          ...
          roundSettings = {
            SAY = [
              "awos Smoke Test sty+ct+ap Server"
              "https://awos.org"
            ];
          };
        };
  ...
  testScript =
    ...
    ''
      ...
      # Server configs.
      servers = (
        Server(server, 'high-rubber', 'server', 4534, 'awos Smoke Test Server', 'SmOoThIcE', 'Arduino'),
        Server(server, 'sty', 'server', 4535, 'awos Smoke Test sty+ct+ap Server', 'Arduino', 'SmOoThIcE'),
        Server(server, 'trunk', 'server', 4536, 'awos Smoke Test 0.4 Server', 'Arduino', 'SmOoThIcE')
      )
      ...
      for srv in servers:
        ...
        srv.node.wait_until_succeeds(
          f"journalctl -u armagetronad-{srv.name} -e | grep -q 'Admin: https://awos.org'"
        )
        srv.node.wait_until_succeeds(
