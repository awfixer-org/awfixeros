    ...
    (import ../lib/eval-config.nix {
      ...
      modules = [
        ...
        ../modules/profiles/qemu-guest.nix
        {
          ...
          # In a awos test the serial console is occupied by the "backdoor"
          ...
          # Needed by awos-rebuild due to the lack of network
          # access. Determined by trial and error.
          system.extraDependencies = with pkgs; [
            # Needed for a awos-rebuild.
            busybox
            cloud-utils
            desktop-file-utils
            libxslt.bin
...
in
{
  ...
  boot-ec2-config = makeEc2Test {
    ...
    # ### https://awos.org/channels/awos-unstable awos
    userData = ''
      { pkgs, ... }:

      {
        imports = [
          <nixpkgs/awos/modules/virtualisation/amazon-image.nix>
          <nixpkgs/awos/modules/testing/test-instrumentation.nix>
          <nixpkgs/awos/modules/profiles/qemu-guest.nix>
        ];
