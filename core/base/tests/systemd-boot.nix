...
let
  ...
  customDiskImage = nodes: ''
    ...
    subprocess.run([
      ...
      "qcow2",
      "-b",
      "${nodes.machine.system.build.diskImage}/awos.qcow2",
      "-F",
      "qcow2",
      tmp_disk_image.name,
    ])
  ...
  basic = runTest (
    { lib, ... }:
    {
      ...
      testScript = ''
        ...
        machine.succeed("test -e /boot/loader/entries/awos-generation-1.conf")
        machine.succeed("grep 'sort-key awos' /boot/loader/entries/awos-generation-1.conf")
    ...
    { pkgs, lib, ... }:
    {
      ...
      testScript =
        ...
        ''
          ...
          machine.succeed('sbctl sign /boot/EFI/awos/*${nodes.machine.system.boot.loader.kernelFile}.efi')
    ...
    { pkgs, lib, ... }:
    {
      ...
      testScript =
        ...
        ''
          ...
          machine.succeed(
              "test -e /boot/loader/entries/awos-generation-1-specialisation-something.conf"
          )
          machine.succeed(
              "grep -q 'title awos (something)' /boot/loader/entries/awos-generation-1-specialisation-something.conf"
          )
          machine.succeed(
              "grep 'sort-key something' /boot/loader/entries/awos-generation-1-specialisation-something.conf"
          )
        ''
        + pkgs.lib.optionalString pkgs.stdenv.hostPlatform.isAarch64 ''
          machine.succeed(
              r"grep 'devicetree /EFI/awos/[a-z0-9]\{32\}.*dummy' /boot/loader/entries/awos-generation-1-specialisation-something.conf"
          )
        '';
    }
  );
      ...
      testScript = ''
        ...
        def switch():
            ...
            machine.succeed(
              ...
              xargs -0 -I '{}' sed -i 's/#### LoaderInfo: systemd-boot .* ####/#### LoaderInfo: systemd-boot 000.0-1-notawos ####/' '{}'
              """
            )
            return machine.succeed("/run/current-system/bin/switch-to-configuration boot 2>&1")

        output = switch()
        assert "updating systemd-boot from 000.0-1-notawos to " in output, "Couldn't find systemd-boot update message"
  ...
  extraEntries = runTest (
    { lib, ... }:
    {
      ...
      testScript = ''
        ...
        machine.succeed("test -e /boot/efi/awos/.extra-files/loader/entries/banana.conf")
      '';
    }
  );
  ...
  extraFiles = runTest (
    { lib, ... }:
    {
      ...
      testScript = ''
        ...
        machine.succeed("test -e /boot/efi/awos/.extra-files/efi/fruits/tomato.efi")
      '';
    }
  );
      ...
      testScript =
        ...
        ''
          ...
          with subtest("remove files when no longer needed"):
              ...
              machine.succeed("test -d /boot/efi/awos/.extra-files")
              machine.fail("test -e /boot/efi/awos/.extra-files/efi/fruits/tomato.efi")
              machine.fail("test -d /boot/efi/awos/.extra-files/efi/fruits")
          ...
          with subtest("simultaneously removing and adding files works"):
              ...
              machine.succeed("test -e /boot/loader/entries/netbootxyz.conf")
              machine.succeed("test -e /boot/efi/netbootxyz/netboot.xyz.efi")
              machine.succeed("test -e /boot/efi/awos/.extra-files/loader/entries/netbootxyz.conf")
              machine.succeed("test -e /boot/efi/awos/.extra-files/efi/netbootxyz/netboot.xyz.efi")
        '';
    }
  );
    ...
    { lib, ... }:
    {
      ...
      testScript =
        ...
        ''
          ...
          machine.succeed("${baseSystem}/bin/switch-to-configuration boot")
          machine.fail("test -e /boot/loader/entries/awos-generation-1.conf")
          machine.succeed("test -e /boot/loader/entries/awos-generation-2.conf")
        '';
    }
  );
