...

{
  ...
  options.security.sudo = {
    ...
    configFile = lib.mkOption {
      ...
      # Note: if syntax errors are detected in this file, the awos
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        ...
        message = ''
          awos' `sudo` module does not support `sudo-rs`; see `security.sudo-rs` instead.
        '';
      }
    ];
    ...
    security.sudo.configFile = lib.concatStringsSep "\n" (
      lib.filter (s: s != "") [
        ''
          # Don't edit this file. Set the awos options ‘security.sudo.configFile’
