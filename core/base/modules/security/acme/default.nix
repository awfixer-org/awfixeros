      ...
      orderRenewService = {
        ...
        script = wrapInFlock ''
          ...
          if cmp -s domainhash.txt certificates/domainhash.txt && [ -e '${certificateKey}' ] && [ -e 'certificates/${keyName}.crt' ] && [ -n "$(find accounts -name '${data.email}.key')" ]; then
            ...
            if ! lego ${renewOpts} --days ${toString data.validMinDays}; then
              if is_expiration_skippable out/full.pem; then
                echo 1>&2 "awos-acme: Ignoring failed renewal because expiration isn't within the coming ${toString data.validMinDays} days"
              else
          ...
          # Do a full run
            ...
            # Produce a nice error for those doing their first awos-rebuild with these certs
...
in
{
    ...
    security.acme = {
      ...
      useRoot = lib.mkOption {
        ...
        description = ''
          ...
          owned certificates" in the awos manual, and only using this as a fallback
          or for testing.
        '';
      };
  ...
  imports = [
    ...
    (lib.mkRemovedOptionModule [ "security" "acme" "directory" ]
      "ACME Directory is now hardcoded to /var/lib/acme and its permissions are managed by systemd. See https://github.com/awos/nixpkgs/issues/53852 for more info."
    )
