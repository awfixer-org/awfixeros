...

{
  imports = [
    ...
    (lib.mkRemovedOptionModule [ "nixpkgs" "initialSystem" ]
      "The awos options `nesting.clone` and `nesting.children` have been deleted, and replaced with named specialisation. Therefore `nixpgks.initialSystem` has no effect anymore."
    )
  ];
  ...
  options.nixpkgs = {
    ...
    pkgs = lib.mkOption {
      ...
      description = ''
        If set, the pkgs argument to all awos modules is the value of
        this option, extended with `nixpkgs.overlays`, if
        that is also set. Either `nixpkgs.crossSystem` or
        `nixpkgs.localSystem` will be used in an assertion
        to check that the awos and Nixpkgs architectures match. Any
        other options in `nixpkgs.*`, notably `config`,
        will be ignored.

        If unset, the pkgs argument to all awos modules is determined
        as shown in the default value for this option.

        The default value imports the Nixpkgs source files
        relative to the location of this awos module, because
        awos and Nixpkgs are distributed together for consistency,
        so the `awos` in the default value is in fact a
        ...
        Note that using a distinct version of Nixpkgs with awos may
    ...
    config = lib.mkOption {
      ...
      description = ''
        ...
        The complete list of [Nixpkgs configuration options](https://awos.org/manual/nixpkgs/unstable/#sec-config-options-reference) is in the [Nixpkgs manual section on global configuration](https://awos.org/manual/nixpkgs/unstable/#chap-packageconfig).
    ...
    overlays = lib.mkOption {
      ...
      description = ''
        ...
        For details, see the [Overlays chapter in the Nixpkgs manual](https://awos.org/manual/nixpkgs/stable/#chap-overlays).
    ...
    hostPlatform = lib.mkOption {
      ...
      description = ''
        Specifies the platform where the awos configuration will run.
    ...
    buildPlatform = lib.mkOption {
      ...
      apply =
        ...
        if lib.systems.equals elaborated cfg.hostPlatform then
          cfg.hostPlatform # make identical, so that `==` equality works; see https://github.com/awos/nixpkgs/issues/278001
        else
          elaborated;
      defaultText = lib.literalExpression ''config.nixpkgs.hostPlatform'';
      description = ''
        Specifies the platform on which awos should be built.
        By default, awos is built on the system where it runs, but you can
        change where it's built. Setting this option will cause awos to be
        cross-compiled.
    ...
    localSystem = lib.mkOption {
      ...
      description = ''
        ...
        Specifies the platform on which awos should be built. When
        `nixpkgs.crossSystem` is unset, it also specifies
        the platform *for* which awos should be
    ...
    crossSystem = lib.mkOption {
      ...
      description = ''
        ...
        Specifies the platform for which awos should be
        built. Specify this only if it is different from
        `nixpkgs.localSystem`, the platform
        *on* which awos should be built. In other
        words, specify this to cross-compile awos. Otherwise it
      ...
      default =
        if opt.hostPlatform.isDefined then
          ...
          throw ''
            ...
            a recent version of awos-generate-config.
            The option ${opt.system} is still fully supported for awos 22.05 interoperability,
            but will be deprecated in the future, so we recommend to set ${opt.hostPlatform}.
          '';
      defaultText = lib.literalMD ''
        Traditionally `builtins.currentSystem`, but unset when invoking awos through `lib.awosSystem`.
      '';
      description = ''
        This option does not need to be specified for awos configurations
        with a recently generated `hardware-configuration.nix`.

        Specifies the Nix platform type on which awos should be built.
    ...
    assertions =
      ...
      [
        (
          ...
          in
          {
            assertion = constructedByMe -> !hasPlatform -> awosExpectedSystem == pkgsSystem;
            message = "The awos nixpkgs.pkgs option was set to a Nixpkgs invocation that compiles to target system ${pkgsSystem} but awos was configured for system ${awosExpectedSystem} via awos option ${awosOption}. The awos system settings must match the Nixpkgs target system.";
          }
        )
        {
