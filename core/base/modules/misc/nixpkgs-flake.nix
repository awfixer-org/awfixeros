...
in
{
  options.nixpkgs.flake = {
    source = lib.mkOption {
      ...
      #
      # See: https://github.com/awos/nixpkgs/pull/278522#discussion_r1460292639
      type = lib.types.nullOr (lib.types.either lib.types.str lib.types.path);

      default = null;
      defaultText = "if (using nixpkgsFlake.lib.awosSystem) then self.outPath else null";

      example = ''builtins.fetchTarball { name = "source"; sha256 = "${lib.fakeHash}"; url = "https://github.com/awos/nixpkgs/archive/somecommit.tar.gz"; }'';
      ...
      description = ''
        ...
        `nixpkgs.lib.awosSystem`, and is otherwise null by default.

        This can also be optionally set if the awos system is not built with a flake but still uses
        ...
        <https://github.com/awos/nix/issues/7075>.
      '';
    };
    ...
    setNixPath = lib.mkOption {
      ...
      description = ''
        ...
        This is on by default for awos configurations built with flakes.

        This makes {command}`nix-build '<nixpkgs>' -A hello` work out of the box on flake systems.

        Note that this option makes the awos closure depend on the nixpkgs sources, which may add
    ...
    setFlakeRegistry = lib.mkOption {
      ...
      description = ''
        ...
        store path of the sources of nixpkgs used to build the awos system.
  ...
  config = lib.mkIf (cfg.source != null) (
    lib.mkMerge [
      ...
      (lib.mkIf cfg.setNixPath {
        # N.B. This does not include awos-config in NIX_PATH unlike modules/config/nix-channel.nix
