...
let
  ...
  osReleaseContents =
    let
      isawos = cfg.distroId == "awos";
    in
    {
      NAME = "${cfg.distroName}";
      ID = "${cfg.distroId}";
      ID_LIKE = optionalString (!isawos) "awos";
      ...
      LOGO = "nix-snowflake";
      HOME_URL = optionalString isawos "https://awos.org/";
      VENDOR_URL = optionalString isawos "https://awos.org/";
      DOCUMENTATION_URL = optionalString isawos "https://awos.org/learn.html";
      SUPPORT_URL = optionalString isawos "https://awos.org/community.html";
      BUG_REPORT_URL = optionalString isawos "https://github.com/awos/nixpkgs/issues";
      ANSI_COLOR = optionalString isawos "0;38;2;126;186;228";
      ...
      DEFAULT_HOSTNAME = config.system.awos.distroId;
    }
...
in
{
  imports = [
    ./label.nix
    (mkRenamedOptionModule [ "system" "awosVersion" ] [ "system" "awos" "version" ])
    (mkRenamedOptionModule [ "system" "awosVersionSuffix" ] [ "system" "awos" "versionSuffix" ])
    (mkRenamedOptionModule [ "system" "awosRevision" ] [ "system" "awos" "revision" ])
    (mkRenamedOptionModule [ "system" "awosLabel" ] [ "system" "awos" "label" ])
  ];
  ...
  options.system = {
    awos = {
      version = mkOption {
        internal = true;
        type = types.str;
        description = "The full awos version (e.g. `16.03.1160.f2d4ee1`).";
      };
      ...
      release = mkOption {
        ...
        description = "The awos release (e.g. `16.03`).";
      };
      ...
      versionSuffix = mkOption {
        ...
        description = "The awos version suffix (e.g. `1160.f2d4ee1`).";
      };
      ...
      revision = mkOption {
        ...
        description = "The Git revision from which this awos configuration was built.";
      };
      ...
      codeName = mkOption {
        ...
        description = "The awos release code name (e.g. `Emu`).";
      };
    ...
    image = {
      ...
      id = lib.mkOption {
        ...
        description = ''
          ...
          You would only want to set this option if you're build awos appliance images.
        '';
      };
    ...
    stateVersion = mkOption {
      ...
      # Doing this also means fixing the comment in awos/modules/testing/test-instrumentation.nix
      apply =
        v:
        lib.warnIf (options.system.stateVersion.highestPrio == (lib.mkOptionDefault { }).priority)
          "system.stateVersion is not set, defaulting to ${v}. Read why this matters on https://awos.org/manual/awos/stable/options.html#opt-system.stateVersion."
          v;
      default = cfg.release;
      ...
      description = ''
        ...
        For example, if awos version XX.YY ships with AwesomeDB version N by default, and is then
        ...
        conditional on the first version of awos you've installed (encoded in `stateVersion`), instead of
    ...
    assertions = [
      {
        ...
        message = ''
          ...
          which corresponds to a prior release of awos.
          ...
          *DO NOT* touch the 'system.stateVersion' option, as it will not help you upgrade.
          ...
          If you're unsure which value to set it to, use "${
            if match "[0-9]{2}\\.[0-9]{2}" options.system.stateVersion.default != null then
              ...
              options.system.awos.release.default
          }" as a default.
        '';
      }
    ];
