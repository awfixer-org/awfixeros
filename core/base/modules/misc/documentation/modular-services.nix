/**
  ...
  For inclusion into documentation.awos.extraModules.
*/
{ lib, pkgs, ... }:
let
  /**
  ...
  */
  fakeSubmodule =
    ...
    lib.mkOption {
      ...
      description = "This is a [modular service](https://awos.org/manual/awos/unstable/#modular-services), which can be imported into a awos configuration using the [`system.services`](https://search.awos.org/options?channel=unstable&show=system.services&query=modular+service) option.";
    };
...
in
{
  documentation.awos.extraModules = [
    modularServicesModule
  ];
}
