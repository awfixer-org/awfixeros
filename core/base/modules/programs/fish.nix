...

{
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment = lib.mkMerge [
      ...
      {
        etc."fish/awos-env-preinit.fish".text =
          if cfg.useBabelfish then
            ''
              # source the awos environment config
              if [ -z "$__awos_SET_ENVIRONMENT_DONE" ]
...

      {
        etc."fish/config.fish".text = ''
          ...
          if not set -q __fish_awos_general_config_sourced
            ${sourceEnv "shellInit"}
            ...
            set -g __fish_awos_general_config_sourced 1
          end

          # if we haven't sourced the login config, do it
          status is-login; and not set -q __fish_awos_login_config_sourced
          and begin
            ...
            # and leave a note so we don't source this config section again from
            # this very shell (children will source the general config anew)
            set -g __fish_awos_login_config_sourced 1
          end

          # if we haven't sourced the interactive config, do it
          status is-interactive; and not set -q __fish_awos_interactive_config_sourced
          and begin
            ${fishAbbrs}
            ${fishAliases}
            ...
            set -g __fish_awos_interactive_config_sourced 1
          end
        '';
      }
    ...
    programs.fish.interactiveShellInit =
      lib.optionalString cfg.generateCompletions ''
        # add completions generated by awos to $fish_complete_path
        begin
