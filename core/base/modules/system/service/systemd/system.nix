  ...
  makeawosEtcFiles =
    ...
    let
      ...
      subServiceConfigData = concatMapAttrs (
        subServiceName: subService: makeawosEtcFiles (dash prefix subServiceName) subService
      ) service.services;
    in
...
in
{
  ...
  options = {
    system.services = mkOption {
      description = ''
        A collection of awos [modular services](https://awos.org/manual/awos/unstable/#modular-services) that are configured as systemd services.
      '';
  ...
  config = {
    ...
    environment.etc = concatMapAttrs (
      serviceName: topLevelService: makeawosEtcFiles serviceName topLevelService
    ) config.system.services;
  };
}
