...
in
{
  ...
  options.boot.initrd.systemd = {
    ...
    root = lib.mkOption {
      ...
      description = ''
        ...
        {manpage}`kernel-command-line(7)`. awos currently does not
  ...
  config = mkIf (config.boot.initrd.enable && cfg.enable) {
    ...
    # For awos activation to succeed, we need to have root writable in initrd.
    ...
    boot.initrd.systemd = {
      ...
      // optionalAttrs (config.environment.etc ? "modprobe.d/awos.conf") {
        "/etc/modprobe.d/awos.conf".source = config.environment.etc."modprobe.d/awos.conf".source;
      }
      // optionalAttrs (with config.system.build.kernel.config; isSet "MODULES" -> isYes "MODULES") {
        ...
        "/etc/modules-load.d/awos.conf".text = concatStringsSep "\n" config.boot.initrd.kernelModules;

        "/etc/sysctl.d/awos.conf".text = "kernel.modprobe = /sbin/modprobe";
      ...
      storePaths = [
        ...
        "${config.system.awos-init.package}/bin/chroot-realpath"
        # Find the etc paths
        "${config.system.awos-init.package}/bin/find-etc"
      ]
      ++ lib.optionals config.system.awos-init.enable [
        "${config.system.awos-init.package}/bin/initrd-init"
      ]
      ++ jobScripts
      ...
      services.initrd-find-awos-closure = lib.mkIf (!config.system.awos-init.enable) {
        description = "Find awos closure";
          ...
          ''
            ...
            export PATH="/bin:${
              lib.makeBinPath [
                ...
                config.system.awos-init.package
              ]
            }"
            ...
            ln --symbolic "$closure" /awos-closure

            # If we are not booting a awos closure (e.g. init=/bin/sh),
            ...
            if ! [ -x "/sysroot$(readlink "/sysroot$closure/prepare-root" || echo "$closure/prepare-root")" ]; then
              ...
              echo "$closure does not look like a awos installation - not activating"
              exit 0
            fi
      ...
      services.initrd-awos-activation = lib.mkIf (!config.system.awos-init.enable) {
        ...
        script = # bash
          ''
            ...
            closure="$(realpath /awos-closure)"

            # Initialize the system
            export IN_awos_SYSTEMD_STAGE1=true
            exec chroot /sysroot "$closure/prepare-root"
          '';
      };

      services.initrd-switch-root =
        if config.system.awos-init.enable then
          {
            path = [
              cfg.package
              cfg.package.util-linux
          ...
          # is the case when not booting a awos system) or with an empty string, causing
