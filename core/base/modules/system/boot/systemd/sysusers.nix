...
let
  ...
  sysusersConfig = pkgs.writeTextDir "00-awos.conf" ''
    ...
    ${lib.concatLines (
      lib.mapAttrsToList (
        username: opts:
        let
          uid = if opts.uid == null then "/var/lib/awos/uid/${username}" else toString opts.uid;
        in
    ...
    ${lib.concatLines (
      lib.mapAttrsToList (
        groupname: opts:
        ''g ${groupname} ${
          if opts.gid == null then "/var/lib/awos/gid/${groupname}" else toString opts.gid
        }''
      ) userCfg.groups
    )}
  ...
  immutablePasswordFilesLocation = "/var/lib/awos/etc";
...

{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd = {
      ...
      tmpfiles.settings.awos-uid = lib.mapAttrs' (
        username: opts:
        lib.nameValuePair "/var/lib/awos/uid/${username}" {
          f = {
            user = username;
          };
        }
      ...
      tmpfiles.settings.awos-gid = lib.mapAttrs' (
        groupname: opts:
        lib.nameValuePair "/var/lib/awos/gid/${groupname}" {
          f = {
            group = groupname;
          };
        }
      ...
      services.systemd-sysusers = {
        ...
        serviceConfig = {
          ...
          ExecStart = lib.mkIf immutableEtc [
            ""
            "${config.systemd.package}/bin/systemd-sysusers --root ${builtins.dirOf immutablePasswordFilesLocation} /etc/sysusers.d/00-awos.conf"
          ];
