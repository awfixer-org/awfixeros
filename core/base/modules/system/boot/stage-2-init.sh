...
if [ "${IN_awos_SYSTEMD_STAGE1:-}" != true ]; then
...
if [ ! -e /proc/1 ]; then
    specialMount() {
        ...
        # from stage 1's /run
        if [ "${IN_awos_SYSTEMD_STAGE1:-}" = true ] && [ "${mountPoint}" = /run ]; then
            return
        fi
...
if [ "${IN_awos_SYSTEMD_STAGE1:-}" = true ] || [ ! -c /dev/kmsg ] ; then
