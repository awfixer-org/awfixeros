...

{
  ...
  options.systemd = {
    ...
    enableStrictShellChecks = mkEnableOption "" // {
      description = ''
        ...
        When enabled, all systemd scripts generated by awos will be checked
        ...
        packages and that are not defined through the awos config. This option
  ...
  config = {
      ...
      concatLists (
        mapAttrsToList (
          ...
          concatLists [
            ...
            (optional hasDeprecated "Service '${name}.service' uses the attribute 'StartLimitInterval' in the Service section, which is deprecated. See https://github.com/awos/nixpkgs/issues/45786.")
    ...
    environment.etc =
      ...
      in
      {
        ...
        "systemd/system-preset/00-awos.preset".text = ''
          ignore *
        '';
        "systemd/user-preset/00-awos.preset".text = ''
          ignore *
        '';
      };
    ...
    systemd.services.systemd-udev-settle.restartIfChanged = false; # Causes long delays in awos-rebuild
    systemd.targets.local-fs.unitConfig.X-StopOnReconfiguration = true;
    systemd.targets.remote-fs.unitConfig.X-StopOnReconfiguration = true;
    systemd.services.systemd-importd.environment = proxy_env;
    systemd.services.systemd-pstore.wantedBy = [ "sysinit.target" ]; # see #81138

    # awos has kernel modules in a different location, so override that here.
  ...
  imports = [
    ...
    (mkRemovedOptionModule [ "systemd" "enableUnifiedCgroupHierarchy" ] ''
      ...
      awos does not officially support this configuration and might cause your system to be unbootable in future versions. You are on your own.
    '')
