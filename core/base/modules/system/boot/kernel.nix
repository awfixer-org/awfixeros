...
let
  ...
  kernelModulesConf = pkgs.writeText "awos.conf" ''
...

{
  ...
  options = {
    ...
    boot.kernel.features = mkOption {
      ...
      description = ''
        ...
        which would have separate awos options.
    ...
    boot.kernelPackages = mkOption {
      ...
      description = ''
        ...
        awos.  Since things like external kernel module packages are
        ...
        [in the Linux section of the manual](https://awos.org/manual/awos/unstable/index.html#sec-kernel-config).
      '';
    };
      ...
      description = ''
        ...
        ```nix
        {
          ...
          features = {                  # attrset of extra "features" the kernel is considered to have
            foo = true;                 # (may be checked by other awos modules, optional)
          };
    ...
    boot.kernel.randstructSeed = mkOption {
      ...
      description = ''
        ...
        only enabled in awos hardened kernels. Using a custom seed requires
    ...
    boot.initrd.allowMissingModules = mkOption {
      ...
      description = ''
        ...
        provide as many modules as typical awos kernels.
  ...
  config = mkMerge [
    (mkIf config.boot.initrd.enable {
      boot.initrd.availableKernelModules = optionals config.boot.initrd.includeDefaultModules (
        [
          ...
          # shouldn't be included by default since awos-generate-config
    ...
    (mkIf config.boot.kernel.enable {
      ...
      # Create /etc/modules-load.d/awos.conf, which is read by
      # systemd-modules-load.service to load required kernel modules.
      environment.etc = {
        "modules-load.d/awos.conf".source = kernelModulesConf;
      };
