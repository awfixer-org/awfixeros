...
my $extraEntriesBeforeawos = get("extraEntriesBeforeawos") eq "true";
...
# Add default entries.
$conf .= "$extraEntries\n" if $extraEntriesBeforeawos;

addGeneration("@distroName@", "", $defaultConfig, $entryOptions, 1);

$conf .= "$extraEntries\n" unless $extraEntriesBeforeawos;
...
sub getEfiTarget {
    if (($grub ne "") && ($grubEfi ne "")) {
        ...
        #       architectures in awos. That would have to be fixed in the awos modules first.
        return "no"
...
if (($ENV{'awos_INSTALL_GRUB'} // "") eq "1") {
    warn "awos_INSTALL_GRUB env var deprecated, use awos_INSTALL_BOOTLOADER";
    $ENV{'awos_INSTALL_BOOTLOADER'} = "1";
}
my $requireNewInstall = $devicesDiffer || $extraGrubInstallArgsDiffer || $nameDiffer || $versionDiffer || $efiDiffer || $efiMountPointDiffer || (($ENV{'awos_INSTALL_BOOTLOADER'} // "") eq "1");
