...
# Copy a file from the Nix store to $target/awos.
declare -A filesCopied

copyToKernelsDir() {
    local src=$(readlink -f "$1")
    local dst="$target/awos/$(cleanName $src)"
...
# Copy its kernel, initrd and dtbs to $target/awos, and echo out an
# extlinux menu entry
addEntry() {
    ...
    awosLabel="$(cat $path/awos-version)"
    extraParams="$(cat $path/kernel-params)"

    echo
    echo "LABEL awos-$tag"
    if [ "$tag" = "default" ]; then
        echo "  MENU LABEL awos - Default"
    else
        echo "  MENU LABEL awos - Configuration $tag ($timestamp - $awosLabel)"
    fi
    echo "  LINUX ../awos/$(basename $kernel)"
    echo "  INITRD ../awos/$(basename $initrd)"
    ...
    if [ -d "$dtbDir" ]; then
        ...
        if [ -n "$dtbName" ]; then
            echo "  FDT ../awos/$(basename $dtbs)/${dtbName}"
        else
            echo "  FDTDIR ../awos/$(basename $dtbs)"
        fi
    else
        if [ -n "$dtbName" ]; then
...
cat > $tmpFile <<EOF
# Generated file, all changes will be lost on awos-rebuild!

# Change this to e.g. awos-42 to temporarily boot to an older configuration.
DEFAULT awos-default
...
# Remove obsolete files from $target/awos.
for fn in $target/awos/*; do
