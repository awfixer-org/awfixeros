...
let
  ...
  systemdBootBuilder = pkgs.replaceVarsWith {
    ...
    replacements = rec {
      ...
      inherit (config.system.awos) distroName;
      ...
      copyExtraFiles = pkgs.writeShellScript "copy-extra-files" ''
        ...
        ${concatStrings (
          mapAttrsToList (n: v: ''
            ...
            ${pkgs.coreutils}/bin/install -D $empty_file "${bootMountPoint}/${awosDir}/.extra-files/"${escapeShellArg n}
          '') cfg.extraFiles
        )}
        ...
        ${concatStrings (
          mapAttrsToList (n: v: ''
            ...
            ${pkgs.coreutils}/bin/install -D $empty_file "${bootMountPoint}/${awosDir}/.extra-files/loader/entries/"${escapeShellArg n}
          '') cfg.extraEntries
        )}
      '';
    };
...
in
{
  ...
  options.boot.loader.systemd-boot = {
    ...
    sortKey = mkOption {
      ...
      type = types.str;
      description = ''
        The sort key used for the awos bootloader entries.
        This key determines sorting relative to non-awos entries.
        See also <https://uapi-group.org/specifications/specs/boot_loader_specification/#sorting>

        This option can also be used to control the sorting of awos specialisations.
        ...
        generation was removed from the awos configuration.
    ...
    memtest86 = {
      ...
      sortKey = mkOption {
        ...
        description = ''
          ...
          so if you want something to appear after all the awos entries,
    ...
    windows = mkOption {
      ...
      description = ''
        ...
        awos use the same EFI System Partition (ESP). In that case, Windows will automatically be
  ...
  config = mkIf cfg.enable {
    ...
    ++ concatMap (filename: [
      ...
      {
        assertion = !(hasInfix "awos/.extra-files" (toLower filename));
        message = "boot.loader.systemd-boot.extraFiles.${lib.strings.escapeNixIdentifier filename} is invalid: files cannot be placed in the awos/.extra-files directory";
      }
    ...
    boot.bootspec.extensions."org.awos.systemd-boot" = {
