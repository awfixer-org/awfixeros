...
awos_DIR = Path("@awosDir@".strip("/")) # Path relative to the XBOOTLDR or ESP mount point
TIMEOUT = "@timeout@"
...
def bootspec_from_json(bootspec_json: dict[str, Any]) -> BootSpec:
    specialisations = bootspec_json['org.awos.specialisation.v1']
    specialisations = {k: bootspec_from_json(v) for k, v in specialisations.items()}
    systemdBootExtension = bootspec_json.get('org.awos.systemd-boot', {})
    sortKey = systemdBootExtension.get('sortKey', 'awos')
    devicetree = systemdBootExtension.get('devicetree')

    if devicetree:
        devicetree = Path(devicetree)

    main_json = bootspec_json['org.awos.bootspec.v1']
...
def copy_from_file(file: Path, dry_run: bool = False) -> Path:
    ...
    efi_file_path = awos_DIR / (f"{suffix}.efi" if suffix == store_subdir else f"{store_subdir}-{suffix}.efi")
    if not dry_run:
...
def remove_old_entries(gens: list[SystemIdentifier]) -> None:
    rex_profile = re.compile(r"^awos-(.*)-generation-.*\.conf$")
    rex_generation = re.compile(r"^awos.*-generation-([0-9]+)(-specialisation-.*)?\.conf$")
    known_paths = []
    for gen in gens:
    ...
    for path in (BOOT_MOUNT_POINT / "loader/entries").glob("awos*-generation-[1-9]*.conf", case_sensitive=False):
        ...
        if (prof, gen_number, None) not in gens:
            path.unlink()
    for path in (BOOT_MOUNT_POINT / awos_DIR).iterdir():
        if path.name not in known_paths and not path.is_dir():
            path.unlink()


def cleanup_esp() -> None:
    for path in (EFI_SYS_MOUNT_POINT / "loader/entries").glob("awos*"):
        path.unlink()
    awos_dir = EFI_SYS_MOUNT_POINT / awos_DIR
    if awos_dir.is_dir():
        shutil.rmtree(awos_dir)
...
def install_bootloader(args: argparse.Namespace) -> None:
    ...
    if os.getenv("awos_INSTALL_GRUB") == "1":
        warnings.warn("awos_INSTALL_GRUB env var deprecated, use awos_INSTALL_BOOTLOADER", DeprecationWarning)
        os.environ["awos_INSTALL_BOOTLOADER"] = "1"
    ...
    if os.getenv("awos_INSTALL_BOOTLOADER") == "1":
        ...
        if installed_match is None:
            raise Exception("Could not find any previously installed systemd-boot. If you are switching to systemd-boot from a different bootloader, you need to run `awos-rebuild switch --install-bootloader`")
    ...
    (BOOT_MOUNT_POINT / awos_DIR).mkdir(parents=True, exist_ok=True)
    ...
    for gen in gens:
        try:
            ...
            # See https://github.com/awos/nixpkgs/issues/114552
    ...
    extra_files_dir = BOOT_MOUNT_POINT / awos_DIR / ".extra-files"
