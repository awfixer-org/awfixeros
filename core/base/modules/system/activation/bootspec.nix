...
#
# See: https://github.com/awos/rfcs/pull/125
{
  config,
  pkgs,
  lib,
  ...
  schemas = {
    v1 = rec {
      ...
      json = pkgs.writeText filename (
        builtins.toJSON
          # Merge extensions first to not let them shadow awos bootspec data.
          (
            cfg.extensions
            // {
              "org.awos.bootspec.v1" = {
                system = config.boot.kernelPackages.stdenv.hostPlatform.system;
                kernel = "${config.boot.kernelPackages.kernel}/${config.system.boot.loader.kernelFile}";
                kernelParams = config.boot.kernelParams;
                label = "${config.system.awos.distroName} ${config.system.awos.codeName} ${config.system.awos.label} (Linux ${config.boot.kernelPackages.kernel.modDirVersion})";
              }
        ...
        let
          ...
          toplevelInjector =
            lib.escapeShellArgs [
              ...
              ''
                ."org.awos.bootspec.v1".toplevel = $toplevel |
                ."org.awos.bootspec.v1".init = $init
              ''
              "--sort-keys"
              "--arg"
              "toplevel"
          ...
          specialisationInjector =
            ...
            lib.escapeShellArgs [
              "${pkgs.buildPackages.jq}/bin/jq"
              "--sort-keys"
              ''."org.awos.specialisation.v1" = ($ARGS.named | map_values(. | first))''
            ]
