      ...
      // optionalAttrs withDry {
        supportsDryActivation = mkOption {
          ...
          description = ''
            ...
            `awos_ACTION` being set to `dry-activate`.
...

{
  ...
  options = {
    ...
    system.activationScripts = mkOption {
      ...
      description = ''
        A set of shell script fragments that are executed when a awos
        system configuration is activated.  Examples are updating
        /etc, creating accounts, and so on.  Since these are executed
        every time you boot the system or run
        {command}`awos-rebuild`, it's important that they are
        idempotent and fast.
      '';
    ...
    system.userActivationScripts = mkOption {
      ...
      description = ''
        ...
        service when a awos system configuration is activated. Examples are
    ...
    system.build.installBootLoader = mkOption {
      ...
      type = types.unique {
        message = ''
          ...
          been checked until awos 22.05. Earlier versions defaulted to the last
  ...
  config = {
    ...
    systemd.user = lib.mkIf config.system.activatable {
      services.awos-activation = {
        description = "Run user-specific awos activation";
