...
let
  systemBuilder = ''
    ...
    ${
      if config.boot.initrd.enable && config.boot.initrd.systemd.enable then
        ''
          ...
          ${lib.optionalString (!config.system.awos-init.enable) ''
    ...
    echo -n "$awosLabel" > $out/awos-version
  ...
  baseSystem = pkgs.stdenvNoCC.mkDerivation (
    {
      name = "awos-system-${config.system.name}-${config.system.awos.label}";
      ...
      awosLabel = config.system.awos.label;
  ...
  options = {
    ...
    system.build = {
      toplevel = mkOption {
        ...
        readOnly = true;
        description = ''
          This option contains the store path that typically represents a awos system.
    ...
    system.copySystemConfiguration = mkOption {
      ...
      default = false;
      description = ''
        If enabled, copies the awos configuration file
        (usually {file}`/etc/awos/configuration.nix`)
    ...
    system.name = mkOption {
      ...
      description = ''
        ...
        `"awos-system-''${config.system.name}-''${config.system.awos.label}"`
      '';
    };
  ...
  config = {
    ...
    system.extraSystemBuilderCmds =
      optionalString config.system.copySystemConfiguration ''
        ln -s '${import ../../../lib/from-env.nix "awos_CONFIG" <awos-config>}' \
    ...
    system.systemBuilderArgs = {
      ...
      distroId = config.system.awos.distroId;
