...
# in the current one.  For efficiency, don't look under /etc/awos
# (where all the awos sources live).
sub cleanup {
    if ($File::Find::name eq "/etc/awos") {
        $File::Find::prune = 1;
        return;
    }
    if (-l $_) {
...
sub link {
    ...
    # awos-enter sets up /etc/resolv.conf as a bind mount, so skip it.
    if ($fn eq "resolv.conf" and $ENV{'IN_awos_ENTER'}) {
        return;
    }
...
# Create /etc/awos tag if not exists.
