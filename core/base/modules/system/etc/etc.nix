    ...
    system.build.etcActivationCommands =
      ...
      if config.system.etc.overlay.enable then
        ...
        ''
          ...
          # an initrd though, like for instance when using  awos-enter,
          ...
          # changed.
          if [[ ! $IN_awos_SYSTEMD_STAGE1 ]] && [[ "${config.system.build.etc}/etc" != "$(readlink -f /run/current-system/etc)" ]]; then
            echo "remounting /etc..."
            ...
            ${lib.optionalString config.system.etc.overlay.mutable ''
              ...
              # for instance by awos-enter
              mkdir --parents /.rw-etc/upper /.rw-etc/work
              chmod 0755 /.rw-etc /.rw-etc/upper /.rw-etc/work
            ''}

            tmpMetadataMount=$(TMPDIR="/run" mktemp --directory -t awos-etc-metadata.XXXXXXXXXX)
            ...
            # directly without an initrd, like with awos-enter.
            if ! mountpoint -q /etc; then
              ...
              tmpEtcMount=$(TMPDIR="/run" mktemp --directory -t awos-etc.XXXXXXXXXX)
            ...
            findmnt --type erofs --list --kernel --output TARGET | while read -r mountPoint; do
              if [[ ("$mountPoint" =~ ^/run/awos-etc-metadata\..{10}$ || "$mountPoint" =~ ^/run/awos-etc-metadata$ ) &&
