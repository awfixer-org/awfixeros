...

{
  ...
  options.xdg.portal = {
    ...
    xdgOpenUsePortal = mkOption {
      ...
      default = false;
      description = ''
        Sets environment variable `awos_XDG_OPEN_USE_PORTAL` to `1`
        This will make `xdg-open` use the portal to open programs, which resolves bugs involving
        programs opening inside FHS envs or with unexpected env vars set from wrappers.
        See [#160923](https://github.com/awos/nixpkgs/issues/160923) for more info.
      '';
    };
  ...
  config =
    ...
    mkIf cfg.enable {
      ...
      environment = {
        ...
        sessionVariables = {
          awos_XDG_OPEN_USE_PORTAL = mkIf cfg.xdgOpenUsePortal "1";
