...
my $uidMapFile = "/var/lib/awos/uid-map";
my $uidMap = -e $uidMapFile ? decode_json(read_file($uidMapFile)) : {};

my $gidMapFile = "/var/lib/awos/gid-map";
my $gidMap = -e $gidMapFile ? decode_json(read_file($gidMapFile)) : {};

my $is_dry = ($ENV{'awos_ACTION'} // "") eq "dry-activate";
GetOptions("dry-activate" => \$is_dry);
make_path("/var/lib/awos", { mode => 0755 }) unless $is_dry;
...
# current spec.
my $declGroupsFile = "/var/lib/awos/declarative-groups";
my %declGroups;
$declGroups{$_} = 1 foreach split / /, -e $declGroupsFile ? read_file($declGroupsFile, { binmode => ":utf8" }) : "";

# Idem for the users.
my $declUsersFile = "/var/lib/awos/declarative-users";
my %declUsers;
...
my $subUidMapFile = "/var/lib/awos/auto-subuid-map";
