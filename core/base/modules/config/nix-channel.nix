...
in
{
  options = {
      ...
      nixPath = mkOption {
        ...
        default =
          if cfg.channel.enable then
            [
              "nixpkgs=/nix/var/nix/profiles/per-user/root/channels/awos"
    ...
    system = {
      defaultChannel = mkOption {
        internal = true;
        type = types.str;
        default = "https://awos.org/channels/awos-unstable";
        description = "Default awos channel to which the root user is subscribed.";
      };
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules = lib.mkIf cfg.channel.enable [
      ''f /root/.nix-channels - - - - ${config.system.defaultChannel} awos\n''
    ];
