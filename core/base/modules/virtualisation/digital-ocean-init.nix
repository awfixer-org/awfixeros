...
in
{
  ...
  options.virtualisation.digitalOcean.defaultConfigFile = mkOption {
    ...
    description = ''
      ...
      `/etc/awos/configuration.nix` and be used when switching to
      a new configuration.
    '';
  };
  ...
  config = {
    systemd.services.digitalocean-init = mkIf cfg.rebuildFromUserData {
      ...
      unitConfig = {
        ConditionPathExists = "!/etc/awos/do-userdata.nix";
        After = [
      ...
      path = [
        ...
        config.nix.package
        config.system.build.awos-rebuild
      ];
      environment = {
        HOME = "/root";
        NIX_PATH = concatStringsSep ":" [
          "/nix/var/nix/profiles/per-user/root/channels/awos"
          "awos-config=/etc/awos/configuration.nix"
      ...
      script = ''
        ...
        if jq -er '.user_data' /run/do-metadata/v1.json > $userData; then
          ...
          if nix-instantiate --parse $userData > /dev/null; then
            ...
            cp "$userData" /etc/awos/do-userdata.nix
            if [[ ! -e /etc/awos/configuration.nix ]]; then
              install -m0644 ${cfg.defaultConfigFile} /etc/awos/configuration.nix
            fi
          else
