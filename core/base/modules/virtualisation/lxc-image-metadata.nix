        ...
        example = lib.literalExpression ''
          {
            ...
            # create /etc/awos/hostname.nix with a configuration for keeping the hostname applied
            "hostname-nix" = {
              enable = true;
              target = "/etc/awos/hostname.nix";
            ...
            "configuration-nix" = {
              enable = true;
              target = "/etc/awos/configuration.nix";
      ...
      contents = [
        {
          source = toYAML "metadata.yaml" {
            ...
            creation_date = 1;
            properties = {
              description = "${config.system.awos.distroName} ${config.system.awos.codeName} ${config.system.awos.label} ${pkgs.stdenv.hostPlatform.system}";
              os = "${config.system.awos.distroId}";
              release = "${config.system.awos.codeName}";
            };
