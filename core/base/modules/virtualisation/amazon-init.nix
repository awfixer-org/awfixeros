...
let
  ...
  script = ''
    ...
    export NIX_PATH=nixpkgs=/nix/var/nix/profiles/per-user/root/channels/awos:awos-config=/etc/awos/configuration.nix:/nix/var/nix/profiles/per-user/root/channels

    userData=/etc/ec2-metadata/user-data

    # Check if user-data looks like a shell script and execute it with the
    # runtime shell if it does. Otherwise treat it as a awos configuration
    # expression
    ...
    if [ -s "$userData" ]; then
      ...
      if sed '/^\(#\|SSH_HOST_.*\)/d' < "$userData" | grep -q '\S'; then
        ...
        cp "$userData" /etc/awos/configuration.nix
      else
