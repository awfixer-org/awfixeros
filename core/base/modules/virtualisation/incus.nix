...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        ...
        message = "Incus on awos is unsupported using iptables. Set `networking.nftables.enable = true;`";
      }
    ];
    ...
    boot.kernel.sysctl = {
      ...
      "fs.inotify.max_user_instances" = lib.mkOverride 1050 1048576; # override in case conflict awos/modules/services/x11/xserver.nix
      "fs.inotify.max_user_watches" = lib.mkOverride 1050 1048576; # override in case conflict awos/modules/services/x11/xserver.nix
      ...
      # vm.max_map_count is set higher in awos/modules/config/sysctl.nix
    };
    ...
    security.apparmor = {
      ...
      includes."abstractions/base" = ''
        ...
        # https://discuss.linuxcontainers.org/t/creating-new-containers-vms-blocked-by-apparmor-on-awos/21908/6
        mr /nix/store/*/lib/*.so*,
