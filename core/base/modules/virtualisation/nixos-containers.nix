...
let
  ...
  configurationPrefix = optionalString (versionAtLeast config.system.stateVersion "22.05") "awos-";
  ...
  awos-container = pkgs.awos-container.override {
    inherit stateDirectory configurationDirectory;
  };

  # The container's init script, a small wrapper around the regular
  # awos stage-2 init script.
  containerInit = (
    cfg:
    let
      renderExtraVeth = (
  ...
  startScript = cfg: ''
    ...
    mkdir -p "$root/var/lib/private" "$root/root" /run/awos-containers
    chmod 0700 "$root/var/lib/private" "$root/root" /run/awos-containers
    ...
    if [ -n "$FLAKE" ] && [ ! -e "/nix/var/nix/profiles/per-container/$INSTANCE/system" ]; then
      # we create the etc/awos-container config file, then if we utilize the update function, we can then build all the necessary system files for the container
      ${lib.getExe awos-container} update "$INSTANCE"
    fi
  ...
  serviceDirectives = cfg: {
    ExecReload = pkgs.writeScript "reload-container" ''
      #! ${pkgs.runtimeShell} -e
      ${awos-container}/bin/awos-container run "$INSTANCE" -- \
...

{
  options = {
    ...
    boot.isContainer = mkOption {
      ...
      description = ''
        Whether this awos machine is a lightweight container running
        in another awos system.
      '';
    };
    ...
    boot.enableContainers = mkOption {
      ...
      description = ''
        Whether to enable support for awos containers.
      '';
    };
    ...
    containers = mkOption {
            ...
            options = {
              config = mkOption {
                description = ''
                  ...
                  container, as a awos module.
                '';
                type = lib.mkOptionType {
                  name = "Toplevel awos config";
                  merge =
                    loc: defs:
                    (import "${toString config.nixpkgs}/awos/lib/eval-config.nix" {
                      modules =
                        let
                          extraConfig =
                            { options, ... }:
                                ...
                                assertions = [
                                  {
                                    ...
                                    message = ''
                                      ...
                                      supports interface altnames (i.e. at least Linux 5.8 - please see https://github.com/awos/nixpkgs/issues/38509
                                      for details).
                                    '';
                                  }
                                  {
                                    ...
                                    message = ''
                                      Names containing underscores are not allowed in awos-containers. Please rename the container '${name}'
                                    '';
                                  }
                                ];
                              };
              ...
              path = mkOption {
                ...
                description = ''
                  ...
                  the evaluated awos system configuration, typically a
              ...
              specialArgs = mkOption {
                ...
                default = { };
                description = ''
                  A set of special arguments to be passed to awos modules.
                  This will be merged into the `specialArgs` used to evaluate
                  the awos configurations.
                '';
              };
              ...
              restartIfChanged = mkOption {
                ...
                default = true;
                description = ''
                  Whether the container should be restarted during a awos
              ...
              flake = lib.mkOption {
                ...
                default = null;
                example = "github:awos/nixpkgs/master";
                description = ''
                  The Flake URI of the awos configuration to use for the container.
      ...
      '';
      description = ''
        A set of awos system configurations to be run as lightweight
  ...
  config = mkMerge [
    ...
    (mkIf (config.boot.enableContainers) (
      ...
      in
      {
            ...
            ++ (mapAttrsToList (
              ...
              nameValuePair "container@${name}" (
                ...
                recursiveUpdate unit {
                  ...
                  environment.root =
                    if containerConfig.ephemeral then "/run/awos-containers/%i" else "${stateDirectory}/%i";
                }
        ...
        # Generate a configuration file in /etc/awos-containers for each
        ...
        services.udev.extraRules = optionalString config.networking.networkmanager.enable ''
          # Don't manage interfaces created by awos-container.
