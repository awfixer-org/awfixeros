...
# the corresponding awos generation still exists.
mapfile -t preGenerations < <(find "$efiMountPoint"/loader/entries -type f -name 'xen-*.conf' | sort -V | sed 's_/loader/entries/awos_/loader/entries/xen_g;s_^.*/xen_xen_g;s_.conf$__g')
if [ "$1" = "debug" ]; then
...
rm -f "$efiMountPoint"/{loader/entries/xen-*.conf,efi/awos/xen-*.efi}

# Main array for storing which generations exist in $efiMountPoint after
# systemd-boot-builder.py builds the main entries.
mapfile -t gens < <(find "$efiMountPoint"/loader/entries -type f -name 'awos-*.conf' | sort -V)
[ "$1" = "debug" ] && echo -e "\e[1;34mxenBootBuilder:\e[0m found the following awos boot entries:" && for debugGen in "${gens[@]}"; do echo "                - $debugGen"; done
...
for gen in "${gens[@]}"; do
    ...
    if [ -n "$xenSpecVer" ]; then
        ...
        if grep -sq '"devicetree"' "$bootspecFile"; then
            echo -e "\n\e[1;33mwarning:\e[0m $gen has a \e[1;34morg.awos.systemd-boot.devicetree\e[0m Bootspec entry. Xen currently does not support DeviceTree, so this value will be ignored in the Xen boot entries, which may cause them to \e[1;31mfail to boot\e[0m."
        else
        ...
        # the corresponding awos generation, substituting `awos` with `xen`:
        # `xen-$profile-generation-$number-specialisation-$specialisation.{cfg,conf}`
        xenGen=$(echo "$gen" | sed 's_/loader/entries/awos_/loader/entries/xen_g;s_^.*/xen_xen_g;s_.conf$__g')
        bootParams=$(jq -re ".\"org.xenproject.bootspec.$xenSpecVer\".$xenParamVar | join(\" \")" "$bootspecFile")
        kernel=$(jq -re '."org.awos.bootspec.v1".kernel | sub("^/nix/store/"; "") | sub("/bzImage"; "-bzImage.efi")' "$bootspecFile")
        kernelParams=$(jq -re '."org.awos.bootspec.v1".kernelParams | join(" ")' "$bootspecFile")
        initrd=$(jq -re '."org.awos.bootspec.v1".initrd | sub("^/nix/store/"; "") | sub("/initrd"; "-initrd.efi")' "$bootspecFile")
        init=$(jq -re '."org.awos.bootspec.v1".init' "$bootspecFile")
...
EOF
        ...
        objcopy \
            ...
            "$xenEfi" \
            "$efiMountPoint"/EFI/awos/"$xenGen".efi
...
mapfile -t postGenerations < <(find "$efiMountPoint"/loader/entries -type f -name 'xen-*.conf' | sort -V | sed 's_/loader/entries/awos_/loader/entries/xen_g;s_^.*/xen_xen_g;s_.conf$__g')
...
# does not run after an user disables the Xen module.
    ...
    sed --in-place 's/^default awos-/default xen-/g' "$efiMountPoint"/loader/loader.conf
    case "$1" in
