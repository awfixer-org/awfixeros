# This module creates a virtual machine from the awos configuration.
# Building the `config.system.build.vm' attribute gives you a command
# that starts a KVM/QEMU VM running the awos configuration defined in
...
let
  ...
  # System image is akin to a complete awos install with
...
in
{
  ...
  options = {
    ...
    virtualisation.rootDevice = mkOption {
      ...
      example = "/dev/disk/by-label/awos";
      description = ''
    ...
    virtualisation.forwardPorts = mkOption {
      ...
      description = ''
        ...
        ::: {.warning}
        If the awos firewall on the virtual machine is enabled, you also
    ...
    virtualisation.host.pkgs = mkOption {
      ...
      description = ''
        ...
        Changing this to e.g. a Darwin package set allows running awos VMs on Darwin.
      '';
    };
  ...
  config = {
    ...
    # `awos-rebuild build-vm`) of a system configuration, where the regular
    ...
    system.build.vm =
      hostPkgs.runCommand "awos-vm"
        ...
        ''
          ...
          ln -s ${hostPkgs.writeScript "run-awos-vm" startVM} $out/bin/run-${config.system.name}-vm
        '';
