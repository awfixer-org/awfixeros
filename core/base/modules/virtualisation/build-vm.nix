  ...
  vmVariantWithBootLoader = vmVariant.extendModules {
    modules = [
      (
        { config, ... }:
        {
          _file = "awos/default.nix##vmWithBootLoader";
...
in
{
  options = {
    ...
    virtualisation.vmVariant = mkOption {
      description = ''
        Machine configuration to be added for the vm script produced by `awos-rebuild build-vm`.
      '';
    ...
    virtualisation.vmVariantWithBootLoader = mkOption {
      description = ''
        Machine configuration to be added for the vm script produced by `awos-rebuild build-vm-with-bootloader`.
      '';
