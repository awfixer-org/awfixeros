...
}:
{
  options.image = {
    baseName = lib.mkOption {
      type = lib.types.str;
      default = "awos-image-${config.system.awos.label}-${pkgs.stdenv.hostPlatform.system}";
      defaultText = lib.literalExpression "awos-image-\${config.system.awos.label}-\${pkgs.stdenv.hostPlatform.system}";
      description = ''
