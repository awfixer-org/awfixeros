...
in
{
    ...
    networking.networkmanager = {
      ...
      plugins = mkOption {
        ...
        description = ''
          ...
          See <https://search.awos.org/packages?query=networkmanager-> for available plugin packages.
  ...
  imports = [
    ...
    (mkRemovedOptionModule [ "networking" "networkmanager" "firewallBackend" ] ''
      This option was removed as awos is now using iptables-nftables-compat even when using iptables, therefore Networkmanager now uses the nftables backend unconditionally.
    '')
    (mkRenamedOptionModule
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc = {
      ...
      # see https://github.com/awos/nixpkgs/issues/64965
      "ipsec.secrets".text = ''
    ...
    # Turn off awos' network management when networking is managed entirely by NetworkManager
    networking = mkMerge [
