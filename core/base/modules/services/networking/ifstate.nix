  ...
  config = lib.mkMerge [
    (lib.mkIf cfg.enable {
      ...
      systemd.services.ifstate = commonServiceConfig // {
        ...
        # mount is always available on awos, avoid adding additional store paths to the closure
        path = [ "/run/wrappers" ];
    ...
    (lib.mkIf initrdCfg.enable {
      ...
      boot.initrd = {
        ...
        systemd = {
          ...
          # https://github.com/awos/nixpkgs/blob/master/awos/modules/system/boot/networkd.nix#L3444
          ...
          services.ifstate-initrd = commonServiceConfig // {
            ...
            # mount is always available on awos, avoid adding additional store paths to the closure
            # https://github.com/awos/nixpkgs/blob/2b8e2457ebe576ebf41ddfa8452b5b07a8d493ad/awos/modules/system/boot/systemd/initrd.nix#L550-L551
            path = [
