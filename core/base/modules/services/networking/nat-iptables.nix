...
let
  ...
  flushNat = ''
    ${helpers}
    ip46tables -w -t nat -D PREROUTING -j awos-nat-pre 2>/dev/null|| true
    ip46tables -w -t nat -F awos-nat-pre 2>/dev/null || true
    ip46tables -w -t nat -X awos-nat-pre 2>/dev/null || true
    ip46tables -w -t nat -D POSTROUTING -j awos-nat-post 2>/dev/null || true
    ip46tables -w -t nat -F awos-nat-post 2>/dev/null || true
    ip46tables -w -t nat -X awos-nat-post 2>/dev/null || true
    ip46tables -w -t nat -D OUTPUT -j awos-nat-out 2>/dev/null || true
    ip46tables -w -t nat -F awos-nat-out 2>/dev/null || true
    ip46tables -w -t nat -X awos-nat-out 2>/dev/null || true
    ip46tables -w -t filter -D FORWARD -j awos-filter-forward 2>/dev/null || true
    ip46tables -w -t filter -F awos-filter-forward 2>/dev/null || true
    ip46tables -w -t filter -X awos-filter-forward 2>/dev/null || true
  ...
  mkSetupNat =
    ...
    ''
      ...
      ${concatMapStrings (iface: ''
        ${iptables} -w -t nat -A awos-nat-pre \
          -i '${iface}' -j MARK --set-mark 1
        ${iptables} -w -t filter -A awos-filter-forward \
          -i '${iface}' ${
      ...
      ${concatMapStrings (fwd: ''
        ...
        ${concatMapStrings (
          ...
          ''
            ...
            ${iptables} -w -t nat -A awos-nat-out \
  ...
  setupNat = ''
    ...
    ip46tables -w -t nat -N awos-nat-pre
    ip46tables -w -t nat -N awos-nat-post
    ip46tables -w -t nat -N awos-nat-out
    ip46tables -w -t filter -N awos-filter-forward
    ...
    ip46tables -w -t nat -A PREROUTING -j awos-nat-pre
    ip46tables -w -t nat -A POSTROUTING -j awos-nat-post
    ip46tables -w -t nat -A OUTPUT -j awos-nat-out
    ip46tables -w -t filter -A FORWARD -j awos-filter-forward
  '';
