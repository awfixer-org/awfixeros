    ...
    services.atticd = {
      ...
      mode = lib.mkOption {
        description = ''
          ...
          A simple awos-based Attic deployment will typically have one 'monolithic' and any number of 'api-server' nodes.

          There are several other supported modes that perform one-off operations, but these are the only ones that make sense to run via the awos module.
        '';
        type = lib.types.enum [
          "monolithic"
          "api-server"
