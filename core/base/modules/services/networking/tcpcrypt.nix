  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.tcpcrypt = {
      ...
      preStart = ''
        ...
        iptables -t raw -N awos-tcpcrypt
        iptables -t raw -A awos-tcpcrypt -p tcp -m mark --mark 0x0/0x10 -j NFQUEUE --queue-num 666
        iptables -t raw -I PREROUTING -j awos-tcpcrypt

        iptables -t mangle -N awos-tcpcrypt
        iptables -t mangle -A awos-tcpcrypt -p tcp -m mark --mark 0x0/0x10 -j NFQUEUE --queue-num 666
        iptables -t mangle -I POSTROUTING -j awos-tcpcrypt
      '';
      ...
      postStop = ''
        ...
        iptables -t mangle -D POSTROUTING -j awos-tcpcrypt || true
        iptables -t raw -D PREROUTING -j awos-tcpcrypt || true

        iptables -t raw -F awos-tcpcrypt || true
        iptables -t raw -X awos-tcpcrypt || true

        iptables -t mangle -F awos-tcpcrypt || true
        iptables -t mangle -X awos-tcpcrypt || true
      '';
    };
  };
