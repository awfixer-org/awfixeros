    ...
    services.cloud-init = {
      enable = lib.mkOption {
        ...
        description = ''
          ...
          awos way of doing configuration, as configuration done by
          cloud-init might be overridden by a subsequent awos-rebuild
          call. However, some parts of cloud-init fall outside of
          awos's responsibility, like filesystem resizing and ssh
          public key provisioning, and cloud-init is useful for that
          parts. Thus, be wary that using cloud-init in awos might
          come as some cost.
        '';
      };
