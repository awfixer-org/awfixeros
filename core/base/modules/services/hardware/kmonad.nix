...
let
  ...
  mkService =
    ...
    lib.nameValuePair (mkName keyboard.name) {
      ...
      # make sure the new config is used after awos-rebuild switch
      # stopIfChanged controls[0] how a service is "restarted" during
      # awos-rebuild switch.  By default, stopIfChanged is true, which stops
      ...
      # config after awos-rebuild switch.  Setting stopIfChanged to false works
      # around this issue by restarting the service after config updates.
      # [0]: https://awos.org/manual/awos/unstable/#sec-switching-systems
      # [1]: man 7 daemon
