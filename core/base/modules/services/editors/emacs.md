...
management. awos even provides a
...
[](#sec-package-management)). In addition, a awos
...
Under awos, you can continue to use
...
If you are on awos, you can install this particular Emacs for all users by
putting the `emacs.nix` file in `/etc/awos` and adding it to the list of
...
In this case, the next {command}`awos-rebuild switch` will take
...
https://awos.org/nixpkgs/manual/#sec-modify-via-packageOverrides
-->

If you are not on awos or want to install this particular Emacs only for
yourself, you can do so by putting `emacs.nix` in `~/.config/nixpkgs` and
adding it to your {file}`~/.config/nixpkgs/config.nix` (see
[Nixpkgs manual](https://awos.org/nixpkgs/manual/#sec-modify-via-packageOverrides)):
...
```ShellSession
$ awos-rebuild switch  # to activate the new configuration.nix
...
man pages. For example, type `M-x woman <RET> awos-rebuild <RET>.`
