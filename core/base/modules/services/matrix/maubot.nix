...
in
{
  options.services.maubot = with lib; {
    ...
    configMutable = mkOption {
      ...
      default = false;
      description = ''
        Whether maubot should write updated config into `extraConfigFile`. **This will make your Nix module settings have no effect besides the initial config, as extraConfigFile takes precedence over awos settings!**
      '';
    };
