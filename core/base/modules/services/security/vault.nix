...

{
  options = {
    services.vault = {
      ...
      dev = lib.mkOption {
        ...
        default = false;
        description = ''
          In this mode, Vault runs in-memory and starts unsealed. This option is not meant production but for development and testing i.e. for awos tests.
        '';
      };
      ...
      extraSettingsPaths = lib.mkOption {
        ...
        default = [ ];
        description = ''
          Configuration files to load besides the immutable one defined by the awos module.
          ...
          ```
          # https://releases.awos.org/nixops/latest/manual/manual.html#opt-deployment.keys
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.vault = {
      ...
      restartIfChanged = false; # do not restart on "awos-rebuild switch". It would seal the storage and disrupt the clients.
