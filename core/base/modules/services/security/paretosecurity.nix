...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # However, in awos, due to its immutable nature and the way the Nix store works,
    # the [Install] sections are not processed during system activation. Instead, we
    # must explicitly tell awos which units to enable by specifying their target
    ...
    # In awos, systemd services are configured with minimal PATH. However,
