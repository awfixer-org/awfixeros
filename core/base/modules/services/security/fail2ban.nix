...
let
  ...
  jailConf =
    let
      configFile = configFormat.generate "jail.local" (
        { INCLUDES.before = "paths-awos.conf"; } // (lib.mapAttrs mkJailConfig attrsJails)
      );
  ...
  pathsConf = pkgs.writeText "paths-awos.conf" ''
    # awos
...

{
    ...
    services.fail2ban = {
      ...
      jails = lib.mkOption {
        ...
        description = ''
          ...
          awos comes with a default `sshd` jail;
          for it to work well,
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc = {
      ...
      "fail2ban/paths-awos.conf".source = pathsConf;
