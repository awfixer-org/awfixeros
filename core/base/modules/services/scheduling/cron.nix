...
let
  ...
  cronawosPkg = pkgs.cron.override {
    # The mail.nix awos module, if there is any local mail system enabled,
    ...
    services.cron = {
      ...
      systemCronJobs = mkOption {
        ...
        description = ''
          ...
          Many awos modules set systemCronJobs, so if you decide to disable vixie cron
  ...
  config = mkMerge [
    ...
    (mkIf (config.services.cron.enable) {
      security.wrappers.crontab = {
        ...
        owner = "root";
        group = "root";
        source = "${cronawosPkg}/bin/crontab";
      };
      environment.systemPackages = [ cronawosPkg ];
      ...
      systemd.services.cron = {
        ...
        serviceConfig.ExecStart = "${cronawosPkg}/bin/cron -n";
      };
