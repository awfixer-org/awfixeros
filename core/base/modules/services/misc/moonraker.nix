...
let
  ...
  format = pkgs.formats.ini {
    # https://github.com/awos/nixpkgs/pull/121613#issuecomment-885241996
    listToValue =
      l:
  ...
  config = lib.mkIf cfg.enable {
    warnings =
      ...
      ++ (lib.optional (lib.head (cfg.settings.update_manager.enable_system_updates or [ false ])) ''
        Enabling system updates is not supported on awos and will lead to non-removable warnings in some clients.
      '')
    ...
    security.polkit.extraConfig = lib.optionalString cfg.allowSystemControl ''
      // awos/moonraker: Allow Moonraker to perform system-level operations
      //
