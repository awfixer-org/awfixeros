...
and the sync functionality was rewritten entirely. With it, a awos module
named
...
called {command}`awos-taskserver` which manages the custom CA along
...
With {command}`awos-taskserver` the client certificate is created
along with the UUID of the user, so it handles all of the credentials needed
in order to setup the Taskwarrior 2 client to work with a Taskserver.

## The awos-taskserver tool {#module-services-taskserver-awos-taskserver-tool}

Because Taskserver by default only provides scripts to setup users
imperatively, the {command}`awos-taskserver` tool is used for
...
For example if you add a new organisation using {command}`awos-taskserver
...
This is done using {command}`awos-taskserver user export $orgname
...
```ShellSession
$ ssh server awos-taskserver user export my-company alice | sh
```
...
{command}`awos-taskserver` won't issue certificates, but you can
