...
let
  ...
  awos-taskserver =
    ...
    buildPythonApplication {
      format = "setuptools";
      name = "awos-taskserver";

      src = pkgs.runCommand "awos-taskserver-src" { preferLocalBuild = true; } ''
        mkdir -p "$out"
        cat "${
        ...
        setup(name="awos-taskserver",
              py_modules=["main"],
              install_requires=["Click"],
              entry_points="[console_scripts]\\nawos-taskserver=main:cli")
        EOF
      '';
...
in
{
  options = {
    services.taskserver = {
      enable = lib.mkOption {
        ...
        description =
          let
            url = "https://awos.org/manual/awos/stable/index.html#module-services-taskserver";
          in
          ''
            Whether to enable the Taskwarrior 2 server.

            More instructions about awos in conjunction with Taskserver can be
            found [in the awos manual](${url}).
          '';
      };
  ...
  config = lib.mkMerge [
    (lib.mkIf cfg.enable {
      environment.systemPackages = [ awos-taskserver ];
      ...
      systemd.services.taskserver = {
        ...
        preStart =
          let
            ...
            helperTool = "${awos-taskserver}/bin/awos-taskserver";
          in
