...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.variables = {
      DYSNOMIA_STATEDIR = "/var/state/dysnomia-awos";
    ...
    services.dysnomia.package = pkgs.dysnomia.override (
      ...
      // lib.optionalAttrs (cfg.enableLegacyModules) {
        enableLegacy = builtins.trace ''
          ...
          disable legacy mode with the following awos configuration option:

          dysnomia.enableLegacyModules = false;

          In a future version of Dysnomia (and awos) the legacy option will go away!
        '' true;
      }
    );
    ...
    services.dysnomia.properties = {
      ...
      supportedTypes = [
        ...
        # These are not base modules, but they are still enabled because they work with technology that are always enabled in awos
        "systemd-unit"
        "sysvinit-script"
        "awos-configuration"
      ]
