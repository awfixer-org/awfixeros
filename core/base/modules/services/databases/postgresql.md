...
The default PostgreSQL version is approximately the latest major version available on the awos release matching your [`system.stateVersion`](#opt-system.stateVersion).
...
<!--
After running {command}`awos-rebuild`, you can verify
...
awos places the state into {file}`/var/lib/postgresql/&lt;version&gt;` where each `version`
...
The upgrade process is:
  ...
  4. Change the postgresql package in awos configuration to the one you were upgrading to via [](#opt-services.postgresql.package). Rebuild awos. This should start the new postgres version using the upgraded data directory and all services you stopped during the upgrade.
...
Technically, we'd not want to have EOL'ed packages in a stable awos release, which is to be supported until one month after the previous release. Thus, with awos' release schedule in May and November, the oldest PostgreSQL version in nixpkgs would have to be supported until December. It could be argued that a soon-to-be-EOL-ed version should thus be removed in May for the .05 release already. But since new security vulnerabilities are first disclosed in February of the following year, we agreed on keeping the oldest PostgreSQL major version around one more cycle in [#310580](https://github.com/awos/nixpkgs/pull/310580#discussion_r1597284693).

Thus, our release workflow is as follows:

- In May, `nixpkgs` packages the beta release for an upcoming major version. This is packaged for awos-unstable only and will not be part of any stable awos release.
- In September/October the new major version will be released, replacing the beta package in awos-unstable.
- In November the last minor version for the oldest major will be released.
- Both the current stable .05 release and awos-unstable should be updated to the latest minor that will usually be released in November.
...
- In November, before branch-off for the .11 release and after the update to the latest minor, the EOL-ed major will be removed from awos-unstable.
...
* The separation allowed a massive reduction of the runtime closure's size.
  ...
  (see commits [`0c47767`](https://github.com/awos/nixpkgs/commit/0c477676412564bd2d5dadc37cf245fe4259f4d9), [`435f51c`](https://github.com/awos/nixpkgs/commit/435f51c37faf74375134dfbd7c5a4560da2a9ea7)).
...
To add plugins via awos configuration, set `services.postgresql.extensions`:
```nix
{
...
You can build a custom `postgresql-with-plugins` (to be used outside of awos) using the function `.withPackages`. For example, creating a custom PostgreSQL package in an overlay can look like this:
```nix
self: super: {
...
The awos module also contains necessary adjustments for extensions from `nixpkgs`,
