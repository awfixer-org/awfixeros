...
let
  ...
  customLovelaceModulesResources = {
    lovelace.resources = map (card: {
      url = "/local/awos-lovelace-modules/${card.entrypoint or (card.pname + ".js")}?${card.version}";
      type = "module";
...
in
{
  ...
  options.services.home-assistant = {
    # Running home-assistant on awos is considered an installation method that is unsupported by the upstream project.
      ...
      example = literalExpression ''
        {
          ...
          http = {};
          feedreader.urls = [ "https://awos.org/blogs.xml" ];
        }
      '';
      description = ''
          ...
          copyCustomLovelaceModules =
            if cfg.customLovelaceModules != [ ] then
              ''
                ...
                ln -fns ${customLovelaceModulesDir} "${cfg.configDir}/www/awos-lovelace-modules"
              ''
            else
              ''
                rm -f "${cfg.configDir}/www/awos-lovelace-modules"
              '';
        ...
        let
          ...
          capabilities = unique (
            [
              ...
              # https://github.com/awos/nixpkgs/issues/120617#issuecomment-830685115
              ""
            ]
