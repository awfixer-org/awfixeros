  ...
  options.services.restic.backups = lib.mkOption {
        ...
        { name, ... }:
        {
          options = {
            passwordFile = lib.mkOption {
              ...
              '';
              example = "/etc/awos/restic-password";
            };
            ...
            extraBackupArgs = lib.mkOption {
              ...
              example = [
                "--cleanup-cache"
                "--exclude-file=/etc/awos/restic-ignore"
              ];
            };
    ...
    example = {
      localbackup = {
        ...
        passwordFile = "/etc/awos/secrets/restic-password";
        initialize = true;
      };
      remotebackup = {
        paths = [ "/home" ];
        ...
        passwordFile = "/etc/awos/secrets/restic-password";
        extraOptions = [
          "sftp.command='ssh backup@host -i /etc/awos/secrets/backup-private-key -s sftp'"
        ];
        timerConfig = {
          OnCalendar = "00:05";
          RandomizedDelaySec = "5h";
      ...
      commandbackup = {
        ...
        passwordFile = "/etc/awos/secrets/restic-password";
        environmentFile = "/etc/awos/secrets/restic-environment";
        pruneOpts = [
          "--keep-daily 14"
          "--keep-weekly 4"
          "--keep-monthly 2"
