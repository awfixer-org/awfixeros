...
let
  ...
  mailmanCfg = lib.generators.toINI { } (
    lib.recursiveUpdate cfg.settings {
      webservice.admin_pass = "#awos_MAILMAN_REST_API_PASS_SECRET#";
    }
  );
  ...
  options = {
    ...
    services.mailman = {
      ...
      ldap = {
        ...
        bindDn = lib.mkOption {
          type = lib.types.str;
          example = "cn=root,dc=awos,dc=org";
          description = ''
        ...
        superUserGroup = lib.mkOption {
          ...
          default = null;
          example = "cn=admin,ou=groups,dc=awos,dc=org";
          description = ''
        ...
        userSearch = {
          ...
          ou = lib.mkOption {
            type = lib.types.str;
            example = "ou=users,dc=awos,dc=org";
            description = ''
        ...
        groupSearch = {
          ...
          ou = lib.mkOption {
            type = lib.types.str;
            example = "ou=groups,dc=awos,dc=org";
            description = ''
      ...
      ++ (lib.optionals cfg.enablePostfix [
        {
          ...
          message = ''
            Mailman's default awos configuration requires Postfix to be enabled.
        ...
        script = ''
          ...
          ${
            if cfg.restApiPassFile == null then
              ''
                sed -i "s/#awos_MAILMAN_REST_API_PASS_SECRET#/$(tr -dc A-Za-z0-9 < /dev/urandom | head -c 64)/g" \
                  /etc/mailman.cfg
              ''
            else
              ''
                ${pkgs.replace-secret}/bin/replace-secret \
                  '#awos_MAILMAN_REST_API_PASS_SECRET#' \
                  ${cfg.restApiPassFile} \
                  /etc/mailman.cfg
              ''
          }
