...
let
  ...
  awos-background-light = pkgs.awos-artwork.wallpapers.nineish;
  awos-background-dark = pkgs.awos-artwork.wallpapers.nineish-dark-gray;

  awos-gsettings-overrides = pkgs.budgie-gsettings-overrides.override {
    inherit (cfg) extraGSettingsOverrides extraGSettingsOverridePackages;
    inherit awos-background-dark awos-background-light;
  };
    ...
    text = ''
      ...
      <wallpapers>
        ...
        <wallpaper deleted="false">
          ...
          <filename>${awos-background-dark.gnomeFilePath}</filename>
          <options>zoom</options>
          <shade_type>solid</shade_type>
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    # GSettings overrides.
    environment.sessionVariables.NIX_GSETTINGS_OVERRIDES_DIR = "${awos-gsettings-overrides}/share/gsettings-schemas/awos-gsettings-overrides/glib-2.0/schemas";
