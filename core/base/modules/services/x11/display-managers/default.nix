  ...
  fakeSession = action: ''
    session_is_systemd_aware=$(
      ...
      for i in $XDG_CURRENT_DESKTOP; do
        case $i in
          KDE|GNOME|Pantheon|X-awos-SYSTEMD-AWARE) echo "1"; exit; ;;
          *) ;;
        esac
      done
    )
    ...
    if [ -z "$session_is_systemd_aware" ]; then
      /run/current-system/systemd/bin/systemctl --user ${action} awos-fake-graphical-session.target
    fi
  '';
...

{
  ...
  config = {
    ...
    systemd.user.targets.awos-fake-graphical-session = {
      unitConfig = {
