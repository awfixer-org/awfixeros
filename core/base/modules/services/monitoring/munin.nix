...
let
  ...
  pluginConfDir = pkgs.stdenv.mkDerivation {
    ...
    buildCommand = ''
      mkdir $out
      ln -s ${pluginConf} $out/awos-config
    '';
  };
  ...
  # them afterwards in an attempt to get them to run on awos.
  ...
  # of munin-contrib in your awos configuration.
    ...
    services.munin-node = {
      ...
      disabledPlugins = lib.mkOption {
        ...
        description = ''
          ...
          information, and the awos build of Munin does not write this file.
        '';
        example = [
          "diskstats"
          "zfs_usage_*"
