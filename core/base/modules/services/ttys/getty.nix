...

{
    ...
    services.getty = {
      ...
      greetingLine = mkOption {
        ...
        description = ''
          ...
          The default shows current awos version label, machine type and tty.
        '';
      };
  ...
  config = mkIf config.console.enable {
    ...
    # awos.label would not rebuild manual pages
    services.getty.greetingLine = mkDefault ''<<< Welcome to ${config.system.awos.distroName} ${config.system.awos.label} (\m) - \l >>>'';
    services.getty.helpLine = mkIf (
      config.documentation.awos.enable && config.documentation.doc.enable
    ) "\nRun 'awos-help' for the awos manual.";
