...
let
  ...
  configFile = format.generate "ASF.json" (
    ...
    // {
      ...
      # and awos takes care of restarting the service
  ...
  options.services.archisteamfarm = {
    ...
    settings = lib.mkOption {
      ...
      description = ''
        ...
        Do note that `AutoRestart`  and `UpdateChannel` is always to `false` respectively `0` because awos takes care of updating everything.
