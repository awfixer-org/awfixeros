...
in
{
  ...
  imports = [
    (lib.mkRemovedOptionModule [ "services" "rabbitmq" "cookie" ] ''
      ...
      Please remove it from your awos configuration and deploy a cookie securely instead.
      The renamed `unsafeCookie` must ONLY be used in isolated non-production environments such as awos VM tests.
    '')
  ];
    ...
    services.rabbitmq = {
      ...
      configItems = lib.mkOption {
        ...
        description = ''
          ...
          although outside awos that filename may have Erlang syntax, particularly
