...
let
  ...
  awos-background-light = pkgs.awos-artwork.wallpapers.simple-blue;
  awos-background-dark = pkgs.awos-artwork.wallpapers.simple-dark-gray;

  # TODO: Having https://github.com/awos/nixpkgs/issues/54150 would supersede this
  awos-gsettings-desktop-schemas = pkgs.gnome.awos-gsettings-overrides.override {
    inherit (cfg) extraGSettingsOverrides extraGSettingsOverridePackages favoriteAppsOverride;
    inherit flashbackEnabled awos-background-dark awos-background-light;
  };

  awos-background-info = pkgs.writeTextFile {
    name = "awos-background-info";
    text = ''
      <?xml version="1.0"?>
      ...
      <wallpapers>
        <wallpaper deleted="false">
          <name>Blobs</name>
          <filename>${awos-background-light.gnomeFilePath}</filename>
          <filename-dark>${awos-background-dark.gnomeFilePath}</filename-dark>
    ...
    '';
    destination = "/share/gnome-background-properties/awos.xml";
  };
...

{
  ...
  config = lib.mkMerge [
    (lib.mkIf (cfg.enable || flashbackEnabled) {
      # Seed our configuration into awos-generate-config
      ...
      environment.sessionVariables.NIX_GSETTINGS_OVERRIDES_DIR = "${awos-gsettings-desktop-schemas}/share/gsettings-schemas/awos-gsettings-overrides/glib-2.0/schemas";
    })
    ...
    (lib.mkIf serviceCfg.core-os-services.enable {
      ...
      environment.pathsToLink = [
        "/share" # TODO: https://github.com/awos/nixpkgs/issues/47173
      ];
    })
    ...
    (lib.mkIf serviceCfg.core-developer-tools.enable {
      environment.systemPackages = utils.removePackagesByName [
        ...
        # https://github.com/awos/nixpkgs/issues/60908
        # pkgs.gnome-boxes
        pkgs.sysprof
      ] config.environment.gnome.excludePackages;
