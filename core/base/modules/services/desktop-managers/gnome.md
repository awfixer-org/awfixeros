...
The default applications used in awos are very minimal, inspired by the defaults used in [gnome-build-meta](https://gitlab.gnome.org/GNOME/gnome-build-meta/blob/48.0/elements/core/meta-gnome-core-apps.bst).
...
It is also possible to disable many of the [core services](https://github.com/awos/nixpkgs/blob/b8ec4fd2a4edc4e30d02ba7b1a2cc1358f3db1d5/awos/modules/services/x11/desktop-managers/gnome.nix#L329-L348). For example, if you do not need indexing files, you can disable TinySPARQL with:
...
Icon themes and GTK themes don’t require any special option to install in awos.
...
::: {.warning}
Overrides really only change the default values for GSettings keys so if you or an application changes the setting value, the value set by the override will be ignored. Until [awos’s dconf module implements changing values](https://github.com/awos/nixpkgs/issues/54150), you will either need to keep that in mind and clear the setting from the backend using `dconf reset` command when that happens, or use the [module from home-manager](https://nix-community.github.io/home-manager/options.html#opt-dconf.settings).
:::
...
```nix
{
  services.desktopManager.gnome = {
    extraGSettingsOverrides = ''
      ...
      picture-uri='file://${pkgs.awos-artwork.wallpapers.mosaic-blue.gnomeFilePath}'
