...
- allow awos to provision a default `opencloud.yaml` for you, containing default credentials
  for communication between the microservices
- provide additional secrets via `environmentFile`, provisioned out of band

Please note that current awos module for OpenCloud is configured to run in
