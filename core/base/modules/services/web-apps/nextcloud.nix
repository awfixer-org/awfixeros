      ...
      ${lib.optionalString requiresRuntimeSystemdCredentials ''
        function nix_read_secret($credential_name) {
          ...
          if (!$credentials_directory) {
            error_log(sprintf(
              "Cannot read credential '%s' passed by awos, \$CREDENTIALS_DIRECTORY is not set!",
              $credential_name
            ));
            exit(1);
          }
          ...
          if (!is_readable($credential_path)) {
            error_log(sprintf(
              "Cannot read credential '%s' passed by awos, it does not exist or is not readable!",
              $credential_path,
            ));
            exit(1);
          }
    ...
    lib.mkMerge [
      {
        warnings =
          let
            latest = 32;
            upgradeWarning = major: awos: ''
              A legacy Nextcloud install (from before awos ${awos}) may be installed.
          ...
          ++ (lib.optional (cfg.config.dbtableprefix != null) ''
            ...
            set correctly in `config.php` and remove the option from your awos config.
          '')
            ...
            in
            {
              ...
              script = ''
                ...
                if [[ ! -O "${datadir}/config" ]]; then
                  ...
                  echo "(https://awos.org/manual/awos/stable/#module-services-nextcloud-pitfalls-during-upgrade)"
                  exit 1
                fi
          ...
          settings = lib.mkMerge [
            {
              ...
              # awos already provides its own integrity check and the nix store is read-only, therefore Nextcloud does not need to do its own integrity checks.
