...
if (-e "/etc/awos-generate-config.conf") {
    my $cfg = new Config::IniFiles -file => "/etc/awos-generate-config.conf";
...
for (my $n = 0; $n < scalar @ARGV; $n++) {
    my $arg = $ARGV[$n];
    if ($arg eq "--help") {
        exec "man awos-generate-config" or die;
    }
    elsif ($arg eq "--dir") {
        $n++;
        $outDir = $ARGV[$n];
...
# Pull in awos configuration for containers.
...
my $hwConfig = <<EOF;
# Do not modify this file!  It was generated by ‘awos-generate-config’
# and may be overwritten by future invocations.  Please make changes
# to /etc/awos/configuration.nix instead.
...
EOF
            ...
            $bootLoaderConfig = <<EOF;
  # Use the extlinux boot loader. (awos wants to enable GRUB by default)
...
@configuration@
EOF
        print STDERR "For more hardware-specific settings, see https://github.com/awos/awos-hardware.\n"
    } else {
        print STDERR "warning: not overwriting existing $fn\n";
    }
}
