# This module creates a bootable ISO image containing the given awos
...
let
    ...
    let
      menuConfig = {
        name = lib.concatStrings [
          ...
          cfg.system.awos.distroName
          " "
          cfg.system.awos.label
  ...
  menuBuilderIsolinux =
    ...
    ''
      ${lib.optionalString cfg.isoImage.showConfiguration ''
        LABEL ${label}
        MENU LABEL ${cfg.isoImage.prependToMenuLabel}${cfg.system.awos.distroName} ${cfg.system.awos.label}${cfg.isoImage.appendToMenuLabel}${
          lib.optionalString (cfg.isoImage.configurationName != null) (" " + cfg.isoImage.configurationName)
        }
  ...
  grubMenuCfg = ''
    ...
    search --set=root --file /EFI/awos-installer-image
  ...
  efiDir =
    pkgs.runCommand "efi-directory"
      ...
      ''
        ...
        touch $out/EFI/awos-installer-image
        ...
        # line. We need this to allow the awos iso to be booted from grub directly.
        if [ \''${iso_path} ] ; then
...

{
  ...
  options = {
    ...
    isoImage.volumeID = lib.mkOption {
      # awos-$EDITION-$RELEASE-$ARCH
      default = "awos${
        lib.optionalString (config.isoImage.edition != "") "-${config.isoImage.edition}"
      }-${config.system.awos.release}-${pkgs.stdenv.hostPlatform.uname.processor}";
      type = lib.types.str;
      description = ''
    ...
    isoImage.efiSplashImage = lib.mkOption {
      default = pkgs.fetchurl {
        url = "https://raw.githubusercontent.com/awos/awos-artwork/a9e05d7deb38a8e005a2b52575a3f59a63a4dba0/bootloader/efi-background.png";
    ...
    isoImage.splashImage = lib.mkOption {
      default = pkgs.fetchurl {
        url = "https://raw.githubusercontent.com/awos/awos-artwork/a9e05d7deb38a8e005a2b52575a3f59a63a4dba0/bootloader/isolinux/bios-boot.png";
    ...
    isoImage.syslinuxTheme = lib.mkOption {
      default = ''
        MENU TITLE ${config.system.awos.distroName}
    ...
    isoImage.prependToMenuLabel = lib.mkOption {
      ...
      example = "Install ";
      description = ''
        The string to prepend before the menu label for the awos system.
        This will be directly prepended (without whitespace) to the awos version
        string, like for example if it is set to `XXX`:

        `XXXawos 99.99-pre666`
      '';
    };
    ...
    isoImage.appendToMenuLabel = lib.mkOption {
      ...
      example = " Live System";
      description = ''
        The string to append after the menu label for the awos system.
        This will be directly appended (without whitespace) to the awos version
        string, like for example if it is set to `XXX`:

        `awos 99.99-pre666XXX`
      '';
    };
  ...
  config = {
    ...
    isoImage.contents =
      ...
      [
        {
          source = pkgs.writeText "version" config.system.awos.label;
          target = "/version.txt";
        }
      ]
    ...
    image.baseName = "awos${
      lib.optionalString (config.isoImage.edition != "") "-${config.isoImage.edition}"
    }-${config.system.awos.label}-${pkgs.stdenv.hostPlatform.system}";
    ...
    boot.postBootCommands = ''
      ...
      # awos-rebuild also requires a "system" profile and an
      # /etc/awos tag.
      touch /etc/awos
