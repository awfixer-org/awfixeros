...
in
{
  ...
  options = {
    ...
    system.autoUpgrade = {
      ...
      enable = lib.mkOption {
        ...
        default = false;
        description = ''
          Whether to periodically upgrade awos to the latest
          version. If enabled, a systemd timer will run
          `awos-rebuild switch --upgrade` once a
          day.
        '';
      };
      ...
      operation = lib.mkOption {
        ...
        description = ''
          Whether to run
          `awos-rebuild switch --upgrade` or run
          `awos-rebuild boot --upgrade`
        '';
      };
      ...
      flake = lib.mkOption {
        ...
        description = ''
          The Flake URI of the awos configuration to build.
      ...
      channel = lib.mkOption {
        type = lib.types.nullOr lib.types.str;
        default = null;
        example = "https://awos.org/channels/awos-14.12-small";
        description = ''
          The URI of the awos channel to use for automatic
      ...
      flags = lib.mkOption {
        ...
        example = [
          "-I"
          "stuff=/home/alice/awos-stuff"
          "--option"
          "extra-binary-caches"
          "http://my-cache.example.org/"
        ];
        description = ''
          Any additional flags passed to {command}`awos-rebuild`.
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.awos-upgrade = {
      description = "awos Upgrade";
      ...
      script =
        let
          awos-rebuild = "${config.system.build.awos-rebuild}/bin/awos-rebuild";
        ...
        if cfg.allowReboot then
          ''
            ${awos-rebuild} boot ${toString (cfg.flags ++ upgradeFlag)}
            ...
            if [ "''${booted}" = "''${built}" ]; then
              ${awos-rebuild} ${cfg.operation} ${toString cfg.flags}
            ${lib.optionalString (cfg.rebootWindow != null) ''
          ...
          ''
            ${awos-rebuild} ${cfg.operation} ${toString (cfg.flags ++ upgradeFlag)}
          '';
