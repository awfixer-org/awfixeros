    ...
    boot.zfs = {
      ...
      extraPools = lib.mkOption {
        ...
        description = ''
          ...
          awos's {option}`fileSystems` option, which makes awos automatically
          import the associated pool.

          However, in some cases (e.g. if you have many filesystems) it may be preferable
          to exclusively use ZFS commands to manage filesystems. If so, since awos/systemd
          will not be managing those filesystems, you will need to specify the ZFS pool here
          so that awos automatically imports it on every boot.
        '';
      };
      ...
      forceImportRoot = lib.mkOption {
        ...
        description = ''
          ...
          If you set this option to `false` and awos subsequently fails to
  ...
  config = lib.mkMerge [
    (lib.mkIf cfgZfs.enabled {
      ...
      boot = {
        ...
        # https://github.com/awos/nixpkgs/issues/106093
      ...
      # TODO FIXME See https://github.com/awos/nixpkgs/pull/99386#issuecomment-798813567. To not break people's bootloader and as probably not everybody would read release notes that thoroughly add inSystem.
        ...
        let
          ...
          createSyncService =
            ...
            lib.nameValuePair "zfs-sync-${pool}" {
              ...
              script = ''
                ${cfgZfs.package}/sbin/zfs set awos:shutdown-time="$(date)" "${pool}"
              '';
            };
