# From an end-user configuration file (`configuration.nix'), build a awos
...
# around lib.evalModules, so that modular systems that have awos configs
...
let
  ...
  withWarnings =
    ...
    lib.warnIf (evalConfigArgs ? extraArgs)
      ...
      ''
        ...
        pkgs, which you know is configured correctly for this awos configuration,
        please import the `awosModules.readOnlyPkgs` module from the nixpkgs flake or
  ...
  awosWithUserModules = noUserModules.extendModules { modules = allUserModules; };
...
in
withWarnings (withExtraAttrs awosWithUserModules)
