# Note: This is a private API, internal to awos. Its interface is subject
...
#
#  * partition #3: awos, a partition dedicated to a zpool.
#
...
# `services.zfs.expandOnBoot`.
{
  ...
  # /etc/awos/configuration.nix. This configuration will be embedded
...
let
  ...
  rootFilename = "awos.root${filenameSuffix}";
  ...
  channelSources =
    ...
    in
    pkgs.runCommand "awos-${config.system.awos.version}" { } ''
      mkdir -p $out
      ...
      (
        pkgs.runCommand name
          ...
          ''
            ...
            cat ${mergedConfig} > /mnt/etc/awos/configuration.nix
