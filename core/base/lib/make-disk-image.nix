/*
  ...
  `make-disk-image` has a bit of magic to minimize the amount of work to do in a virtual machine. It also might arguably have too much, or at least too specific magic, so please consider to work towards the effort of unifying our image builders, as outlined in https://github.com/awos/nixpkgs/issues/324817 before adding more.
  ...
  - run `awos-install` in a temporary folder
  - transfer from the temporary store the additional paths registered to the installed awos
...
*/
{
  ...
  # The awos configuration to be installed onto the disk image.
  config,
  ...
  # Filesystem label
  label ? if onlyNixStore then "nix-store" else "awos",

  # The initial awos configuration file to be copied to
...
let
  ...
  channelSources = pkgs.runCommand "awos-${config.system.awos.version}" { } ''
    mkdir -p $out
  ...
  binPath = lib.makeBinPath (
    ...
    [
      ...
      e2fsprogs
      lkl
      config.system.build.awos-install
      awos-enter
      nix
      systemdMinimal
    ]
  ...
  prepareImage = ''
    ...
    # Provide a Nix database so that awos-install can copy closures.
    export NIX_STATE_DIR=$TMPDIR/state
    nix-store --load-db < ${closureInfo}/registration

    chmod 755 "$TMPDIR"
    echo "running awos-install..."
    awos-install --root $root --no-bootloader --no-root-passwd \
  ...
  buildImage = pkgs.vmTools.runInLinuxVM (
    pkgs.runCommand name
      ...
      ''
        ...
        mkdir -p /mnt/etc/awos
        ${lib.optionalString (configFile != null) ''
          cp ${configFile} /mnt/etc/awos/configuration.nix
        ''}
        ...
        ${lib.optionalString installBootLoader ''
          ...
          # clobbers $HOME/.nix-defexpr/channels/awos This would cause a  folder
          ...
          export HOME=$TMPDIR
          awos_INSTALL_BOOTLOADER=1 awos-enter --root $mountPoint -- /nix/var/nix/profiles/system/bin/switch-to-configuration boot
        ''}
        ...
        for ((i = 0; i < ''${#targets_[@]}; i++)); do
          ...
          if [ -n "$user$group" ]; then
            # We have to awos-enter since we need to use the user and group of the VM
            awos-enter --root $mountPoint -- chown -R "$user:$group" "$target"
          fi
        done
