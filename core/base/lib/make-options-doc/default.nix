...
/**
  ...
  ```json
  {
    ...
    "fileSystems.<name>.options": {
      "declarations": ["awos/modules/tasks/filesystems.nix"],
      "default": {
      ...
      "relatedPackages": "- [`pkgs.tmux`](\n    https://search.awos.org/packages?show=tmux&sort=relevance&query=tmux\n  )\n",
    },
    ...
  }
  ```
  ...
  > Note: awos itself uses this output to to build the configuration.nix man page"
  ...
  ```nix
  let
    # Evaluate a awos configuration
    eval = import (pkgs.path + "/awos/lib/eval-config.nix") {
      modules = [
        ./module.nix
      ];
    };
  ...
  ## Example: non-awos modules

  `awosOptionsDoc` can also be used to build documentation for non-awos modules.
...
*/
{
  ...
  # used to split the options doc build into a static part (awos/modules) and a dynamic part
  # (non-awos modules imported via configuration.nix, other module sources).
      ...
      describe =
        ...
        ''
          - [${lib.optionalString (title != null) "${title} aka "}`pkgs.${name}`](
              https://search.awos.org/packages?show=${name}&sort=relevance&query=${name}
...
rec {
  ...
  optionsAsciiDoc =
    pkgs.runCommand "options.adoc"
      {
        nativeBuildInputs = [ pkgs.awos-render-docs ];
      }
      ''
        awos-render-docs -j $NIX_BUILD_CORES options asciidoc \
          --manpage-urls ${pkgs.path + "/doc/manpage-urls.json"} \
          --revision ${lib.escapeShellArg revision} \
          ${optionsJSON}/share/doc/awos/options.json \
          $out
      '';
  ...
  optionsCommonMark =
    pkgs.runCommand "options.md"
      ...
      ''
        awos-render-docs -j $NIX_BUILD_CORES options commonmark \
          --manpage-urls ${pkgs.path + "/doc/manpage-urls.json"} \
          --revision ${lib.escapeShellArg revision} \
          ''${extraArgs[@]} \
          ${optionsJSON}/share/doc/awos/options.json \
  ...
  optionsJSON =
    pkgs.runCommand "options.json"
      {
        meta.description = "List of awos options in JSON format";
        nativeBuildInputs = [
          pkgs.brotli
          pkgs.python3
        ];
      ...
      ''
        ...
        if grep /nixpkgs/awos/modules $dst/options.json; then
          echo "The manual appears to depend on the location of Nixpkgs, which is bad"
          echo "since this prevents sharing via the awos channel.  This is typically"
