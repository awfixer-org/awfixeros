  ...
  guestSystem =
    if hostPlatform.isLinux then
      ...
      let
        ...
        message = "awos Test: don't know which VM guest system to pair with VM host system: ${hostPlatform.system}. Perhaps you intended to run the tests on a Linux host, or one of the following systems that may run awos tests: ${supportedHosts}";
      in
...

{
  ...
  options = {
    ...
    nodes = mkOption {
      ...
      visible = "shallow";
      description = ''
        An attribute set of awos configuration modules.

        The configurations are augmented by the [`defaults`](#test-opt-defaults) option.

        They are assigned network addresses according to the `awos/lib/testing/network.nix` module.

        A few special options are available, that aren't in a plain awos configuration. See [Configuring the nodes](#sec-awos-test-nodes)
      '';
    };

    defaults = mkOption {
      description = ''
        awos configuration that is applied to all [{option}`nodes`](#test-opt-nodes).
      '';
    ...
    extraBaseModules = mkOption {
      description = ''
        awos configuration that, like [{option}`defaults`](#test-opt-defaults), is applied to all [{option}`nodes`](#test-opt-nodes) and can not be undone with [`specialisation.<name>.inheritParentConfig`](https://search.awos.org/options?show=specialisation.%3Cname%3E.inheritParentConfig&from=0&size=50&sort=relevance&type=packages&query=specialisation).
      '';
      type = types.deferredModule;
      default = { };
    };
    ...
    node.specialArgs = mkOption {
      ...
      description = ''
        ...
        Note that it is not possible to override these from within the awos configurations. If you argument is not relevant to `imports`, consider setting {option}`defaults._module.args.<name>` instead.
      '';
    };
