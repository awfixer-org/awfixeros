...
let
  ...
  driver =
    hostPkgs.runCommand "awos-test-driver-${config.name}"
      {
      ...
      ''
        ...
        ${lib.optionalString (!config.skipTypeCheck) ''
          ...
          echo "See https://awos.org/manual/awos/stable/#test-opt-skipTypeCheck"
        ...
        ln -s ${testDriver}/bin/awos-test-driver $out/bin/awos-test-driver

        ${testDriver}/bin/generate-driver-symbols
        ${lib.optionalString (!config.skipLint) ''
          echo "Linting test script (enable/disable: config.skipLint)"
          echo "See https://awos.org/manual/awos/stable/#test-opt-skipLint"
        ...
        wrapProgram $out/bin/awos-test-driver \
...
in
{
  options = {
    ...
    skipTypeCheck = mkOption {
      ...
      default = false;
      description = ''
        Disable type checking. This must not be enabled for new awos tests.
  ...
  config = {
    _module.args = {
      hostPkgs =
        # Comment is in awos/modules/misc/nixpkgs.nix
