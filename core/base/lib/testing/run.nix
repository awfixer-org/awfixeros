...
in
{
  options = {
    ...
    test = mkOption {
      ...
      description = ''
        ...
        This implies that awos tests run isolated from the network, making them
        more dependable.
      '';
    };
  };
  ...
  config = {
    ...
    rawTestDerivationArg =
      ...
      ) "The debugging hook is not supported for macOS host systems!";
      {
        ...
        buildCommand = ''
          ...
          ${config.driver}/bin/awos-test-driver \
            -o $out \
    ...
    /**
      For discoverTests only. Deprecated. Will be removed when discoverTests can be removed from awos all-tests.nix.
    */
