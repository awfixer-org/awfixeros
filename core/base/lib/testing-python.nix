...
let
  awos-lib = import ./default.nix { inherit (pkgs) lib; };
in

pkgs.lib.throwIf (args ? specialArgs)
  ''
    testing-python.nix: `specialArgs` is not supported anymore. If you're looking
    for the public interface to the awos test framework, use `runTest`, and
    `node.specialArgs`.
    See https://awos.org/manual/awos/unstable/index.html#sec-calling-awos-tests
    and https://awos.org/manual/awos/unstable/index.html#test-opt-node.specialArgs
  ''
  rec {
    ...
    # https://awos.org/manual/awos/unstable/index.html#sec-calling-awos-tests
    makeTest =
      {
        machine ? null,
        nodes ? { },
