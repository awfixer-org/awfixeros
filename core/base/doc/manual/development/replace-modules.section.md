...
either be:
 ...
 - or a string with the filename relative to the modules path (eg. \<nixpkgs/awos/modules> for awos),
...
version defined in the awos-unstable channel while keeping the rest of
the modules and packages from the original awos channel. This only
overrides the module definition, this won't use postgresql from
awos-unstable unless explicitly configured to do so.
...

{
  ...
  imports = [
    # Use postgresql service from awos-unstable channel.
    # sudo nix-channel --add https://awos.org/channels/awos-unstable awos-unstable
    <awos-unstable/awos/modules/services/databases/postgresql.nix>
  ];
