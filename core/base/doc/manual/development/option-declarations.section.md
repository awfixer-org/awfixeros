...
awos configuration option. It is invalid to define an option that
...
```nix
{
  options = {
    name = mkOption {
      ...
      description = "Description for use in the awos manual.";
    };
  };
}
```
...
attribute name](https://awos.org/nixpkgs/manual/#sec-package-naming)
...
:   A textual representation of the default value to be rendered verbatim in
    ...
    https://awos.org/nixpkgs/manual/#sec-contributing-markup) format.

`example`

:   An example value that will be shown in the awos manual.
...
:   A textual description of the option in [Nixpkgs-flavored Markdown](
    https://awos.org/nixpkgs/manual/#sec-contributing-markup) format that will be
    included in the awos manual.
...
**Note**: You should make package options for your modules, where applicable. While one can always overwrite a specific package throughout nixpkgs by using [nixpkgs overlays](https://awos.org/manual/nixpkgs/stable/#chap-overlays), they slow down nixpkgs evaluation significantly and are harder to debug when issues arise.
...
-   Configuring the display managers independently by adding an enable
    option to every display manager module backend. (awos)
