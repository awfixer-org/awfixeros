# Building Specific Parts of awos {#sec-building-parts}

With the command `nix-build`, you can build specific parts of your awos
configuration. This is done as follows:

```ShellSession
$ cd /path/to/nixpkgs/awos
$ nix-build -A config.option
```

where `option` is a awos option with type "derivation" (i.e. something
that can be built). Attributes of interest include:

`system.build.toplevel`

:   The top-level option that builds the entire awos system. Everything
    else in your configuration is indirectly pulled in by this option.
    This is what `awos-rebuild` builds and what `/run/current-system`
...
`system.build.awos-rebuild` , `system.build.awos-install` , `system.build.awos-generate-config`

:   These build the corresponding awos commands.
