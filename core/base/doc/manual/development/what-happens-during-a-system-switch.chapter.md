...
Running `awos-rebuild switch` is one of the more common tasks under awos.
...
`awos-rebuild`, like many deployment solutions, calls `switch-to-configuration`
which resides in a awos system at `$out/bin/switch-to-configuration`. The
...
configuration will be the next one to boot. Unless `awos_NO_SYNC` is set to
