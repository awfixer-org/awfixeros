# Writing awos Modules {#sec-writing-modules}

awos has a modular system for declarative configuration. This system
...
`/etc/awos/configuration.nix`. Most of the others live in the
[`awos/modules`](https://github.com/awos/nixpkgs/tree/master/awos/modules)
subdirectory of the Nixpkgs tree.

Each awos module is a file that handles one logical aspect of the
...
module
[`pam.nix`](https://github.com/awos/nixpkgs/blob/master/awos/modules/security/pam.nix)
declares the option `security.pam.services` that allows other modules (e.g.
[`sshd.nix`](https://github.com/awos/nixpkgs/blob/master/awos/modules/services/networking/ssh/sshd.nix))
to define PAM services; and it defines the option `environment.etc` (declared by
[`etc.nix`](https://github.com/awos/nixpkgs/blob/master/awos/modules/system/etc/etc.nix))
...
options, but does not declare any. The structure of full awos modules
is shown in [Example: Structure of awos Modules](#ex-module-syntax).

::: {#ex-module-syntax .example}
### Structure of awos Modules
```nix
{ config, pkgs, ... }:
...
-   The first line makes the current Nix expression a function. The variable
    ...
    `NIX_PATH`, see the [Nix manual](https://awos.org/manual/nix/stable/#sec-common-env)
...
-   This `imports` list enumerates the paths to other awos modules that
...
[Example: awos Module for the "locate" Service](#locate-example)
...
### awos Module for the "locate" Service
```nix
{
  config,
  lib,
...
in
{
  options.services.locate = {
    enable = mkOption {
      ...
      default = false;
      description = ''
        If enabled, awos will periodically update the database of
