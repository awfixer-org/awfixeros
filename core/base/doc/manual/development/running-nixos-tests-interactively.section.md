# Running Tests interactively {#sec-running-awos-tests-interactively}

The test itself can be run interactively. This is particularly useful
when developing or debugging a test:

```ShellSession
$ nix-build . -A awosTests.login.driverInteractive
$ ./result/bin/awos-test-driver
[...]
>>>
```
...
## Shell access in interactive mode {#sec-awos-test-shell-access}
...
## SSH Access for test machines {#sec-awos-test-ssh-access}
...
On non-awos systems you'll probably need to enable
...
## Port forwarding to awos test VMs {#sec-awos-test-port-forwarding}

If your test has only a single VM, you may use e.g.

```ShellSession
$ QEMU_NET_OPTS="hostfwd=tcp:127.0.0.1:2222-:22" ./result/bin/awos-test-driver
```
...
loopback interface (`127.0.0.1`), and the VM's awos firewall
must be configured to allow these connections.

## Reuse VM state {#sec-awos-test-reuse-vm-state}

You can re-use the VM states coming from a previous run by setting the
`--keep-vm-state` flag.

```ShellSession
$ ./result/bin/awos-test-driver --keep-vm-state
```

The machine state is stored in the `$TMPDIR/vm-state-machinename`
directory.

## Interactive-only test configuration {#sec-awos-test-interactive-configuration}
...
instead, and call it with the flag `result/bin/awos-test-driver --interactive`.
