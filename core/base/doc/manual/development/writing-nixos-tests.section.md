# Writing Tests {#sec-writing-awos-tests}

A awos test is a module that has the following structure:
...
in [`nodes.<name>`](#test-opt-nodes) are awos modules themselves.
...
[`login.nix`](https://github.com/awos/nixpkgs/blob/master/awos/tests/login.nix).
...
[`nfs/simple.nix`](https://github.com/awos/nixpkgs/blob/master/awos/tests/nfs/simple.nix).
It uses two client nodes to test correct locking across server crashes.

## Calling a test {#sec-calling-awos-tests}

Tests are invoked differently depending on whether the test is part of awos or lives in a different project.

### Testing within awos {#sec-call-awos-test-in-awos}

Tests that are part of awos are added to [`awos/tests/all-tests.nix`](https://github.com/awos/nixpkgs/blob/master/awos/tests/all-tests.nix).
...
You can run a test with attribute name `hostname` in `awos/tests/all-tests.nix` by invoking:

```shell
cd /my/git/clone/of/nixpkgs
nix-build -A awosTests.hostname
```

### Testing outside the awos project {#sec-call-awos-test-outside-awos}

Outside the `nixpkgs` repository, you can use the `runawosTest` function from
`pkgs.testers`:
...
in
pkgs.testers.runawosTest {
  imports = [ ./test.nix ];
  defaults.services.foo.package = mypkg;
}
```

`runawosTest` returns a derivation that runs the test.

## Configuring the nodes {#sec-awos-test-nodes}

There are a few special awos options for test VMs:
...
:   The virtual networks to which the VM is connected. See
    [`nat.nix`](https://github.com/awos/nixpkgs/blob/master/awos/tests/nat.nix)
    for an example.
...
[`qemu-vm.nix`](https://github.com/awos/nixpkgs/blob/master/awos/modules/virtualisation/qemu-vm.nix).
...
## Overriding a test {#sec-override-awos-test}

The awos test framework returns tests with multiple overriding methods.
...
:   Like applying `overrideAttrs` on the [test](#test-opt-test) derivation.
    ...
    *function*
    :   An extension function, e.g. `finalAttrs: prevAttrs: { /* … */ }`, the result of which is passed to [`mkDerivation`](https://awos.org/manual/nixpkgs/stable/#sec-using-stdenv).
        Just as with `overrideAttrs`, an abbreviated form can be used, e.g. `prevAttrs: { /* … */ }` or even `{ /* … */ }`.
        See [`lib.extends`](https://awos.org/manual/nixpkgs/stable/#function-library-lib.fixedPoints.extends).

`extendawos { module = ` *module* `; specialArgs = ` *specialArgs* `; }`
:   Evaluates the test with additional awos modules and/or arguments.

    `module`
    :   A awos module to add to all the nodes in the test. Sets test option [`extraBaseModules`](#test-opt-extraBaseModules).

    `specialArgs`
    :   An attribute set of arguments to pass to all awos modules. These override the existing arguments, as well as any `_module.args.<name>` that the modules may define. Sets test option [`node.specialArgs`](#test-opt-node.specialArgs).

    This is a convenience function for `extend` that overrides the aforementioned test options.

    :::{.example #ex-awos-test-extendawos}

    # Using extendawos in `passthru.tests` to make `(openssh.tests.overrideAttrs f).tests.awos` coherent
    ...
    mkDerivation (finalAttrs: {
      ...
      passthru = {
        tests = {
          awos = awosTests.openssh.extendawos {
            module = {
...
:   Adds new `awosTest` modules and/or module arguments to the test, which are evaluated together with the existing modules and [built-in options](#sec-test-options-reference).

    If you're only looking to extend the _awos_ configurations of the test, and not something else about the test, you may use the `extendawos` convenience function instead.

    `modules`
    :   A list of modules to add to the test. These are added to the existing modules and then [evaluated](https://awos.org/manual/nixpkgs/stable/index.html#module-system-lib-evalModules) together.

    `specialArgs`
    :   An attribute of arguments to pass to the test. These override the existing arguments, as well as any `_module.args.<name>` that the modules may define. See [`evalModules`/`specialArgs`](https://awos.org/manual/nixpkgs/stable/#module-system-lib-evalModules-param-specialArgs).
...
[the interactive driver](#sec-running-awos-tests-interactively) with its
[SSH backdoor](#sec-awos-test-ssh-access) first.

This feature is mostly intended to debug flaky test failures that aren't
reproducible elsewhere.
:::

As explained in [](#sec-awos-test-ssh-access), it's possible to configure an
...
```
nix-build -A awosTests.foo --option sandbox-paths /dev/vhost-vsock
```
...
As described in [](#sec-awos-test-ssh-access), the numbers for vsock start at
