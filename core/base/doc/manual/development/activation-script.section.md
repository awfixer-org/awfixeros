...
configuration which resides in a awos system in `$out/activate`. Since its
contents depend on your system configuration, the contents may differ.
This chapter explains how the script works in general and some common awos
...
is also run when `awos-rebuild dry-activate` is run. To differentiate between
real and dry activation, the `$awos_ACTION` environment variable can be
...
The files that can be written to are `/run/awos/activation-restart-list` and
`/run/awos/activation-reload-list` with their respective counterparts for
dry activation being `/run/awos/dry-activation-restart-list` and
`/run/awos/dry-activation-reload-list`. Those files can contain
...
## awos snippets {#sec-activation-script-awos-snippets}

There are some snippets awos enables by default because disabling them would
