...
- Nix has been updated from 2.3 to 2.8. This mainly brings experimental support
  ...
  release notes for [nix-2.4](https://awos.org/manual/nix/stable/release-notes/rl-2.4.html),
  [nix-2.5](https://awos.org/manual/nix/stable/release-notes/rl-2.5.html),
  [nix-2.6](https://awos.org/manual/nix/stable/release-notes/rl-2.6.html),
  [nix-2.7](https://awos.org/manual/nix/stable/release-notes/rl-2.7.html) and
  [nix-2.8](https://awos.org/manual/nix/stable/release-notes/rl-2.8.html)
...
- The GNOME and Plasma installation CDs now use `pkgs.calamares` and `pkgs.calamares-awos-extensions` to allow users to easily install and set up awos with a GUI.
...
- [nix-ld](https://github.com/Mic92/nix-ld), Run unpatched dynamic binaries on awos. Available as [programs.nix-ld](#opt-programs.nix-ld.enable).
...
- The update of the haskell package set brings with it a new version of the `xmonad`
  ...
  example code the corresponding awos module was adjusted, you may want to have a look at it.
...
- The configuration and state directories used by `awos-containers` have been
  moved from `/etc/containers` and `/var/lib/containers` to
  `/etc/awos-containers` and `/var/lib/awos-containers`.
...
- In the PowerDNS Recursor module (`services.pdns-recursor`), default values of several IP address-related awos options have been updated to match the default upstream behavior.
...
- The Keycloak package (`pkgs.keycloak`) has been switched from the
  ...
  - `services.keycloak.extraConfig` has been removed in favor of the
    new [settings-style](https://github.com/awos/rfcs/blob/master/rfcs/0042-config-option.md)
...
- Services in the `hadoop` module previously set `openFirewall` to true by default.
  ...
  `openFirewall = true;` to be added to to hadoop services when upgrading from awos 21.11.
...
- The `dendrite` package has been upgraded from 0.5.1 to
  ...
  in awos, require merging of the federation sender and signing key
...
- `switch-to-configuration` (the script that is run when running `awos-rebuild switch` for example) has been reworked
    * The interface that allows activation scripts to restart units has been streamlined. Restarting and reloading is now done by a single file `/run/awos/activation-restart-list` that honors `restartIfChanged` and `reloadIfChanged` of the units.
        * Preferring to reload instead of restarting can still be achieved using `/run/awos/activation-reload-list`.
    * The script now uses a proper ini-file parser to parse systemd units. Some values are now only searched in one section instead of in the entire unit. This is only relevant for units that don't use the awos systemd moule.
...
- The `services.bookstack.extraConfig` option has been replaced by
  ...
  [settings-style](https://github.com/awos/rfcs/blob/master/rfcs/0042-config-option.md)
  configuration.
...
- The option [virtualisation.vmVariant](#opt-virtualisation.vmVariant) was added
  to allow users to make changes to the `awos-rebuild build-vm` configuration
...
- The configuration portion of the `nix-daemon` module has been reworked and exposed as [nix.settings](options.html#opt-nix-settings):
  * Legacy options have been mapped to the corresponding options under [nix.settings](options.html#opt-nix.settings) and will be deprecated when awos 21.11 reaches end of life.
...
- Peertube now uses services.redis.servers to start a new redis server, instead of using a global redis server.
  ...
  ```
- Added the `keter` awos module. Keter reverse proxies requests to your loaded application based on virtual hostnames.

- If you are using Wayland you can choose to use the Ozone Wayland support
  in Chrome and several Electron apps by setting the environment variable
  `awos_OZONE_WL=1` (for example via
  `environment.sessionVariables.awos_OZONE_WL = "1"`).
...
- The `services.stubby` module was converted to a [settings-style](https://github.com/awos/rfcs/blob/master/rfcs/0042-config-option.md) configuration.
...
- The options `boot.extraModprobeConfig` and `boot.blacklistedKernelModules` now also take effect in the initrd by copying the file `/etc/modprobe.d/awos.conf` into the initrd.

- `awos-generate-config` now puts the dhcp configuration in `hardware-configuration.nix` instead of `configuration.nix`.
...
- The `services.mbpfan` module was converted to a [RFC 0042](https://github.com/awos/rfcs/blob/master/rfcs/0042-config-option.md) configuration.
...
- The `hadoop` package has added support for `aarch64-linux` and `aarch64-darwin` as of 3.3.1 ([#158613](https://github.com/awos/nixpkgs/pull/158613)).

- The `R` package now builds again on `aarch64-darwin` ([#158992](https://github.com/awos/nixpkgs/pull/158992)).
...
- The default `scribus` version is now 1.5, while version 1.4 is still available as `scribus_1_4` ([#172700](https://github.com/awos/nixpkgs/pull/172700)).
...
- The `spark3` package has been updated from 3.1.2 to 3.2.1 ([#160075](https://github.com/awos/nixpkgs/pull/160075)):

  - Testing has been enabled for `aarch64-linux` in addition to `x86_64-linux`.
  - The `spark3` package is now usable on `aarch64-darwin` as a result of [#158613](https://github.com/awos/nixpkgs/pull/158613) and [#158992](https://github.com/awos/nixpkgs/pull/158992).

- The option `services.snapserver.openFirewall` will no longer default to
  `true` starting with awos 22.11. Enable it explicitly if you need to control
...
- The new [`postgresqlTestHook`](https://awos.org/manual/nixpkgs/stable/#sec-postgresqlTestHook) runs a PostgreSQL server for the duration of package checks.
