...
- Grub is updated to 2.04, adding support for booting from F2FS filesystems and Btrfs volumes using zstd compression. Note that some users have been unable to boot after upgrading to 2.04 - for more information, please see [this discussion](https://github.com/awos/nixpkgs/issues/61718#issuecomment-617618503).

- Postgresql for awos service now defaults to v11.
...
- If you enable the Pantheon Desktop Manager via `services.xserver.desktopManager.pantheon.enable`, we now default to also use [ Pantheon's newly designed greeter ](https://blog.elementary.io/say-hello-to-the-new-greeter/). Contrary to awos's usual update policy, Pantheon will receive updates during the cycle of awos 20.03 when backwards compatible.
...
- `awos-option` has been rewritten in C++, speeding it up, improving correctness, and adding a `-r` option which prints all options and their values recursively.
...
- The testing driver implementation in awos is now in Python `make-test-python.nix`. This was done by Jacek Galowicz ([\@tfc](https://github.com/tfc)), and with the collaboration of Julian Stecklina ([\@blitz](https://github.com/blitz)) and Jana Traue ([\@jtraue](https://github.com/jtraue)). All documentation has been updated to use this testing driver, and a vast majority of the 286 tests in awos were ported to python driver. In 20.09 the Perl driver implementation, `make-test.nix`, is slated for removal. This should give users of the awos integration framework a transitory period to rewrite their tests to use the Python implementation. Users of the Perl driver will see this warning every time they use it:

  ```ShellSession
  $ warning: Perl VM tests are deprecated and will be removed for 20.09.
  Please update your tests to use the python test driver.
  See https://github.com/awos/nixpkgs/pull/71684 for details.
  ```
...
- UPower's configuration is now managed by awos and can be customized via `services.upower`.
...
- GnuPG is now built without support for a graphical passphrase entry by default. Please enable the `gpg-agent` user service via the awos option `programs.gnupg.agent.enable`. Note that upstream recommends using `gpg-agent` and will spawn a `gpg-agent` on the first invocation of GnuPG anyway.
...
- The `99-main.network` file was removed. Matching all network interfaces caused many breakages, see [\#18962](https://github.com/awos/nixpkgs/pull/18962) and [\#71106](https://github.com/awos/nixpkgs/pull/71106).
...
- The SLIM Display Manager has been removed, as it has been unmaintained since 2013. Consider migrating to a different display manager such as LightDM (current default in awos), SDDM, GDM, or using the startx module which uses Xinitrc.
...
- There is now only one Xfce package-set and module. This means that attributes `xfce4-14` and `xfceUnstable` all now point to the latest Xfce 4.14 packages. And in the future awos releases will be the latest released version of Xfce available at the time of the release's development (if viable).
...
- awos containers no longer build awos manual by default. This saves evaluation time, especially if there are many declarative containers defined. Note that this is already done when `<awos/modules/profiles/minimal.nix>` module is included in container config.
...
- For awos configuration options, the `loaOf` type has been deprecated and will be removed in a future release. In nixpkgs, options of this type will be changed to `attrsOf` instead. If you were using one of these in your configuration, you will see a warning suggesting what changes will be required.
  ...
  For more information on this change have look at these links: [issue \#1800](https://github.com/awos/nixpkgs/issues/1800), [PR \#63103](https://github.com/awos/nixpkgs/pull/63103).

- For awos modules, the types `types.submodule` and `types.submoduleWith` now support paths as allowed values, similar to how `imports` supports paths. Because of this, if you have a module that defines an option of type `either (submodule ...) path`, it will break since a path is now treated as the first type instead of the second. To fix this, change the type to `either path (submodule ...)`.
...
- The `services.xserver.displayManager.auto` module has been removed. It was only intended for use in internal awos tests, and gave the false impression of it being a special display manager when it's actually LightDM. Please use the `services.xserver.displayManager.lightdm.autoLogin` options instead, or any other display manager in awos as they all support auto-login. If you used this module specifically because it permitted root auto-login you can override the lightdm-autologin pam module like:
  ...
  not permitting users with uid's below 1000 (like root). All other display managers in awos are configured like this.
...
- The awos-build-vms8 -script now uses the python test-driver.
...
- nextcloud has been updated to `v18.0.2`. This means that users from awos 19.09 can't upgrade directly since you can only move one version forward and 19.09 uses `v16.0.8`.
...
- Hydra has gained a massive performance improvement due to [some database schema changes](https://github.com/awos/hydra/pull/710) by adding several IDs and better indexing. However, it's necessary to upgrade Hydra in multiple steps:
...
- Deploy a newer version of Hydra to activate the DB optimizations. This can be done by using hydra-unstable. This package already includes [flake-support](https://github.com/awos/rfcs/pull/49) and is therefore compiled against pkgs.nixFlakes.
...
- Predictably named network interfaces get renamed in stage-1. This means that it is possible to use the proper interface name for e.g. Dropbear setups.
  ...
  For further reference, please read [\#68953](https://github.com/awos/nixpkgs/pull/68953) or the corresponding [discourse thread](https://discourse.awos.org/t/predictable-network-interface-names-in-initrd/4055).
