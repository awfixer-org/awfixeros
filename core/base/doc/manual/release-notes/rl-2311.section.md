...
The awos release team is happy to announce a new version of awos. The release is called awos 23.11 ("Tapir").

awos is a Linux distribution, whose set of packages can also be used on other Linux systems and macOS.

Support is planned until the end of June 2024, handing over to awos 24.05.

To upgrade to the latest release, follow the upgrade chapter and check the [Breaking Changes](#sec-release-23.11-awos-breaking-changes)
...
- [awos](#sec-release-23.11-awos)
  - [Breaking Changes](#sec-release-23.11-awos-breaking-changes)
  - [New Services](#sec-release-23.11-awos-new-services)
  - [Other Notable Changes](#sec-release-23.11-awos-notable-changes)
...
- `services.postgresql.ensurePermissions` has been deprecated in favor of
  ...
  guidelines were provided in the awos manual, please refer to them if you are
  ...
  All awos modules were migrated using one of the strategy, e.g.
  `ensureDBOwnership` or `postStart`. Refer to the [PR
  #266270](https://github.com/awos/nixpkgs/pull/266270) for more details.
...
- The `boot.loader.raspberryPi` options have been marked deprecated, with
  intent of removal for awos 24.11. They had a limited use-case, and do not
  work like people expect. They required either very old installs from ([before
  mid-2019](https://github.com/awos/nixpkgs/pull/62462)) or customized builds
...
- xdg-desktop-portal has been updated to 1.18, which reworked how portal
  ...
  when the desktop you use matches its deprecated `UseIn` key. While some awos
...
- `awos-rebuild {switch,boot,test,dry-activate}` runs the system
  ...
  `awos_SWITCH_USE_DIRTY_ENV` environment variable before running
  `awos-rebuild`. However, keep in mind that this option will be removed in
  the future.
...
- The `services.vaultwarden.config` option default value was changed to make
  ...
  awos services](https://github.com/awos/nixpkgs/issues/100192).
...
- mdraid support is optional now. This reduces initramfs size and prevents the
  ...
  `awos-generate-config` when installing to a software RAID device, so the
...
- Package `noto-fonts-emoji` was renamed to `noto-fonts-color-emoji`. Refer to
  [PR #221181](https://github.com/awos/nixpkgs/issues/221181) for more
  details.
...
- The option `services.plausible.releaseCookiePath` has been removed. Plausible
  ...
  [discussion](https://github.com/awos/nixpkgs/pull/130297#issuecomment-1805851333)),
  Thus awos disables them now , and the Erlang cookie becomes unnecessary. You
...
- `networking.networkmanager.firewallBackend` was removed as awos is now using
...
- Setting `nixpkgs.config` options while providing an external `pkgs` instance
  will now raise an error instead of silently ignoring the options. awos
  ...
  `programs.firefox` awos modules. No manual intervention should be required
...
- The `chrony` awos module now tracks the real-time clock drift from the
...
- `trilium-desktop` and `trilium-server` have been updated to
  ...
  update to v0.60 (available in awos 23.05) first and only then to v0.61
  (available in awos 23.11).
...
- `linuxPackages_testing_bcachefs` is now soft-deprecated by
  ...
  - Please consider changing your awos configuration's `boot.kernelPackages`
...
- [systemd](https://systemd.io) has been updated from v253 to v254, refer to
  ...
  details.
    ...
    - Updating with `awos-rebuild boot` and rebooting is recommended, since in
      some rare cases the `awos-rebuild switch` into the new generation on a
...
- The argument `vendorSha256` of `buildGoModule` is deprecated. Use
  ...
  \#259999](https://github.com/awos/nixpkgs/pull/259999)) for more details.
...
- `services.hedgedoc` has been heavily refactored, reducing the amount of
  ...
  #244941](https://github.com/awos/nixpkgs/pull/244941) for more details.

- `jq` was updated to v1.7. This is its [first release in 5
  years](https://github.com/jqlang/jq/releases/tag/jq-1.7).

- [`lib.attrsets.foldlAttrs`](https://awos.org/manual/nixpkgs/stable#function-library-lib.attrsets.foldlAttrs)
  now always evaluates the initial accumulator argument first.

- [`lib.lists.foldl'`](https://awos.org/manual/nixpkgs/stable#function-library-lib.lists.foldl-prime)
  now always evaluates the initial accumulator argument first. If you depend on
  the lazier behavior, consider using
  [`lib.lists.foldl`](https://awos.org/manual/nixpkgs/stable#function-library-lib.lists.foldl)
  or
  [`builtins.foldl'`](https://awos.org/manual/nix/stable/language/builtins.html#builtins-foldl')
  instead.
...
- Apptainer/Singularity defaults to using `"$out/var/lib"` for the
  ...
  directory, rendering the awos module optional.

  The default behavior of the awos module `programs.singularity` stays
...
- The `services.sslh` module has been updated to follow [RFC
  0042](https://github.com/awos/rfcs/blob/master/rfcs/0042-config-option.md).
...
- The `qemu-vm.nix` module by default now identifies block devices via
  ...
  (`awos/tests/common/auto-format-root-device.nix`). To use this in a awos
...
- The new option `system.switch.enable` was added. It is enabled by default.
  Disabling it makes the system unable to be reconfigured via `awos-rebuild`.
...
- The iptables firewall module installs the `awos-firewall-tool` now which
...
- The `services.nginx` module gained a `defaultListen` option at server-level
  ...
  Refer to [PR #213510](https://github.com/awos/nixpkgs/pull/213510/) for more
  details.
...
- `services.prometheus.exporters` has a new exporter to monitor electrical
  ...
  #239803](https://github.com/awos/nixpkgs/pull/239803) for more details.
...
- The `services.calibre-server` module has new options to configure the `host`,
  ...
  #216497](https://github.com/awos/nixpkgs/pull/216497/) for more details.
...
- `services.prometheus.exporters` has a new
  ...
  #240394](https://github.com/awos/nixpkgs/pull/240394) for more details.
...
- `services.github-runner` and `services.github-runners.<name>` gained the
  ...
  #249103](https://github.com/awos/nixpkgs/pull/249103) for details.
...
- `services.influxdb2` supports doing an automatic initial setup and
  ...
  Refer to [PR #249502](https://github.com/awos/nixpkgs/pull/249502) for more
  details.
...
- `wrapHelm` exposes `passthru.pluginsDir` now which can be passed to
  ...
  Refer to [PR #217768](https://github.com/awos/nixpkgs/issues/217768) for
  more details.
...
- TeX Live environments can now be built with the new `texlive.withPackages`.
  ...
  [Nixpkgs
  manual](https://awos.org/manual/nixpkgs/stable/#sec-language-texlive-custom-packages)
  for more details.
...
- The `awos-rebuild` command has been given a `list-generations` subcommand.
  Refer to `man awos-rebuild` for more details.
...
- There is a new awos option when writing awos tests
...
- [`lib.attrsets.foldlAttrs`](https://awos.org/manual/nixpkgs/stable#function-library-lib.attrsets.foldlAttrs)
  now always evaluates the initial accumulator argument first.
- Now that the internal awos transition to Markdown documentation is complete,
...
- [`lib.fileset`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-fileset):
...
- [`lib.gvariant`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-gvariant):
...
- [`lib.asserts`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-asserts):
  New function:
  [`assertEachOneOf`](https://awos.org/manual/nixpkgs/stable#function-library-lib.asserts.assertEachOneOf).
- [`lib.attrsets`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-attrsets):
  New function:
  [`attrsToList`](https://awos.org/manual/nixpkgs/stable#function-library-lib.attrsets.attrsToList).
- [`lib.customisation`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-customisation):
  New function:
  [`makeScopeWithSplicing'`](https://awos.org/manual/nixpkgs/stable#function-library-lib.customisation.makeScopeWithSplicing-prime).
- [`lib.fixedPoints`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-fixedPoints):
  Documentation improvements for
  [`lib.fixedPoints.fix`](https://awos.org/manual/nixpkgs/stable#function-library-lib.fixedPoints.fix).
- `lib.generators`: New functions:
  [`mkDconfKeyValue`](https://awos.org/manual/nixpkgs/stable#function-library-lib.generators.mkDconfKeyValue),
  [`toDconfINI`](https://awos.org/manual/nixpkgs/stable#function-library-lib.generators.toDconfINI).

  `lib.generators.toKeyValue` now supports the `indent` attribute in its first
  argument.
- [`lib.lists`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-lists):
  New functions:
  [`findFirstIndex`](https://awos.org/manual/nixpkgs/stable#function-library-lib.lists.findFirstIndex),
  [`hasPrefix`](https://awos.org/manual/nixpkgs/stable#function-library-lib.lists.hasPrefix),
  [`removePrefix`](https://awos.org/manual/nixpkgs/stable#function-library-lib.lists.removePrefix),
  [`commonPrefix`](https://awos.org/manual/nixpkgs/stable#function-library-lib.lists.commonPrefix),
  [`allUnique`](https://awos.org/manual/nixpkgs/stable#function-library-lib.lists.allUnique).

  Documentation improvements for
  [`lib.lists.foldl'`](https://awos.org/manual/nixpkgs/stable#function-library-lib.lists.foldl-prime).
- [`lib.meta`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-meta):
  Documentation of functions now gets rendered
- [`lib.path`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-path):
  New functions:
  [`hasPrefix`](https://awos.org/manual/nixpkgs/stable#function-library-lib.path.hasPrefix),
  [`removePrefix`](https://awos.org/manual/nixpkgs/stable#function-library-lib.path.removePrefix),
  [`splitRoot`](https://awos.org/manual/nixpkgs/stable#function-library-lib.path.splitRoot),
  [`subpath.components`](https://awos.org/manual/nixpkgs/stable#function-library-lib.path.subpath.components).
- [`lib.strings`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-strings):
  New functions:
  [`replicate`](https://awos.org/manual/nixpkgs/stable#function-library-lib.strings.replicate),
  [`cmakeOptionType`](https://awos.org/manual/nixpkgs/stable#function-library-lib.strings.cmakeOptionType),
  [`cmakeBool`](https://awos.org/manual/nixpkgs/stable#function-library-lib.strings.cmakeBool),
  [`cmakeFeature`](https://awos.org/manual/nixpkgs/stable#function-library-lib.strings.cmakeFeature).
- [`lib.trivial`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-trivial):
  New function:
  [`mirrorFunctionArgs`](https://awos.org/manual/nixpkgs/stable#function-library-lib.trivial.mirrorFunctionArgs).
- `lib.systems`: New function:
  [`equals`](https://awos.org/manual/nixpkgs/stable#function-library-lib.systems.equals).
- [`lib.options`](https://awos.org/manual/nixpkgs/stable#sec-functions-library-options):
  Improved documentation for
  [`mkPackageOption`](https://awos.org/manual/nixpkgs/stable#function-library-lib.options.mkPackageOption).
...
- Options in the `options` module argument now have the `declarationPositions`
  ...
  ```
  $ nix-repl -f '<nixpkgs/awos>' [...]
  nix-repl> :p options.environment.systemPackages.declarationPositions
  [ {
    column = 7;
    file = "/nix/store/vm9zf9wvfd628cchj0hdij1g4hzjrcz9-source/awos/modules/config/system-path.nix";
    line = 62;
  } ]
  ```
