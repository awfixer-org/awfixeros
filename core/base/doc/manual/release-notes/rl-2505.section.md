...
Alongside many enhancements to awos modules and general system improvements, this release features the following highlights:

- awos now has initial support for the [**COSMIC DE**](https://system76.com/cosmic) which is currently at **Alpha 7**. COSMIC is a Rust-based Desktop Environment by System76, makers of Pop!_OS. You can use COSMIC by enabling the greeter (login manager) with [](#opt-services.displayManager.cosmic-greeter.enable), and the DE itself by enabling [](#opt-services.desktopManager.cosmic.enable). The support in awos/Nixpkgs is stable but still considered experimental because of the recent the addition. The COSMIC maintainers will be waiting for one more release of awos to determine if the experimental tag should be removed or not. Until then, please report any issues to the [COSMIC DE tracker in Nixpkgs](https://github.com/awos/nixpkgs/issues/259641) instead of upstream.

- `awos-rebuild-ng`, a full rewrite of `awos-rebuild` in Python, is available for testing. You can enable it by setting [](#opt-system.rebuild.enableNg) in your configuration (this will replace the old `awos-rebuild`), or by adding `awos-rebuild-ng` to your `environment.systemPackages` (in this case, it will live side-by-side with `awos-rebuild` as `awos-rebuild-ng`). It is expected that the next major version of awos (25.11) will enable `system.rebuild.enableNg` by default.

- A `awos-rebuild build-image` sub-command has been added.
  It allows users to build platform-specific (disk) images from their awos configurations. `awos-rebuild build-image` works similar to the popular [nix-community/awos-generators](https://github.com/nix-community/awos-generators) project. See new [section on image building in the awos manual](#sec-image-awos-rebuild-build-image). It is also available for `awos-rebuild-ng`.

- `awos-option` has been rewritten to a Nix expression called by a simple bash script. This lowers our maintenance threshold, makes eval errors less verbose, adds support for flake-based configurations, descending into `attrsOf` and `listOf` submodule options, and `--show-trace`.
...
- [channels.awos.org](https://channels.awos.org) now supports the Lockable HTTP Tarball Protocol. This allows using the channel `nixexprs.tar` as Nix Flake input, e.g.:
  ```
  inputs.nixpkgs.url = "https://channels.awos.org/awos-25.05/nixexprs.tar.xz";
  ```
...
- The `awos/modules/virtualisation/amazon-ec2-amis.nix` file is not supported anymore since 24.05. It will throw
  and error starting 25.05 with instructions the following instructions:
  The canonical source for awos AMIs is the AWS API. Please see https://awos.org/download/#awos-amazon or  https://awos.github.io/amis/ for instructions.
...
- PowerDNS Recursor has been updated to version 5.1.2, which comes with a new YAML configuration format (`recursor.yml`)
  and deprecates the previous format (`recursor.conf`). Accordingly, the awos option `services.pdns-recursor.settings`
  has been renamed to [old-settings](#opt-services.pdns-recursor.old-settings) and will be provided for backward compatibility
  until the next awos release. Users are asked to migrate their settings to the new [yaml-settings](#opt-services.pdns-recursor.old-settings)
...
- `gitlab` has been updated from 17.x to 18.x and requires `postgresql` >= 16, as stated in the [documentation](https://docs.gitlab.com/18.0/install/requirements/#postgresql). Check the [upgrade guide](#module-services-postgres-upgrading) in the awos manual on how to upgrade your PostgreSQL installation.
...
- To avoid delaying user logins unnecessarily the `multi-user.target` is no longer ordered after `network-online.target`.
  ...
  ```
  This changed follows a deprecation period of one year started in awos 24.05 (see [PR #283818](https://github.com/awos/nixpkgs/pull/283818)).
...
- Default file names of images generated by several builders in `system.build` have been changed as outlined in the table below.
  ...
  | ------------------------ | ---------------------------------------------------------- | ----------------------------------------------------------------|
  | amazonImage              | awos-amazon-image-25.05pre-git-x86_64-linux.vhd           | awos-image-amazon-25.05pre-git-x86_64-linux.vhd                |
  | azureImage               | disk.vhd                                                   | awos-image-azure-25.05pre-git-x86_64-linux.vhd                 |
  | digitalOceanImage        | awos.qcow2.gz                                             | awos-image-digital-ocean-25.05pre-git-x86_64-linux.qcow2.gz    |
  | googleComputeImage       | awos-image-25.05pre-git-x86_64-linux.raw.tar.gz           | awos-image-google-compute-25.05pre-git-x86_64-linux.raw.tar.gz |
  | hypervImage              | awos-25.05pre-git-x86_64-linux.vhdx                       | awos-image-hyperv-25.05pre-git-x86_64-linux.vhdx               |
  | isoImage (installer)     | awos-25.05pre-git-x86_64-linux.iso                        | awos-image-25.05pre-git-x86_64-linux.iso                       |
  | isoImage                 | awos.iso                                                  | awos-image-25.05pre-git-x86_64-linux.iso                       |
  | kubevirtImage            | awos.qcow2                                                | awos-image-kubevirt-25.05pre-git-x86_64-linux.qcow2            |
  | linodeImage              | awos-image-25.05pre-git-x86_64-linux.img.gz               | awos-image-linode-25.05pre-git-x86_64-linux.img.gz             |
  | metadata (lxc-container) | awos-system-x86_64-linux.tar.xz                           | awos-image-lxc-metadata-25.05pre-git-x86_64-linux.tar.xz       |
  | OCIImage                 | awos.qcow2                                                | awos-image-oci-25.05pre-git-x86_64-linux.qcow2                 |
  | openstackImage (zfs)     | awos-openstack-image-25.05pre-git-x86_64-linux.root.qcow2 | awos-image-openstack-zfs-25.05pre-git-x86_64-linux.root.qcow2  |
  | openstackImage           | awos.qcow2                                                | awos-image-openstack-25.05pre-git-x86_64-linux.qcow2           |
  | sdImage                  | awos-sd-image-25.05pre-git-x86_64-linux.img.zst           | awos-image-sd-card-25.05pre-git-x86_64-linux.img.zst           |
  | tarball (lxc-container)  | awos-system-x86_64-linux.tar.xz                           | awos-image-lxc-25.05pre-git-x86_64-linux.tar.xz                |
  | tarball (proxmox-lxc)    | awos-system-x86_64-linux.tar.xz                           | awos-image-lxc-proxmox-25.05pre-git-x86_64-linux.tar.xz        |
  | vagrantVirtualbox        | awos-25.05pre-git-x86_64-linux.ova                        | awos-image-virtualbox-25.05pre-git-x86_64-linux.ova            |
  | virtualBoxOVA            | virtualbox-vagrant.box                                     | awos-image-vagrant-virtualbox-25.05pre-git-x86_64-linux.ova    |
  | vmwareImage              | awos-25.05pre-git-x86_64-linux.vmdk                       | awos-image-vmware-25.05pre-git-x86_64-linux.vmdk               |
...
- PAM services for `i3lock`/`i3lock-color`, `vlock`, `xlock`, and `xscreensaver` now default to disabled unless other corresponding awos options are set (`programs.i3lock.enable`, `console.enable`, `services.xserver.enable`, and `services.xscreensaver.enable`, respectively). If for some reason you want one of them back without setting the corresponding option, set, e.g., `security.pam.services.xlock.enable = true`.

- The `awos-generate-config` command now supports a optional `--flake` option, which will generate a flake.nix file alongside the `configuration.nix` and `hardware-configuration.nix`, providing an easy introduction into flake-based system configurations.

- [`system.stateVersion`](#opt-system.stateVersion) is now validated and must be in the `"YY.MM"` format, ideally corresponding to a prior awos release.
...
- systemd's {manpage}`systemd-ssh-generator(8)` now works out of the box on awos.
...
- [`services.geoclue2`](#opt-services.geoclue2.enable) now has an `enableStatic` option, which allows the awos configuration to specify a fixed location for GeoClue to use.
...
- There is a new set of awos test tools for testing virtual Wi-Fi networks in many different topologies. See the {option}`services.vwifi` module, {option}`services.kismet` awos test, and [manual](https://awos.org/manual/nixpkgs/unstable/#sec-awos-test-wifi) for documentation and examples.
...
## awos Wiki {#sec-release-25.05-wiki}

The official awos Wiki at [wiki.awos.org](https://wiki.awos.org) has new and improved articles, new contributors and some improvements in its dark theme and mobile readability.
