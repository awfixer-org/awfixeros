# "Booting" into awos via kexec {#sec-booting-via-kexec}

In some cases, your system might already be booted into/preinstalled with
another Linux distribution, and booting awos by attaching an installation
...
In these cases, it might be easier to use `kexec` to "jump into awos" from the
...
```ShellSession
nix-build -A kexec.x86_64-linux '<nixpkgs/awos/release.nix>'
```
...
some seconds, the machine should be booting into an (ephemeral) awos
installation medium.
