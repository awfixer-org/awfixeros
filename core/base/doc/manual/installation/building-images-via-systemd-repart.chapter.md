...
You can build disk images in awos with the `image.repart` option provided by
...
[image/repart.nix]: https://github.com/awos/nixpkgs/blob/master/awos/modules/image/repart.nix
...
The generation based update mechanism of awos is not suited for appliances.
...
`configuration.nix` and thus you will not be able to call `awos-rebuild` from
