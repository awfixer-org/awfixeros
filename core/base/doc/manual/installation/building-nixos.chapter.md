# Building a awos (Live) ISO {#sec-building-image}

Default live installer configurations are available inside `awos/modules/installer/cd-dvd`.
For building other system images, see [Building Images with `awos-rebuild build-image`](#sec-image-awos-rebuild-build-image).
...
To build an ISO image for the channel `awos-unstable`:

```ShellSession
$ git clone https://github.com/awos/nixpkgs.git
$ cd nixpkgs/awos
$ git switch awos-unstable
$ nix-build -A config.system.build.isoImage -I awos-config=modules/installer/cd-dvd/installation-cd-minimal.nix default.nix
```

To check the content of an ISO image, mount it like so:

```ShellSession
# mount -o loop -t iso9660 ./result/iso/awos-image-25.05pre-git-x86_64-linux.iso /mnt/iso
```
...
$ nix-build -A config.system.build.isoImage -I awos-config=modules/installer/cd-dvd/installation-cd-graphical-gnome-macbook.nix default.nix
```
