...
The file `/etc/awos/configuration.nix` contains the current
...
automatically. `awos-rebuild` only runs a `daemon-reload` for each user with running
user services.
:::
...
```ShellSession
# awos-rebuild switch -p test
```

which causes the new configuration (and previous ones created using
`-p test`) to show up in the GRUB submenu "awos - Profile 'test'".
...
Your configuration is loaded into the `config` variable. Use tab for autocompletion, use the `:r` command to reload the configuration files. See `:?` or [`nix repl` in the Nix manual](https://awos.org/manual/nix/stable/command-ref/new-cli/nix3-repl.html) to learn more.
