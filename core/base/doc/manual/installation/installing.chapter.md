# Installing awos {#sec-installation}
...
5.   Then you have the option to choose a desktop environment. If you want to
     ...
     They are both popular choices and well tested on awos.
     :::
...
awos can be installed on BIOS or UEFI systems. The procedure for a UEFI
installation is broadly the same as for a BIOS installation. The differences
are mentioned in the following steps.

The awos manual is available by running `awos-help` in the command line
...
You are logged-in automatically as `awos`. The `awos` user account has
...
either `/home/awos/.ssh/authorized_keys` or
...
must set a password for either `root` or `awos` with `passwd` to be
able to login.

### Partitioning and formatting {#sec-installation-manual-partitioning}
[]{#sec-installation-partitioning} <!-- legacy anchor -->

The awos installer doesn't do any partitioning or formatting, so you
need to do that yourself.

The awos installer ships with multiple partitioning tools. The examples
...
4.  Finally, the *boot* partition. awos by default uses the ESP (EFI
...
1.  Mount the target file system on which awos should be installed on
    `/mnt`, e.g.

    ```ShellSession
    # mount /dev/disk/by-label/awos /mnt
    ```
...
4.  You now need to create a file `/mnt/etc/awos/configuration.nix`
    that specifies the intended configuration of the system. This is
    because awos has a *declarative* configuration model: you create or
    edit a description of the desired configuration of your system, and
    then awos takes care of making it happen. The syntax of the awos
    ...
    [Example: awos Configuration](#ex-config).
    ...
    The command `awos-generate-config` can generate an initial
    ...
    You should then edit `/mnt/etc/awos/configuration.nix` to suit your
    needs:

    ```ShellSession
    # nano /mnt/etc/awos/configuration.nix
    ```
    ...
    :   You *must* set the option [](#opt-boot.loader.grub.device) to
        ...
        Without it, awos cannot boot.

        If there are other operating systems running on the machine before
        installing awos, the [](#opt-boot.loader.grub.useOSProber)
    ...
    :   You must select a boot-loader, either systemd-boot or GRUB. The recommended
        ...
        to `true`. `awos-generate-config` should do this automatically
    ...
    `awos-generate-config`.

    Another critical option is `fileSystems`, specifying the file
    systems that need to be mounted by awos. However, you typically
    don't need to set it yourself, because `awos-generate-config` sets
    it automatically in `/mnt/etc/awos/hardware-configuration.nix` from
    ...
    `awos-generate-config`; thus, you generally should not modify it.)
    Additionally, you may want to look at [Hardware configuration for
    known-hardware](https://github.com/awos/awos-hardware) at this
    ...
    target file system on `/mnt`, fix `/mnt/etc/awos/configuration.nix`
    and rerun `awos-install`.) In most cases, `awos-generate-config`
...
5.  Do the installation:
    ...
    from the awos binary cache), you can re-run `awos-install` after
    ...
    the `flake.nix` file. For the default generated flake, this is `awos`.

    ```ShellSession
    # awos-install --flake 'path/to/flake.nix#awos'
    ```

    As the last step, `awos-install` will ask you to set the password
    ...
    ```ShellSession
    # awos-enter --root /mnt -c 'passwd alice'
    ```

    ::: {.note}
    For unattended installations, it is possible to use
    `awos-install --no-root-passwd` in order to disable the password
    prompt entirely.
    :::
...
7.  You should now be able to boot into the installed awos. The GRUB
    boot menu shows a list of *available configurations* (initially just
    one). Every time you change the awos configuration (see [Changing
...
To summarise, [Example: Commands for Installing awos on `/dev/sda`](#ex-install-sequence)
shows a typical sequence of commands for installing awos on an empty hard
drive (here `/dev/sda`). [Example: awos Configuration](#ex-config) shows a
corresponding configuration Nix expression.

::: {#ex-partition-scheme-MBR .example}
### Example partition schemes for awos on `/dev/sda` (MBR)
```ShellSession
# parted /dev/sda -- mklabel msdos
# parted /dev/sda -- mkpart primary 1MB -8GB
# parted /dev/sda -- mkpart primary linux-swap -8GB 100%
...
### Example partition schemes for awos on `/dev/sda` (UEFI)
```ShellSession
# parted /dev/sda -- mklabel gpt
# parted /dev/sda -- mkpart root ext4 512MB -8GB
# parted /dev/sda -- mkpart swap linux-swap -8GB 100%
...
### Commands for Installing awos on `/dev/sda`
...
{ config, pkgs, ... }: {
  ...
  # necessary, since awos-generate-config figures them out
  # automatically in hardware-configuration.nix.
  #fileSystems."/".device = "/dev/disk/by-label/awos";
