...
The awos configuration mechanism is modular. If your
`configuration.nix` becomes too big, you can split it into multiple
files. Likewise, if you have multiple awos configurations (e.g. for
...
option, awos will try to *merge* the definitions. In the case of
...
`awos-rebuild` will give an error:

```plain
The unique option `services.httpd.adminAddr' is defined multiple times, in `/etc/awos/httpd.nix' and `/etc/awos/configuration.nix'.
```
...
configuration option is. The command `awos-option` allows you to find
out:

```ShellSession
$ awos-option services.xserver.enable
true

$ awos-option boot.kernelModules
[ "tun" "ipv6" "loop" ... ]
```
...
in
{
  imports = [ (netConfig "awos.localdomain") ];
}
```
