...
that come as default. This is a role fulfilled by awos's Profiles,
which come as files living in `<nixpkgs/awos/modules/profiles>`. That
is to say, expected usage is to add them to the imports list of your
`/etc/configuration.nix` as such:

```nix
{ imports = [ <nixpkgs/awos/modules/profiles/profile-name.nix> ]; }
```
