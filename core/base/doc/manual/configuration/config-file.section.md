# awos Configuration File {#sec-configuration-file}

The awos configuration file generally looks like this:
...
awos checks your option definitions for correctness. For instance, if
you try to define an option that doesn't exist (that is, doesn't have a
corresponding *option declaration*), `awos-rebuild` will give an error
like:

```plain
The option `services.httpd.enable' defined in `/etc/awos/configuration.nix' does not exist.
```
...
```plain
The option value `services.httpd.enable' in `/etc/awos/configuration.nix' is not a boolean.
```
...
:   Strings are enclosed in double quotes, e.g.
    ...
    Nix manual [here](https://awos.org/nix/manual/#ssec-values).
...
:   For example,
    ...
    because it's not a awos option but the literal name of a Linux
    kernel setting.)
...
:   Usually, the packages you need are already part of the Nix Packages
    ...
    used by awos's PostgreSQL service to 14.x. For more information on
