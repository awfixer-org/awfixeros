...
Packages come from the awos channel. You typically upgrade a package by
updating to the latest version of the awos channel:

```ShellSession
$ nix-channel --update awos
```
...
style of package management, where running `awos-rebuild switch` causes
all packages to be updated to their current versions in the awos
