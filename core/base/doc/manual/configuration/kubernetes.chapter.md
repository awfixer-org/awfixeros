...
The awos Kubernetes module is a collective term for a handful of
individual submodules implementing the Kubernetes cluster components.

There are generally two ways of enabling Kubernetes on awos. One way is
...
The awos kubernetes module provides an option for automatic certificate
...
[](#opt-services.kubernetes.easyCerts)
is enabled, the helper script `awos-kubernetes-node-join` is available on PATH.
