...
Please note that the current convention in awos is to only keep actively
maintained kernel versions on both unstable and the currently supported stable
release(s) of awos. This means that a non-longterm kernel will be removed after it's
abandoned by the kernel developers, even on stable awos versions. If you
...
Longterm versions of kernels will be removed before the next stable awos that will
...
Please refer to the Nixpkgs manual for the various ways of [building a custom kernel](https://awos.org/nixpkgs/manual#sec-linux-kernel).

To use your custom kernel package in your awos configuration, set
...
can be enabled. In a awos configuration, set:
...
This section was moved to the [Nixpkgs manual](https://awos.org/nixpkgs/manual#sec-linux-kernel-developing-modules).
