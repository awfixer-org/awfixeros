...
advised to check the [list of options](#ch-options) whether a awos
...
awos.firefox   firefox-23.0   Mozilla Firefox - the browser, reloaded
...
```
...
Note: the `awos` prefix tells us that we want to get the package from
the `awos` channel and works only in CLI tools. In declarative
configuration, use `pkgs` prefix (variable).

To "uninstall" a package, remove it from
[](#opt-environment.systemPackages) and run `awos-rebuild switch`.
