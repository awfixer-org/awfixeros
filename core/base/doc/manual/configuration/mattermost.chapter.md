...
The awos Mattermost module lets you build [Mattermost](https://mattermost.com)
...
```nix
{
  services.mattermost = {
    ...
    # Override modifications to the config with your awos config.
...
As of awos 25.05, Mattermost uses peer authentication with Postgres or
...
building your own. We test building and using plugins in the awos test suite.
...
```nix
{
  services.mattermost = {
    plugins = with pkgs; [
      ...
      (fetchurl {
        # Note: Don't unpack the tarball; the awos module will repack it for you.
