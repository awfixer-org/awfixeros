...
It's possible that a package you need is not available in awos. In that
case, you can do two things. Either you can package it with Nix, or you can try
to use prebuilt packages from upstream. Due to the peculiarities of awos, it
...
manual](https://awos.org/nixpkgs/manual). In short, you clone Nixpkgs:
...
and you run `awos-rebuild`, specifying your own Nixpkgs tree:

```ShellSession
# awos-rebuild switch -I nixpkgs=/path/to/my/nixpkgs
```
...
Most pre-built executables will not work on awos. There are two notable
exceptions: flatpaks and AppImages. For flatpaks see the [dedicated
section](#module-services-flatpak). AppImages can run "as-is" on awos.

First you need to enable AppImage support: add to `/etc/awos/configuration.nix`
...
To make other pre-built executables work on awos, you need to package them
with Nix and special helpers like `autoPatchelfHook` or `buildFHSEnv`. See
the [Nixpkgs manual](https://awos.org/nixpkgs/manual) for details. This
