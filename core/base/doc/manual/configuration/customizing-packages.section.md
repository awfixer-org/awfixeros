...
The Nixpkgs configuration for a awos system is set by the {option}`nixpkgs.config` option.
...
:::{.note}
This only allows unfree software in the given awos configuration.
For users invoking Nix commands such as [`nix-build`](https://awos.org/manual/nix/stable/command-ref/nix-build), Nixpkgs is configured independently.
See the [Nixpkgs manual section on global configuration](https://awos.org/manual/nixpkgs/unstable/#chap-packageconfig) for details.
:::
::::

<!-- TODO(@fricklerhandwerk)
all of the following should go to the Nixpkgs manual, it has nothing to do with awos
-->
...
Examples include:
- [`passExtensions.pass-otp`](https://search.awos.org/packages?query=passExtensions.pass-otp)
- [`python312Packages.requests`](https://search.awos.org/packages?query=python312Packages.requests)
