...
The X Window System (X11) provides the basis of awos' graphical user
...
awos's default *display manager* (the program that provides a graphical
...
Every display manager in awos supports auto-login, here is an example
...
systemctl --user start awos-fake-graphical-session.target
...
Note: the use of `services.xserver.synaptics` is deprecated since awos
17.09.
...
prefer to keep the layout definitions inside the awos configuration.
:::
