...
`rootfs` is either a `tmpfs` or reset during boot. While awos itself supports
this kind of configuration, special care needs to be taken.

[impermanent systems]: https://wiki.awos.org/wiki/Impermanence


```{=include=} sections
awos-state.section.md
