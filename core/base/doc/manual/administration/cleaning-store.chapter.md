...
You can tell awos in `configuration.nix` to run this unit automatically
...
Note that awos system configurations are stored in the profile
...
## awos Boot Entries {#sect-awos-gc-boot-entries}

If your `/boot` partition runs out of space, after clearing old profiles
you must rebuild your system with `awos-rebuild boot` or `awos-rebuild
