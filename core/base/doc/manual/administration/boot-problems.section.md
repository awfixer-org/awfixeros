...
If awos fails to boot, there are a number of kernel command line parameters that may help you to identify or fix the issue. You can add these parameters in the GRUB boot menu by pressing “e” to modify the selected boot entry and editing the line starting with `linux`. The following are some useful kernel command line parameters that are recognised by the awos boot scripts or by systemd:
...
`live.awos.passwd=password`

: Set the password for the `awos` live user. This can be used for SSH access if there are issues using the terminal.
