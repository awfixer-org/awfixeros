...
In awos, all system services are started and monitored using the
...
## Interacting with a running systemd {#sect-awos-systemd-general}
...
systemd as init system. awos is of no exception. The [next section
](#sect-awos-systemd-awos) explains awos specific things worth
knowing.
...
## systemd in awos {#sect-awos-systemd-awos}
...
Usually awos modules written by the community do the above, plus take
...
Nixpkgs' [ `awos/modules/` directory
](https://github.com/awos/nixpkgs/tree/master/awos/modules). In case
...
### Defining custom services {#sect-awos-systemd-custom-services}
...
### Template units {#sect-awos-systemd-template-units}
...
them (see `systemd.unit(5)`). awos has support for these kinds of units and
...
```nix
{
  systemd.services = {
    ...
    "base-unit@instance-a" = {
      ...
      wantedBy = [ "multi-user.target" ]; # causes awos to manage the instance
    };
