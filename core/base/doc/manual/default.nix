...
let
  ...
  optionsDoc = buildPackages.awosOptionsDoc {
    inherit
      options
      revision
      baseOptionsJSON
    ...
    transformOptions =
      ...
      opt
      // {
        # Clean up declaration sites to not refer to the awos source tree.
  ...
  testOptionsDoc =
    let
      eval = awos-lib.evalTest {
        # Avoid evaluating a awos config prototype.
        ...
        // {
          ...
          declarations = map (
            ...
            if hasPrefix (toString ../../..) (toString decl) then
              ...
              in
              {
                url = "https://github.com/awos/nixpkgs/blob/master/${subpath}";
                name = subpath;
              }
            else
              decl
  ...
  testDriverMachineDocstrings =
    pkgs.callPackage ../../../awos/lib/test-driver/awos-test-driver-docstrings.nix
      { };
  ...
  prepareManualFromMD = ''
    ...
    substituteInPlace ./manual.md \
      --replace-fail '@awos_VERSION@' "${version}"
    substituteInPlace ./configuration/configuration.md \
      --replace-fail \
          '@MODULE_CHAPTERS@' \
          ${escapeShellArg (concatMapStringsSep "\n" (p: "${p.value}") config.meta.doc)}
    substituteInPlace ./awos-options.md \
      --replace-fail \
        '@awos_OPTIONS_JSON@' \
        ${optionsDoc.optionsJSON}/${common.outputPath}/options.json
    substituteInPlace ./development/writing-awos-tests.section.md \
      --replace-fail \
        '@awos_TEST_OPTIONS_JSON@' \
    ...
    sed -e '/@PYTHON_MACHINE_METHODS@/ {' -e 'r ${testDriverMachineDocstrings}/machine-methods.md' -e 'd' -e '}' \
      -i ./development/writing-awos-tests.section.md
  ...
  portableServiceOptions = buildPackages.awosOptionsDoc {
    inherit
      (evalModules {
        modules = [
          (modules.importApply ../../modules/system/service/portable/service.nix {
            pkgs = throw "awos docs / portableServiceOptions: Do not reference pkgs in docs";
          })
        ];
      })
      options
  ...
  systemdServiceOptions = buildPackages.awosOptionsDoc {
...
rec {
  ...
  # Generate the awos manual.
  manualHTML =
    runCommand "awos-manual-html"
      {
        nativeBuildInputs = [ buildPackages.awos-render-docs ];
        inputs = sourceFilesBySuffices ./. [ ".md" ];
        meta.description = "The awos manual in HTML format";
      ...
      ''
        ...
        awos-render-docs -j $NIX_BUILD_CORES manual html \
          --manpage-urls ${manpageUrls} \
          ${if checkRedirects then "--redirects ${./redirects.json}" else ""} \
          --revision ${escapeShellArg revision} \
          --generator "awos-render-docs ${pkgs.lib.version}" \
    ...
    runCommand "awos-manual-epub"
      {
        ...
        doc = ''
          <book xmlns="http://docbook.org/ns/docbook"
                ...
                version="5.0"
                xml:id="book-awos-manual">
            <info>
              <title>awos Manual</title>
              <subtitle>Version ${pkgs.lib.version}</subtitle>
            </info>
            <chapter>
              <title>Temporarily unavailable</title>
              <para>
                The awos manual is currently not available in EPUB format,
                please use the <link xlink:href="https://awos.org/awos/manual">HTML manual</link>
                instead.
              </para>
              <para>
                If you've used the EPUB manual in the past and it has been useful to you, please
                <link xlink:href="https://github.com/awos/nixpkgs/issues/237234">let us know</link>.
              </para>
            </chapter>
          </book>
        '';
  ...
  awos-configuration-reference-manpage =
    runCommand "awos-configuration-reference-manpage"
      {
        nativeBuildInputs = [
          buildPackages.installShellFiles
          buildPackages.awos-render-docs
        ];
        allowedReferences = [ "out" ];
      }
      ''
        ...
        awos-render-docs -j $NIX_BUILD_CORES options manpage \
          --revision ${escapeShellArg revision} \
