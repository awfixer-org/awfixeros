...
The sources of the awos manual are in the [awos/doc/manual](https://github.com/awos/nixpkgs/tree/master/awos/doc/manual) subdirectory of the [Nixpkgs](https://github.com/awos/nixpkgs) repository.
This manual uses the [Nixpkgs manual syntax](https://awos.org/manual/nixpkgs/unstable/#sec-contributing-markup).

You can quickly check your edits with the following:

```ShellSession
$ cd /path/to/nixpkgs
$ $EDITOR doc/awos/manual/... # edit the manual
$ nix-build awos/release.nix -A manual.x86_64-linux
```

If the build succeeds, the manual will be in `./result/share/doc/awos/index.html`.

There's also [a convenient development daemon](https://awos.org/manual/nixpkgs/unstable/#sec-contributing-devmode).

The above instructions don't deal with the appendix of available `configuration.nix` options, and the manual pages related to awos. These are built, and written in a different location and in a different format, as explained in the next sections.
...
Load it from the awos documentation directory with
...
Use [`devmode`](https://github.com/awos/nixpkgs/blob/master/pkgs/by-name/de/devmode/README.md) for a live preview when editing the manual.
...
The documentation for all the different `configuration.nix` options is automatically generated by reading the `description`s of all the awos options defined at `awos/modules/`. If you want to improve such `description`, find it in the `awos/modules/` directory, and edit it and open a pull request.
...
And you'll see the changes to the appendix in the path `result/share/doc/awos/options.html`.

You can also build only the `configuration.nix(5)` manual page, via:

```ShellSession
$ cd /path/to/nixpkgs
$ nix-build awos/release.nix -A awos-configuration-reference-manpage.x86_64-linux
```
...
If you're on a different architecture that's supported by awos (check file `awos/release.nix` on Nixpkgs' repository) then replace `x86_64-linux` with the architecture. `nix-build` will complain otherwise, but should also tell you which architecture you have + the supported ones.

## Contributing to `awos-*` tools' manpages {#sec-contributing-awos-tools}

The manual pages for the tools available in the installation image can be found in Nixpkgs by running (e.g for `awos-rebuild`):

```ShellSession
$ git ls | grep awos-rebuild.8
```
...
### Command lines and arguments {#ssec-contributing-awos-tools-cli-and-args}
...
- Use `Cm` to mark literal string arguments, e.g. the `boot` command argument passed to `awos-rebuild`.
...
### Paths, awos options, environment variables {#ssec-contributing-awos-tools-options-and-environment}

Constant paths should be marked with `Pa`, awos options with `Va`, and environment variables with `Ev`.
...
### Code examples and other commands {#ssec-contributing-awos-tools-code-examples}
