...
let
  ...
  # See also awosTests in pkgs/top-level/all-packages.nix
  allTestsForSystem =
    system:
  ...
  versionModule =
    { config, ... }:
    {
      system.awos.versionSuffix = versionSuffix;
      system.awos.revision = nixpkgs.rev or nixpkgs.shortRev;

      # At creation time we do not have state yet, so just default to latest.
      system.stateVersion = config.system.awos.release;
    };
  ...
  makeSystemTarball =
    ...
    // {
      meta = {
        description = "awos system tarball for ${system} - ${stdenv.hostPlatform.linux-kernel.name}";
...
rec {
  ...
  awos-configuration-reference-manpage = buildFromConfig ({ ... }: { }) (
    config: config.system.build.manual.awos-configuration-reference-manpage
  );
  options =
  ...
  # Ensure that all packages used by the minimal awos config end up in the channel.
  dummy = forAllSystems (
    system:
