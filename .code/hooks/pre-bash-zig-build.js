...
if (argv0 === "zig") {
  ...
  if (
    ...
    positionalArgs[0] === "build" &&
    positionalArgs[1] === "obj"
  ) {
...
if (argv0 === "timeout") {
  ...
  const timeoutArgEndIndex = tokens
    .slice(1)
    .findIndex((t) => !t.startsWith("-") && !/^\d/.test(t));
  ...
  if (actualCommand === "bun" || actualCommand.includes("bun-debug")) {
    ...
    const redirectStderrToStdoutIndex = positionalArgs.findIndex(
      (arg) => arg === "2>&1",
    );
    if (redirectStderrToStdoutIndex !== -1) {
      positionalArgs.splice(redirectStderrToStdoutIndex, 1);
    }
    const redirectStdoutToStderrIndex = positionalArgs.findIndex(
      (arg) => arg === "1>&2",
    );
    ...
    const redirectToFileAppendIndex = positionalArgs.findIndex(
      (arg) => arg === ">>",
    );
    ...
    const redirectTOFileInlineIndex = positionalArgs.findIndex((arg) =>
      arg.startsWith(">"),
    );
...
if (argv0 === "bun" || argv0.includes("bun-debug")) {
  ...
  const hasTest =
    positionalArgs.includes("test") ||
    (positionalArgs[0] === "bd" && positionalArgs[1] === "test");

  if (hasTest) {
    const hasUpdateSnapshots = allArgs.some(
      (arg) => arg === "-u" || arg === "--update-snapshots",
    );
    const hasTestNamePattern = allArgs.some(
      (arg) => arg === "-t" || arg === "--test-name-pattern",
    );

    if (hasUpdateSnapshots && hasTestNamePattern) {
      denyWithReason(
        "error: Cannot use -u/--update-snapshots with -t/--test-name-pattern",
      );
    }
  }
}
